...
`Subquery` gained a `scalar_subquery_index` field used during physical
planning. Use `Subquery::new(...)` when constructing unplanned subqueries,
and include `..` in exhaustive `Subquery` patterns.
