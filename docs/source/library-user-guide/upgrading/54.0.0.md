...
The logical optimizer now reorders the conjuncts of an `AND`-only filter so
that cheap predicates (most binary comparisons, `IS NULL`, `Between`, `InList`,
etc.) evaluate before expensive ones (`LIKE`, regex, scalar function calls,
subqueries). For example, `WHERE col LIKE '%foo%' AND col2 = 5` may evaluate
`col2 = 5` before `col LIKE '%foo%'`.

**Evaluation order is not guaranteed to match the order written in the query.**
The SQL standard explicitly allows implementations to evaluate operands in any
order; major engines (PostgreSQL, SQL Server, Oracle, MySQL) document the
same. Queries should not rely on left-to-right evaluation or short-circuit
semantics for `AND` or `OR`.

**Fallible-predicate patterns are particularly affected.** For example, a query
such as:
...
The intent is likely to filter non-numeric strings before the `CAST` runs, but
this depends on evaluation-order behavior that has never been guaranteed by the
SQL standard and may now raise a runtime cast error if the optimizer reorders
the conjuncts. To force conditional evaluation, rewrite using `CASE`, which has
