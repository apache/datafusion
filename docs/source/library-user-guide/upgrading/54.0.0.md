...
### Remove `as_any` from `ScalarUDFImpl`, `AggregateUDFImpl`, `WindowUDFImpl`, `ExecutionPlan`, and `TableProvider`
...
`AggregateUDFImpl`, `WindowUDFImpl`, `ExecutionPlan`, and `TableProvider`, you
can simply remove the `as_any` function. The below diffs are examples from the
associated PRs.
...
`dyn ScalarUDFImpl`, `dyn AggregateUDFImpl`, and `dyn WindowUDFImpl`. They work correctly
