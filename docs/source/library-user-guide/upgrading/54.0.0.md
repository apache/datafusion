...
The full API on `FileExtensions` is `insert` / `insert_dyn` / `get` /
`get_arc` / `contains` / `remove` / `merge`, all generic over the
concrete type `T`. Values are stored as `Arc<T>` so the map remains cheap
to clone.
