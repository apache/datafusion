...
If you already have an `Arc<T>` and want to avoid an extra allocation,
use `with_extension_arc`:

```rust,ignore
let pf = PartitionedFile::new(path, size)
    .with_extension_arc(Arc::clone(&shared_access_plan));
```
...
The full API on `FileExtensions` is `insert` / `insert_arc` /
`insert_dyn` / `get` / `get_arc` / `contains` / `remove` / `merge`, all
generic over the concrete type `T`. Values are stored as `Arc<T>` so the
map remains cheap to clone.
