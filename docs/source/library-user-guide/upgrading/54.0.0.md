...
`datafusion_physical_expr::expressions::CastColumnExpr` has been removed. The
physical expression layer now uses one cast representation:
`datafusion_physical_expr::expressions::CastExpr`.
...
field semantics, use `CastExpr::new_with_target_field(...)` or the
`cast_with_target_field(...)` helper. The type-only `CastExpr::new(...)` and
`cast(...)` helpers remain available for callers that only have a `DataType`.
