...
users need to implement. In your implementations of `PhysicalExpr`,
`ScalarUDFImpl`, `AggregateUDFImpl`, `WindowUDFImpl`, `ExecutionPlan`,
`TableProvider`, `SchemaProvider`, `CatalogProvider`, and
`CatalogProviderList`, you can simply remove the `as_any` function. The below
diffs are examples from the associated PRs.
...
These methods are available on `dyn PhysicalExpr`, `dyn ExecutionPlan`,
`dyn TableProvider`, `dyn SchemaProvider`, `dyn CatalogProvider`,
`dyn CatalogProviderList`, `dyn ScalarUDFImpl`, `dyn AggregateUDFImpl`,
and `dyn WindowUDFImpl`.
They work correctly
