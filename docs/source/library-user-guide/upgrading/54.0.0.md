...
### Remove `as_any` from `ScalarUDFImpl`, `AggregateUDFImpl`, `WindowUDFImpl`, `ExecutionPlan`, `TableProvider`, `SchemaProvider`, and `CatalogProvider`
...
`SchemaProvider`, and `CatalogProvider`, you can simply remove the `as_any`
...
`dyn SchemaProvider`, `dyn CatalogProvider`, `dyn ScalarUDFImpl`,
