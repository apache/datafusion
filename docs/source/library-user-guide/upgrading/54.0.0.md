...
### Remove `as_any` from `ScalarUDFImpl`, `AggregateUDFImpl`, `WindowUDFImpl`, `ExecutionPlan`, `TableProvider`, and `SchemaProvider`
...
`AggregateUDFImpl`, `WindowUDFImpl`, `ExecutionPlan`, `TableProvider`, and
`SchemaProvider`, you can simply remove the `as_any` function. The below diffs
are examples from the associated PRs.
...
`dyn SchemaProvider`, `dyn ScalarUDFImpl`, `dyn AggregateUDFImpl`, and
`dyn WindowUDFImpl`. They work correctly
