...
### Planner and physical optimizer interfaces moved to `datafusion-session`

`QueryPlanner`, `PhysicalPlanner`, `ExtensionPlanner`, `PhysicalOptimizerRule`,
and `PhysicalOptimizerContext` now live in the `datafusion-session` crate. The
previous paths continue to work through re-exports:
...
`Session` also provides `query_planner`, `optimize`, `physical_optimizers`, and
`statistics_registry`. The default implementations report that the query
planner is unavailable, leave logical plans unchanged, return no physical
optimizer rules, and return no statistics registry, respectively. A custom
session that uses `DefaultQueryPlanner` or `DefaultPhysicalPlanner` should
override these methods to expose its planning and optimization behavior.

`ForeignSession::create_physical_plan` continues to run the complete planning
pipeline in the library that owns the session. Directly passing a
`ForeignSession` to a local `DefaultPhysicalPlanner` uses the default local
optimization capabilities described above. FFI wrappers for the individual
planner and optimizer interfaces are not included in this release.
