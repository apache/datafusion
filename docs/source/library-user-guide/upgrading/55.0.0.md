...
`Session` now also requires a `query_planner` method. Implementations that do
not expose a query planner can return the new `UnsupportedQueryPlanner`:

```rust
use std::sync::Arc;
use datafusion_session::{QueryPlanner, UnsupportedQueryPlanner};
...
The `optimize`, `physical_optimizers`, and `statistics_registry` methods have
default implementations that leave logical plans unchanged, return no physical
