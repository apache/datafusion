...
### `MergeIntoOp` must be constructed with its target qualifier

`MergeIntoOp` now preserves the SQL-visible target qualifier separately from the
target table's catalog identity. The struct is also non-exhaustive, so replace
struct literals with `MergeIntoOp::new`:
...
Use `MergeIntoOp::target_qualifier()` to read the qualifier. MERGE planning and
provider dispatch use it to distinguish an aliased target from source relations.
