...
`ExecutionPlan::try_to_proto` let a plan serialize itself, but there was no way
back: decoding an extension node always routed through
...
`ComposedPhysicalExtensionCodec` had to try each registered codec in turn and
read a decode error as "not mine", which made resolution depend on registration
order and on error strings, and made a name collision between two independent
crates undetectable.

`PhysicalExtensionNode` now has an optional `plan_name`, and a session can map
that name to a decoder. An extension plan implements `ExtensionExecutionPlan`
...
impl ExecutionPlan for MyExec {
    ...
    fn try_to_proto(
        ...
        // The name comes from `Self::PLAN_NAME`, so it cannot drift from the
        // name you register.
...
impl ExtensionExecutionPlan for MyExec {
    ...
    // registration rather than a mis-decode.
    ...
    fn try_from_proto(
        ...
        my_plan_from_bytes(payload, children)
    }
}
...
- Nobody is required to change anything. This is opt-in per plan type: a
  ...
  not know, takes the `PhysicalExtensionCodec` chain exactly as before, and the
  wire format stays compatible in both directions.
- Users constructing `PhysicalExtensionNode` with an exhaustive struct literal
  must set the new `plan_name` field (`None` reproduces the previous behavior).
- Replacing a `PhysicalExtensionCodec` with a registration changes only _how_
  the payload is dispatched, not what the payload is. Readers running an older
  DataFusion ignore `plan_name` and hand the payload to their codec, so a mixed
  fleet keeps working only while the bytes your `try_to_proto` writes stay
  decodable by the codec you removed.

`PhysicalExtensionCodec` is not deprecated: extension `PhysicalExpr`s and UDF
payloads still need it, and it remains the fallback for unmigrated plans.
