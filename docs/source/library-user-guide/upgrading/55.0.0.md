...
are merged into one generic implementation `DefaultCache`. Also the traits `CacheAccessor`,
`FileMetadataCache`, `ListFilesCache` and `FileStatisticsCache` are merged into the `Cache` trait.
...
Use the new `Cache` trait for your custom cache implementation.

See https://github.com/apache/datafusion/pull/22613 for details.
