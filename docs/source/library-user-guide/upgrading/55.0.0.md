...
no longer provides a default implementation. Custom `GroupsAccumulator`
implementations must now implement it:
...
The `GroupsAccumulator::supports_convert_to_state` capability method has also
been removed. Implementations that previously provided it should delete it:
...
This also removes the `supports_convert_to_state` field from
`datafusion_ffi::udaf::groups_accumulator::FFI_GroupsAccumulator`, changing its
ABI layout. FFI providers and consumers must be rebuilt against DataFusion 55
and must not exchange this struct with older major versions.
