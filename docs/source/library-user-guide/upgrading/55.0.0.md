...
`TableSchema::new` and `TableSchema::from_file_schema` remain as conveniences.
`TableSchema::with_table_partition_cols` (the setter on an existing
`TableSchema`) is **deprecated** in favor of the builder; it now _replaces_
rather than appends:

```rust
// Before
let ts = TableSchema::from_file_schema(file_schema)
