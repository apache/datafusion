...
### `ExprType` gained a `NormalizeFloatZero` variant

Floating-point equality keys in ASOF joins now use a
`NormalizeFloatZeroExpr` physical expression to ensure that `-0.0` and `+0.0`
have the same ordering and equality semantics. The generated `ExprType` enum
on `PhysicalExprNode` gained a matching `NormalizeFloatZero` variant (tag 29).
...
Add a `NormalizeFloatZero` arm, or fall back to a wildcard arm:

```rust,ignore
match expr_type {
    // ...
    ExprType::NormalizeFloatZero(node) => { /* ... */ }
    _ => { /* ... */ }
}
```

Plans encoded before this variant existed are unaffected: they never produced
this variant, so decoding is unchanged. Plans containing a
`NormalizeFloatZeroExpr` encoded after this change fail to decode on older
readers that don't recognize the new tag.

See [PR #24375](https://github.com/apache/datafusion/pull/24375) for details.
