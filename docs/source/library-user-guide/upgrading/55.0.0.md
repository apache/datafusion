...
`datafusion_expr::execution_props::SubqueryContext` passed explicitly to the
lowering functions and planner traits. This removes the need for the physical
planner to clone and mutate a `SessionState`, is a prerequisite for letting the
planner take `&dyn Session`, and lets `ExtensionPlanner` implementations lower
...
- `ExtensionPlanner::plan_extension` and `plan_table_scan` receive
  ...
  `PhysicalPlanner::create_physical_expr` when lowering expressions
...
- Code calling the lowering functions above: pass
  `&SubqueryContext::default()` unless you are lowering expressions inside a
  physical plan that contains uncorrelated scalar subqueries.
...
For expression lowering outside of physical planning, pass an empty context:
...
If you populated the removed `ExecutionProps` fields to lower
`Expr::ScalarSubquery` yourself, build a `SubqueryContext` and pass it
explicitly:
