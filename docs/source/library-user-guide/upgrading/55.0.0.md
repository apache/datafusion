...
### `SessionStateBuilder` function-registry getters now return `HashMap` instead of `Vec`

The four `SessionStateBuilder` getters that expose the scalar, higher-order,
aggregate, and window function registries now return an alias-keyed `HashMap`
instead of a `Vec`:
...
(and likewise for `higher_order_functions`, `aggregate_functions`, and
`window_functions`). This matches the shape `SessionState` stores internally, so
`SessionStateBuilder::new_from_existing` carries each registry forward verbatim
instead of flattening it into a non-deterministic order. The
`with_*_functions(Vec<Arc<..>>)` setters are unchanged.

**Who is affected:**

- Code that mutates a builder registry through its getter, e.g.
  `builder.scalar_functions().get_or_insert_with(Vec::new).extend(...)`.

Read-only callers and code using the `with_*_functions` setters are unaffected.

**Migration guide:**

Key your functions before inserting them. The public helper
`index_by_name_and_aliases` builds the same map the setters do (each function
indexed by its primary name and every alias, last-writer-wins):
...
-     .extend(my_scalar_udfs);
+     .get_or_insert_with(HashMap::new)
+     .extend(index_by_name_and_aliases(my_scalar_udfs));
```

See [PR #23850](https://github.com/apache/datafusion/pull/23850) for details.
