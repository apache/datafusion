...
### `PushedDown` distinguishes exact, inexact, and unsupported filter pushdown

`datafusion_physical_plan::filter_pushdown::PushedDown` now reports whether a
consumer applies a predicate exactly, retains it for an inexact use such as
statistics pruning, or does not retain it:

```rust,ignore
// Before                    // After
PushedDown::Yes              PushedDown::Exact
PushedDown::No               PushedDown::Unsupported
```

Implementations that retain a predicate without applying it as a complete row
filter must return `PushedDown::Inexact`. This keeps an exact `FilterExec` in
the plan while informing dynamic-filter producers that the predicate has a
consumer. `PushedDownPredicate::supported` remains available as an alias for
the new `PushedDownPredicate::exact` constructor.
