...
### `INTERSECT ALL` and `EXCEPT ALL` require the `count` and `range` functions

SQL `INTERSECT ALL` and `EXCEPT ALL`, `DataFrame::intersect`, `DataFrame::except`,
and Substrait multiset `INTERSECTION_MULTISET_ALL` and `MINUS_PRIMARY_ALL` plans
now count the rows on each side and expand the result with `unnest(range(..))`
to keep the correct number of duplicate rows. A custom `ContextProvider` or
`SessionState` that does not register the `count` aggregate function and the
`range` scalar function (for example one built with `SessionStateBuilder::new()`
without `with_default_features()`) now returns a planning error for these
queries. Register `datafusion_functions_aggregate::count::count_udaf()` and
`datafusion_functions_nested::range::range_udf()` to plan them.

`LogicalPlanBuilder::intersect` and `LogicalPlanBuilder::except` with
`is_all = true` still build the old plan, which does not preserve duplicate
counts. Use `LogicalPlanBuilder::intersect_all` and
`LogicalPlanBuilder::except_all` instead.
