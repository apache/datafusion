...
### `ExecutionOptions` has a new `soft_max_bytes_per_output_file` field

`ExecutionOptions` gained a public `soft_max_bytes_per_output_file: ConfigNonZeroUsize`
field (default 4 GiB minus one byte, or 4,294,967,295 bytes). When writing
multiple output files, DataFusion starts a new file once a writer reports that
the encoded size has reached this target or the existing
`soft_max_rows_per_output_file` target has been reached. The byte limit is soft:
buffered batches, in-flight batch serialization, individual batch sizes, and
file metadata can make a completed file larger than the target. It does not
apply to single-file output.

**Who is affected:**

- Users constructing `ExecutionOptions` (or `ConfigOptions`) with an exhaustive
  struct literal must provide the new field.
- Users writing multiple output files can set a smaller byte target to rotate
  files before they reach the default size.

**Migration guide:**

Set the field explicitly, or fill it from `Default` as shown above. For example,
to target 128 MiB output files in Rust:

```rust,ignore
let mut config = SessionConfig::new();
config.options_mut().execution.soft_max_bytes_per_output_file =
    ConfigNonZeroUsize::try_new(128 * 1024 * 1024)?;
```

The same setting can be changed with SQL:

```sql
SET datafusion.execution.soft_max_bytes_per_output_file = 134217728;
```
