...
### `ParquetAccessPlan::into_overall_row_selection` was removed

`ParquetAccessPlan::into_overall_row_selection` has been removed. DataFusion now
keeps selections local to each row group during Parquet scanning.

Users who attach a `ParquetAccessPlan` to a `PartitionedFile` for DataFusion to
read need no changes. Only code that directly calls the removed conversion
method needs to migrate; there is no replacement public conversion method.
...
indexes, and each selection uses coordinates within its own row group. Callers
must now handle conversion and selection-length validation themselves. Parquet's
push decoder and async reader accept `RowGroupSelection` values through
`with_row_group_selections`; the synchronous reader still requires
`with_row_groups` and an overall `with_row_selection`.
