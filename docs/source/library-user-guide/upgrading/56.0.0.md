...
### `INTERSECT ALL` and `EXCEPT ALL` require the `row_number` window function
...
now use `row_number()` to keep the correct number of duplicate rows. A custom
`ContextProvider` or `SessionState` that does not register `row_number` (for
example one built with `SessionStateBuilder::new()` without
`with_default_features()`) now returns a planning error for these queries.
Register `datafusion_functions_window::row_number::row_number_udwf()` to plan them.
