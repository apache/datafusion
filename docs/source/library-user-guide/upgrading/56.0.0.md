...
`ChildFilterDescription::from_child_with_allowed_indices` is deprecated and
also resolves columns by position. Its allowed indices now map to the same
indices in the child schema; it no longer preserves the previous name-based
mapping. Existing callers with different parent and child column positions
must migrate, even if all column names are unique, or filters may reference
the wrong child column.
