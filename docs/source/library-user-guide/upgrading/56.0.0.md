...
### Null-aware joins record their `NOT IN` value key count

Multi-column `(a, b) NOT IN (SELECT x, y ...)` is now planned as a null-aware
join. Its first equi-join keys are the tuple elements, followed by any
correlation keys, so a null-aware join now records how many of its leading keys
are `NOT IN` value keys:

- `datafusion_expr::logical_plan::Join` has a new `null_aware_value_keys: usize`
  field.
- `HashJoinExec` has a new `null_aware_value_keys: usize` field, set with
  `HashJoinExecBuilder::with_null_aware_value_keys`.

The count is only read when `null_aware` is set. `Join::try_new`, the
`LogicalPlanBuilder` join methods and `HashJoinExec::try_new` set it to `1`,
the scalar `NOT IN` of earlier releases. Code constructing `Join` with a struct
literal must initialize the field:
...
The generated protobuf `JoinNode` and `HashJoinExecNode` types have a new
`null_aware_value_keys` field, which struct literals must provide too. A 56.0
reader decodes a payload without the field (value `0`) as a scalar `NOT IN`. A
55.0 reader must not consume a 56.0 plan holding a multi-column `NOT IN`: it
ignores the unknown field and would read the extra tuple elements as
correlation keys, returning wrong results.
