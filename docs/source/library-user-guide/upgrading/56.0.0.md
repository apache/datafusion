...
### `FileFormat::create_physical_plan` receives the scan's filters

`FileFormat::create_physical_plan` has a new `filters: &[Arc<dyn PhysicalExpr>]`
argument. `ListingTable` passes the (non partition) filters of the query that
apply to the scan, expressed against the table schema and combined with an
implicit `AND`. The filters are hints: they are still evaluated above the scan,
so a format may use them to skip data that cannot match, for example to refine
statistics during planning, but must not rely on them for correctness.
`ParquetFormat` uses them for eager pruning, see
`datafusion.execution.parquet.eager_pruning`.

**Who is affected:**

- Implementations of the `FileFormat` trait.
- Code that calls `FileFormat::create_physical_plan` directly.

**Migration guide:**

Add the argument to implementations. Formats that do not use the filters can
ignore it:
...
Callers without filters pass an empty slice:

```rust,ignore
format.create_physical_plan(state, conf, &[]).await?;
```

Formats that wrap another format should forward the filters.

### `ParquetOptions` has new `eager_pruning` and `eager_pruning_file_limit` fields

`ParquetOptions` gained the public fields `eager_pruning: EagerParquetPruning`
(default `EagerParquetPruning::Disabled`) and `eager_pruning_file_limit: usize`
(default `256`). With eager pruning enabled, parquet scans prune row groups (and
optionally data pages and bloom filters) while the query is planned, to refine
the statistics of the scan. The generated protobuf `ParquetOptions` message
gained the corresponding `eager_pruning` field and the optional
`eager_pruning_file_limit_opt` field.

**Who is affected:**

- Users constructing `ParquetOptions` or the generated protobuf
  `ParquetOptions` with an exhaustive struct literal. Reading or `SET`-ing
  configuration is unaffected.

**Migration guide:**

Set the new fields, or fill them from `Default`:
