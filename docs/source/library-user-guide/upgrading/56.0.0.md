...
### `datafusion.execution.optional_filter_mode` defaults to `adaptive`

The default value of `datafusion.execution.optional_filter_mode` is now `adaptive` (previously `always`). This change has an effect only when `datafusion.execution.parquet.pushdown_filters` is `true`.

Optional filters are filters that are not necessary for correct results, such as the dynamic filters that hash joins, TopK and aggregates push down into the Parquet scan. With `adaptive`, the Parquet scan pauses an optional filter when it removes too few rows (see `datafusion.execution.optional_filter_max_pass_ratio`) or when it costs more than it saves (see `datafusion.execution.optional_filter_min_saving_ns_per_row`). The scan tries a paused filter again at intervals. Query results do not change. Statistics pruning of files, row groups and pages continues to use all optional filters.

To get the previous behavior, set the option to `always`:
