...
### Adaptive optional filters and adaptive filter placement are the default

The default value of `datafusion.execution.optional_filter_mode` is now `adaptive` (previously `always`), and the default value of `datafusion.execution.adaptive_filter_placement` is now `true` (previously `false`). The Parquet scan uses these two options only when `datafusion.execution.parquet.pushdown_filters` is `true`.

Optional filters are filters that are not necessary for correct results, such as the dynamic filters that hash joins, TopK and aggregates push down into scans. With `adaptive`, the Parquet scan and `FilterExec` pause an optional filter when it removes no rows or when it costs more than it saves (see `datafusion.execution.optional_filter_min_saving_ns_per_row`). They try a paused filter again at intervals. Statistics pruning of files, row groups and pages continues to use all optional filters.

With `adaptive_filter_placement`, each filter conjunct starts as a filter on the decoded batches. The Parquet scan makes a conjunct a row filter (late materialization) at a row group boundary only when the measurements show that the row filter saves more decode time than it costs.

Query results do not change. To get the previous behavior, set the options to their previous values:
