...
### `datafusion.execution.enforce_batch_size_in_joins` is deprecated and ignored

The `datafusion.execution.enforce_batch_size_in_joins` config flag is deprecated
and now ignored (setting it emits a deprecation warning). Symmetric Hash Join
always splits output incrementally to `batch_size`.

**Who is affected:**

- Anyone who set `datafusion.execution.enforce_batch_size_in_joins` or called
  `SessionConfig::with_enforce_batch_size_in_joins` /
  `SessionConfig::enforce_batch_size_in_joins`. The setting and methods are
  still accepted (with a deprecation warning) but have no effect, and will be
  removed in a future release.

**Migration guide:**

- Drop any `enforce_batch_size_in_joins` setting or `SessionConfig` calls.
- Use `datafusion.execution.batch_size` to control join output batch size.
