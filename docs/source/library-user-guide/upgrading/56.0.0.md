...
### `Timestamp(u, Some(tz)) - Timestamp(u, None)` reads the naive operand in `tz`

Subtracting a timezone-naive timestamp from a timezone-aware one _at the same
time unit_ did not coerce at all in `55.0.0`: the naive operand was read as UTC,
which differed both from `=` on the same pair of values and from every other
pairing of time units. It now coerces like the rest, so the naive operand is
read in the aware operand's time zone:

```sql
-- ts_tz is 2024-11-01T00:00:00-04:00 (America/New_York), ts is the naive
-- 2024-11-01T00:00:00; ts_tz = ts is true in both versions
-- 55.0.0: 0 days 4 hours 0 mins 0.000000000 secs
-- 56.0.0: 0 days 0 hours 0 mins 0.000000000 secs
SELECT ts_tz - ts FROM timestamps;
```
