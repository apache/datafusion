...
### Session time zone now applies to mixed timestamp operations

When `datafusion.execution.time_zone` is set, DataFusion now uses it to
interpret timezone-naive timestamps in comparisons and subtraction with
timezone-aware timestamps. This applies to binary comparisons, `IN`,
`BETWEEN`, `CASE expr WHEN`, `IN` subqueries, and `ANY` or `ALL`
comparisons.

Previously, the inferred timezone could depend on the operand precision or the
timezone-aware operand's timezone. Equivalent comparison forms and subtraction
could therefore disagree. The new behavior matches PostgreSQL and DuckDB.

The default remains `None`, so applications that do not configure a session
timezone retain the previous behavior. Applications that set this option should
review queries that mix timezone-aware and timezone-naive timestamps.
