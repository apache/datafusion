...
`docs/source/user-guide/sql/` are generated by `dev/update_config_docs.sh` and
`dev/update_function_docs.sh`. `ci/scripts/check_config_function_docs.sh`
regenerates all four and fails if a committed page differs. `./dev/rust_lint.sh`
runs it and needs `cargo` and `npx`. To run the check on its own, or to update
the pages:

```shell
./ci/scripts/check_config_function_docs.sh
./ci/scripts/check_config_function_docs.sh --write
```

`--write` replaces the four pages, so edit the Rust doc comments or the update
scripts instead of the generated Markdown.
