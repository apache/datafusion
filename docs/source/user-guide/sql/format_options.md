...
| MAX_ROW_GROUP_BYTES                        | No                      | Sets the maximum size of each row group in bytes. When both this and `MAX_ROW_GROUP_SIZE` are set, the row group flushes whenever either limit is reached. Mirrors `parquet.block.size` from parquet-mr.                                                                                                                              | `'max_row_group_bytes'`                               | None                     |
