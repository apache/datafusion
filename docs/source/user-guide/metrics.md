...
| max_sliced_batch_size | Estimated size (in bytes) of the largest slice produced when splitting a batch. Measured via `get_sliced_size` like `max_spilled_batch_size`, but on a slice that is _not_ GC-compacted first, so buffers shared across columns may be counted multiple times — hence an estimate. |
