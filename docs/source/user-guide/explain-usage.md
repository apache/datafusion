...
- `optional_filter_rows_skipped`: rows for which the scan skipped an optional filter (a filter that is not needed for correctness, such as a join dynamic filter) because the filter removed no rows or cost more than it saved. Shown only when `datafusion.execution.optional_filter_mode` is `adaptive` and the scan skipped a filter.
