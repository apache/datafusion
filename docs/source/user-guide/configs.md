...
| datafusion.execution.enable_materialized_ctes | false | Should DataFusion materialize CTEs that are referenced multiple times. When enabled, CTEs referenced more than once are generally computed once and cached, except for cheap CTEs and CTEs consumed below a top-level limit. |
