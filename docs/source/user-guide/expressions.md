...
| approx_top_k(expr, k)                                                           | Returns the approximate `k` most frequent values for `expr` with their estimated counts, ranked by the lower-bound estimate and then by count.           |
