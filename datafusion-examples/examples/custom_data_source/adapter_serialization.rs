...
//!
//! This example demonstrates how to preserve custom `PhysicalExprAdapter` information
//! during plan serialization/deserialization using the `PhysicalExtensionCodec` trait's
...
//! of the `PhysicalExtensionCodec` interception pattern.
...
pub async fn adapter_serialization() -> Result<()> {
    ...
    println!(
        "  1. PhysicalExtensionCodec::serialize_physical_plan can intercept any plan"
    );
    ...
    println!("  4. Adapters are preserved across the serialization round-trip");
...
/// A custom PhysicalExprAdapter that wraps another adapter and adds a metadata tag.
/// The tag is preserved during serialization.
#[derive(Debug, Clone)]
...
/// The tag is stored in the factory and passed to created adapters.
#[derive(Debug)]
...
impl PhysicalExtensionCodec for AdapterPreservingCodec {
    ...
    fn try_decode(
        ...
        not_impl_err!("AdapterPreservingCodec: Unknown extension format")
    }

    // Required method: encode custom extension nodes
    fn try_encode(
        &self,
        _node: Arc<dyn ExecutionPlan>,
        _buf: &mut Vec<u8>,
    ) -> Result<()> {
        // We handle everything in serialize_physical_plan, so this shouldn't be called
        not_impl_err!("AdapterPreservingCodec: try_encode should not be called directly")
    }

    // Interception point: override serialization to detect adapters
    fn serialize_physical_plan(
        &self,
        plan: Arc<dyn ExecutionPlan>,
    ) -> Result<PhysicalPlanNode> {
        // Check if this is a DataSourceExec with a custom adapter
...
/// Extract the tag from a MetadataAdapterFactory (via debug string for simplicity)
