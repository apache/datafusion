...
use datafusion_expr::{
    ...
    Expr, UserDefinedLogicalNode, UserDefinedLogicalNodeCore,
};
    ...
    fn plan_relation(
        ...
        match relation {
            TableFactor::Table { name, alias, .. }
                if name.to_string().to_lowercase() == "sample_data" =>
            {
                let values: Vec<Vec<Expr>> = (1..=10)
                    .map(|i| {
                        vec![
                            Expr::Literal(ScalarValue::Int32(Some(i)), None),
                            Expr::Literal(
                                ScalarValue::Utf8(Some(format!("row_{i}"))),
                                None,
                            ),
                        ]
                    })
                ...
                println!("[TestDataPlanner] Created 'sample_data' with 10 rows");
