...
//! ## Usage
//! ```bash
//! cargo run --example query_planning -- [all|analyzer_rule|array_functions|expr_api|optimizer_rule|parse_sql_expr|plan_to_sql|planner_api|pruning|thread_pools]
//! ```
//!
//! Each subcommand runs a corresponding example:
//! - `all` — run all examples included in this module
//! - `analyzer_rule` — use a custom AnalyzerRule to change a query's semantics (row level access control)
//! - `array_functions` — show logical and physical plans for queries with array_has, array_has_all, array_has_any
...
impl ExampleKind {
    ...
    async fn run(&self) -> Result<()> {
        match self {
            ...
            ExampleKind::ArrayFunctions => {
                array_functions::array_functions_physical_plans().await?
            }
