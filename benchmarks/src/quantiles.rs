...
//! Micro-benchmark for `MEDIAN` and `PERCENTILE_CONT` on primitive types.
...
impl Display for QuantileType {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            QuantileType::Int64 => write!(f, "int64"),
            QuantileType::Float64 => write!(f, "float64"),
        }
    }
}

impl FromStr for QuantileType {
    type Err = String;

    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        match s.to_ascii_lowercase().as_str() {
            "int" | "int64" => Ok(QuantileType::Int64),
            "float" | "float64" => Ok(QuantileType::Float64),
...
pub struct RunOpt {
    ...
    /// Percentile value for PERCENTILE_CONT
    #[structopt(long = "percentile", default_value = "0.9")]
    percentile: f64,

    /// Column type: int64 or float64
    #[structopt(long = "type", default_value = "int64")]
    data_type: QuantileType,
}
...
impl RunOpt {
    pub async fn run(self) -> Result<()> {
        ...
        self.bench_query(
            ...
            "percentile_cont",
            &format!(
                "SELECT percentile_cont({}) WITHIN GROUP (ORDER BY v) FROM t",
                self.percentile
            ),
            &ctx,
        )
