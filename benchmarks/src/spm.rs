...
pub struct RunOpt {
    ...
    #[structopt(
        ...
        short = "p",
        long = "path",
        default_value = "benchmarks/data/low_card_spm.csv"
    )]
    path: PathBuf,
...
impl RunOpt {
    ...
    async fn register_spm(&self, ctx: &SessionContext) -> Result<()> {
        let options = Default::default();
        let path = self.path.as_os_str().to_str().unwrap();
        ctx.register_csv("spm_table", path, options)
            .await
            .map_err(|e| {
                DataFusionError::Context(
                    format!("Registering 'spm' as {path}"),
                    Box::new(e),
                )
            })
    }
