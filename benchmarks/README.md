...
payload width. One keyed workload reads pre-sorted Parquet inputs with declared
ordering so it measures the join without including an input sort.

### Example Run

```bash
# No external data is required. The pre-sorted Parquet inputs are generated by
# the SQL benchmark setup outside the measured query.
