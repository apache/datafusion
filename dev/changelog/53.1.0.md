...
- feat: add `normalize_batch_schema` reusable helper to `datafusion-physical-plan` for zero-copy schema-name normalization of `RecordBatch` output, with operator audit confirming `UnionExec`, sort/merge, and join operators are safe without it; `RecursiveQueryExec` migrated to use the helper (follow-up to PR #21770)
