...
async fn run_query_substrait_round_trip(
    ...
    let round_tripped_plan = match df.logical_plan() {
        // For EXPLAIN plans, round trip just the inner plan
        LogicalPlan::Explain(explain) => {
            let mut explain = explain.clone();

            let plan = to_substrait_plan(&explain.plan, &state)?;
            explain.plan = Arc::new(from_substrait_plan(&state, &plan).await?);
