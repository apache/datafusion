    ...
    let field_vecs = dst.fields()
        ...
        .map(|(field, src_idx)| match src_idx {
            Some(batch_idx) => {
                ...
                rewrite_record_batch_field(field.clone(), src_field, arr, num_rows, false, false)
            },
...
/// Entry point for schema
/// Checks whether we can transform a source to a destination [SchemaRef]
/// The missing source fields behavior can be changed with the third parameter
///   - If it's true, then missing source fields are "filled" - not actually checked
///   - If it's false, then the function returns false if requested fields in the destination are
