...
fn default_builder(options: &ConfigOptions) -> Result<WriterPropertiesBuilder> {
    ...
    let mut builder = WriterProperties::builder()
        ...
        .set_column_index_truncate_length(parquet_context.column_index_truncate_length)
        ...
        for (option, value) in &statement_options.options {
            builder = match option.to_lowercase().as_str(){
                "max_row_group_size" => builder
                    ...
                    .map_err(|_| DataFusionError::InvalidOption(format!("Unable to parse {value} as u64 as required for {option}!")))?),
                "data_pagesize_limit" => builder
                    .set_data_page_size_limit(value.parse()
                    .map_err(|_| DataFusionError::InvalidOption(format!("Unable to parse {value} as usize as required for {option}!")))?),
                ...
                "bloom_filter_fpp" => builder
                    ...
                    .map_err(|_| DataFusionError::InvalidOption(format!("Unable to parse {value} as f64 as required for {option}!")))?),
                "bloom_filter_ndv" => builder
                    .set_bloom_filter_ndv(value.parse()
                    .map_err(|_| DataFusionError::InvalidOption(format!("Unable to parse {value} as u64 as required for {option}!")))?),
                _ => return Err(DataFusionError::InvalidOption(format!("Found unsupported option {option} with value {value} for Parquet format!")))
            }
        }
