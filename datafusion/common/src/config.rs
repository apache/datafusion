...
config_namespace! {
    ...
    /// into the file scans, allowing files without matching values to be skipped/pruned.
    ///
    /// Dynamic filters are also produced by join operators with equi-join predicates.
    /// For hash joins, dynamic filters are applied to the probe side (the stream that is scanned),
    /// which is the right input for Inner and Left joins and the left input for Right joins;
    /// semi and anti joins prune the left/probe input. Full joins are not supported.
    ///
    /// For example, `SELECT * FROM fact LEFT JOIN dim ON fact.id = dim.id WHERE dim.region = 'US'`
    /// will only read `fact` rows whose `id` values match `dim` rows where `region = 'US'`.
    ///
    /// Note that non-equi join predicates do not generate dynamic filters.
