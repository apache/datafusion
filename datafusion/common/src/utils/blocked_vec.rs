...
impl<T: Clone> BlockedVec<T> {
    ...
    /// Resolves the representation once, so a reading loop is free of the check.
    #[inline]
    pub fn storage(&self) -> StorageRef<'_, T> {
        match &self.storage {
            Storage::Flat(values) => StorageRef::Flat(values.as_slice()),
            Storage::Blocked(blocks) => StorageRef::Blocked(blocks.as_slice()),
        }
    }
    ...
    pub unsafe fn get_unchecked(&self, index: usize) -> &T {
        match &self.storage {
            Storage::Flat(values) => unsafe { values.get_unchecked(index) },
    ...
    pub fn push(&mut self, value: T) {
        if self.len == THRESHOLD_LEN && !self.is_blocked() {
            self.switch_to_blocked();
        }
        match &mut self.storage {
            Storage::Flat(values) => values.push(value),
            Storage::Blocked(blocks) => {
                let (block, _) = block_offset(self.len);
                if block == blocks.len() {
