...
impl RenderTreeNode {
    ...
    pub fn add_child_position(&mut self, x: usize, y: usize) {
...
impl RenderTree {
    ...
    fn create_tree<T: FormattedTreeNode>(node: &T, max_width: usize) -> Self {
        ...
        let mut result = Self::new(max_width, width, height);
    ...
    pub fn new(max_width: usize, width: usize, height: usize) -> Self {
        RenderTree {
...
pub fn render_tree<'a, T: FormattedTreeNode>(
    n: &'a T,
    max_width: usize,
) -> impl fmt::Display + use<'a, T> {
    RenderTree::create_tree(n, max_width)
}
