...
use super::primitive_filter::{BitmapFilter, BitmapFilterType, PrimitiveFrozenFilter};
...
struct FixedSizeBinaryFilter<T: BranchlessFilterType> {
    data_type: DataType,
    inner: StaticFilterRef,
    _marker: PhantomData<T>,
}

impl<T: BranchlessFilterType> FixedSizeBinaryFilter<T> {
...
where
    T: BitmapFilterType + BranchlessFilterType,
{
    ...
    BranchlessNative<T>: Copy + Eq + Hash + Send + Sync,
{
    ...
    let inner: StaticFilterRef = if non_null_count <= T::MAX_LIST_LEN {
