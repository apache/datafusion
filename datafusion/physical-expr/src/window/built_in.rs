...
impl BuiltInWindowExpr {
    ...
    /// to `builder`.
    ///
    /// If result of `self.expr` doesn't have an ordering, ordering equivalence
    /// properties is not updated. Otherwise ordering equivalence properties is updated
    /// by ordering of the `self.expr` evaluation result.
    pub fn add_equal_orderings<F: FnOnce() -> EquivalenceProperties>(
        ...
        if let Some(fn_res_ordering) = self.expr.get_result_ordering(schema) {
            if self.partition_by.is_empty() {
                // Ordering of the `self.expr` result is global
                builder.add_equal_conditions(vec![fn_res_ordering]);
            } else {
                // if window expr contains partition by, ordering of the `self.expr` result is not global.
                // When partition by expressions and existing ordering expressions are equal (set equality),
                // ordering of `self.expr` result can be prefixed with existing ordering.
