...
mod properties_tests {
    ...
    fn test_case_expr_ordering_from_raw_oeq_class() -> Result<()> {
        ...
        let when = binary(col("a", &schema)?, Operator::Eq, lit(1i32), &schema)?;
        let then = col("a", &schema)?;
        let case_expr = case(None, vec![(when, then)], None)?;

        let option = SortOptions::default();
        eq_properties.add_ordering(std::iter::once(PhysicalSortExpr::new(
            Arc::clone(&case_expr),
            option,
        )));

        let props = eq_properties.get_expr_properties(Arc::clone(&case_expr));
        assert_eq!(props.sort_properties, SortProperties::Ordered(option));
