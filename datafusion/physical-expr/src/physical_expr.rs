...
/// Create physical partitioning from logical partitioning, threading an
/// explicit [`SubqueryContext`] for scalar-subquery lowering.
pub fn create_physical_partitioning_with_subquery_context(
    partitioning: &LogicalPartitioning,
    input_dfschema: &DFSchema,
    execution_props: &ExecutionProps,
    subquery_ctx: &SubqueryContext,
    ...
    match partitioning {
        ...
        LogicalPartitioning::Range(range) => {
            let ordering = create_physical_sort_exprs_with_subquery_context(
                range.ordering(),
                input_dfschema,
                execution_props,
                subquery_ctx,
