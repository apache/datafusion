...
#[derive(Debug, Clone)]
pub struct EquivalencePropertiesGen<T> {
    classes: Vec<EquivalentClassGen<T>>,
    schema: SchemaRef,
}

impl<T: Eq + Hash + Clone> EquivalencePropertiesGen<T> {
    pub fn new(schema: SchemaRef) -> Self {
        EquivalencePropertiesGen {
            classes: vec![],
            schema,
        }
    }

    pub fn classes(&self) -> &[EquivalentClassGen<T>] {
        &self.classes
    }
    ...
    pub fn extend<I: IntoIterator<Item = EquivalentClassGen<T>>>(&mut self, iter: I) {
        for ec in iter {
    ...
    pub fn add_equal_conditions(&mut self, new_conditions: (&T, &T)) {
        ...
        match (idx1, idx2) {
            ...
            (None, None) => {
                // adding new pairs
                self.classes.push(EquivalentClassGen::<T>::new(
...
pub type EquivalenceProperties = EquivalencePropertiesGen<Column>;
pub type OrderingEquivalenceProperties = EquivalencePropertiesGen<EqualColumns>;

/// Equivalent Class is a set of Columns or OrderedColumns that are known to have the same value in all tuples in a relation
/// Equivalent Class is generated by equality predicates, typically equijoin conditions and equality conditions in filters.
#[derive(Debug, Clone)]
pub struct EquivalentClassGen<T> {
...
impl<T: Eq + Hash + Clone> EquivalentClassGen<T> {
    pub fn new(head: T, others: Vec<T>) -> EquivalentClassGen<T> {
        EquivalentClassGen {
            head,
...
pub type OrderingEquivalentClass = EquivalentClassGen<EqualColumns>;

impl From<Column> for EqualColumns {
    ...
    for (column, columns) in columns_map {
        ...
        for class in ec_classes.iter_mut() {
            if let Some(EqualColumns { mode, .. }) = get_matching_column(class, column) {
                ...
                for col in columns {
                    let col = col.clone();
                    let ordered_column = EqualColumns { col, mode };
