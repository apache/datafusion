...
impl EquivalenceProperties {
    ...
    fn projected_orderings(&self, mapping: &ProjectionMapping) -> Vec<LexOrdering> {
        ...
        for ordering in self.normalized_oeq_class().iter() {
            ...
            for sort_expr in ordering {
                let target_expr = self.eq_group.project_expr(mapping, &sort_expr.expr);
                let target_sort_expr = target_expr.map(|expr| PhysicalSortExpr {
                    expr,
                    options: sort_expr.options,
                });
                if PRINT_ON {
                    println!("sort_expr: {:?}", sort_expr);
                    println!("target_sort_expr: {:?}", target_sort_expr);
                }
                let is_projected = target_sort_expr.is_some();
                if is_projected | any_projection_refers(mapping, &sort_expr.expr) {
                    let dependency_idx = if dependencies.is_empty() {
                        None
                    } else {
                        Some(all_dependencies.len() + dependencies.len() - 1)
                    };
                    dependencies.push(Dependency {
                        source: sort_expr.clone(),
                        target: target_sort_expr,
                        dependency: dependency_idx,
                    });
                }
                if !is_projected {
                    // TOD0: Iteration should stop when an expression cannot projected
                    //     leaf dependency can be a referred expression
                    break;
                }
            }
        ...
        let mut completed_orderings = vec![];
        for (source, target) in mapping.iter() {
            let relevant_indices = relevant_dependencies(&all_dependencies, source);
            let relevant_prefixes = relevant_indices
                .iter()
                .map(|idx| {
                    construct_orderings1(
                        &all_dependencies,
                        all_dependencies[*idx].dependency,
                    )
                })
                .collect::<Vec<_>>();
            assert_eq!(relevant_indices.len(), relevant_prefixes.len());
            let relevant_sources = relevant_indices
                .iter()
                .map(|idx| &all_dependencies[*idx].source)
                .collect::<Vec<_>>();
            let ordering = get_ordering_expr(source, &relevant_sources);
            if PRINT_ON {
            ...
            if let SortProperties::Ordered(options) = ordering {
                ...
                let relevant_source_indices =
                    get_relevant_sources_indices(source, &relevant_sources);
                if PRINT_ON {
                    println!("relevant_source_indices: {:?}", relevant_source_indices);
                }
                for indices in relevant_source_indices {
                    // TODO: Generate these as cross product not 0th index.
                    let prefix_to_use = indices
                        .iter()
                        .map(|idx| relevant_prefixes[*idx][0].clone())
                        .collect::<Vec<_>>();
                    if PRINT_ON {
                        println!("indices: {:?}", indices);
                        println!("prefix_to_use: {:?}", prefix_to_use);
                    }
                    for elems in prefix_to_use.iter().permutations(prefix_to_use.len()) {
                        let mut ordering =
                            elems.into_iter().flatten().cloned().collect::<Vec<_>>();
                        ordering.push(sort_expr.clone());
                        if PRINT_ON {
                            println!("new ordering: {:?}", ordering);
                        }
                        completed_orderings.push(ordering);
                    }
                }
            }
            if PRINT_ON {
                println!("relevant indices: {:?}", relevant_indices);
                println!("completed_orderings: {:?}", completed_orderings);
            }
        }
        // TODO: iterate only leaf dependencies.
        for (idx, _dependency) in all_dependencies.iter().enumerate() {
            let mut prefixes = construct_orderings1(&all_dependencies, Some(idx));
            if let Some(target_sort_expr) = &all_dependencies[idx].target {
                prefixes
                    .iter_mut()
                    .for_each(|ordering| ordering.push(target_sort_expr.clone()));
            }
            completed_orderings.extend(prefixes);
        }
        completed_orderings
            .into_iter()
...
#[derive(Debug, Clone)]
struct Dependency {
    source: PhysicalSortExpr,
    target: Option<PhysicalSortExpr>,
...
fn relevant_dependencies(
    ...
    dependencies
        .iter()
        .enumerate()
        .flat_map(|(idx, dependency)| {
            if expr_refers(source, &dependency.source.expr) {
                Some(idx)
            } else {
                None
            }
...
fn get_ordering_expr(
    expr: &Arc<dyn PhysicalExpr>,
    relevant_column_orderings: &[&PhysicalSortExpr],
) -> SortProperties {
    for column_order in relevant_column_orderings {
        if expr.eq(&column_order.expr) {
            return SortProperties::Ordered(column_order.options);
        }
    }
    let child_states = expr
        .children()
        .iter()
        .map(|child| get_ordering_expr(child, relevant_column_orderings))
        .collect::<Vec<_>>();
    expr.get_ordering(&child_states)
}

fn get_relevant_sources_indices(
    expr: &Arc<dyn PhysicalExpr>,
    relevant_column_orderings: &[&PhysicalSortExpr],
) -> Vec<Vec<usize>> {
    let mut result = vec![];
    for (idx, column_order) in relevant_column_orderings.iter().enumerate() {
        if expr.eq(&column_order.expr) {
            result.push(vec![idx]);
        }
    }
    ...
    let mut child_source_indices = expr
        .children()
        .iter()
        .map(|child| get_relevant_sources_indices(child, relevant_column_orderings))
        .collect::<Vec<_>>();
    child_source_indices.retain(|source_indices| !source_indices.is_empty());

    if child_source_indices.is_empty() {
        return result;
    }
    let bounds = child_source_indices
        .iter()
        .map(|child_source| child_source.len())
        .collect::<Vec<_>>();
    let mut gen = Generator::new(bounds);
    while let Some(indices) = gen.next() {
        let mut vals = vec![];
        izip!(indices.iter(), child_source_indices.iter()).for_each(
            |(idx, child_source)| {
                vals.extend(child_source[*idx].clone());
            },
        );
        result.push(vals);
    }
...
impl Generator {
    fn new(lengths: Vec<usize>) -> Self {
        let idx = lengths.len() - 1;
        let seed = vec![0; lengths.len()];
        Self {
            lengths,
            idx,
            seed,
    ...
    fn next(&mut self) -> Option<Vec<usize>> {
        ...
        let mut idx_to_increment = self.idx;
        loop {
            if self.increment_at_index(idx_to_increment) {
                return Some(self.seed.clone());
            } else if idx_to_increment > 0 {
                // Reset
                self.seed[idx_to_increment] = 0;
                idx_to_increment -= 1;
            } else {
                return None;
            }
    ...
    fn increment_at_index(&mut self, idx: usize) -> bool {
        if self.seed[idx] + 1 == self.lengths[idx] {
            false
        } else {
            self.seed[idx] += 1;
            true
...
fn construct_orderings1(
    dependencies: &[Dependency],
    relevant_idx: Option<usize>,
) -> Vec<Vec<PhysicalSortExpr>> {
    if let Some(relevant_idx) = relevant_idx {
        let mut result =
            construct_orderings1(dependencies, dependencies[relevant_idx].dependency);
        if let Some(target_sort_expr) = &dependencies[relevant_idx].target {
            result
                .iter_mut()
                .for_each(|ordering| ordering.push(target_sort_expr.clone()));
            result
...
// TODO: Consider equivalences also.
fn any_projection_refers(
    mapping: &ProjectionMapping,
    expr: &Arc<dyn PhysicalExpr>,
) -> bool {
    for (source, _target) in mapping.iter() {
        if expr_refers(source, expr) {
            return true;
        }
    }
    false
...
mod tests {
    ...
    fn ordering_satisfy_after_projection_random() -> Result<()> {
        ...
        for seed in 0..N_RANDOM_SCHEMA {
            println!("{}/{}", seed + 1, N_RANDOM_SCHEMA);
            // Create a random schema with random properties
            let (test_schema, eq_properties) = create_random_schema(seed as u64)?;
            // Generate a data that satisfies properties given
            let table_data_with_properties =
    ...
    fn test_generator() -> Result<()> {
        let mut gen = Generator::new(vec![2, 2, 2]);
        while let Some(elem) = gen.next() {
            println!("elem:{:?}", elem);
        }
        Ok(())
    }
