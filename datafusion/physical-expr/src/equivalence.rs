...
pub type OrderingEquivalenceProperties = EquivalencePropertiesGen<OrderedColumns>;
...
pub fn project_ordering_equivalence_properties(
    ...
    for (column, columns) in columns_map {
        for class in ec_classes.iter_mut() {
            if let Some(OrderedColumns { options, .. }) =
                get_matching_column(class, column)
            {
                // Matching entry found
                for col in columns {
                    let col = col.clone();
                    let ordered_column = OrderedColumns { col, options };
