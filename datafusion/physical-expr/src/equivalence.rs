...
/// duplicate entries that have same physical expression inside. For example,
/// `vec![a Some(Asc), a Some(Desc)]` collapses to `vec![a Some(Asc)]`.
pub fn collapse_lex_ordering(
    input: LexOrdering,
    leading_ordering_exprs: &[Arc<dyn PhysicalExpr>],
) -> LexOrdering {
    let mut output = Vec::<PhysicalSortExpr>::new();
    for (idx, item) in input.into_iter().enumerate() {
        if !(output.iter().any(|req| req.expr.eq(&item.expr))
            || (idx > 0 && physical_exprs_contains(leading_ordering_exprs, &item.expr)))
        {
            output.push(item);
        }
    }
...
impl OrderingEquivalenceClass {
    ...
    fn remove_redundant_entries(&mut self) {
        ...
        self.orderings = self
            ...
            .iter()
            .map(|ordering| {
                collapse_lex_ordering(ordering.to_vec(), &leading_ordering_exprs)
            })
            .collect::<Vec<_>>();
        let mut idx = 0;
...
impl EquivalenceProperties {
    ...
    fn get_leading_orderings(
        &self,
        ignore_exprs: &[Arc<dyn PhysicalExpr>],
    ) -> Vec<PhysicalSortExpr> {
        let mut leading_orderings = vec![];
        let normalized_ignore_exprs =
            self.eq_group.normalize_exprs(ignore_exprs.to_vec());
        for ordering in self.normalized_oeq_class().iter() {
            for sort_expr in ordering {
                if !expr_consists_of_ignored_exprs(
                    &normalized_ignore_exprs,
                    &sort_expr.expr,
                ) {
    ...
    fn project_ordering_helper(
        ...
        let result = ordering
            .iter()
            .map_while(|sort_expr| {
                self.project_expr(&sort_expr.expr, mapping)
    ...
    // TODO: Add a stop signal as return value.
    fn calc_ordered_exprs(
        ...
        for (source, target) in mapping.iter() {
            ...
            let sort_options = match expr_ordering.state {
                SortProperties::Ordered(options) => Some(options),
                // Assign an arbitrary ordering direction to singleton columns.
                SortProperties::Singleton => Some(SortOptions {
                    descending: false,
                    nulls_first: false,
                }),
            ...
            if let Some(options) = sort_options {
                ...
                {
                    if let Some(projected_leaf_orderings) =
                        self.project_ordering_helper(mapping, &leaf_orderings)
                    {
                        // All of the leaves are projected.
                        if projected_leaf_orderings.len() == leaf_orderings.len() {
                            // TODO: If expression is known to be strictly monothonic. Then we do not need to append leaf ordering expressions
                            for suffix in projected_leaf_orderings
                                .iter()
                                .permutations(projected_leaf_orderings.len())
                            {
        ...
        for leading_ordering in leading_orderings {
    ...
    fn project_ordering(
        ...
        for sort_expr in ordering.iter() {
            ...
            if new_orderings.is_empty() {
                ...
                let relevant_indices = orderings
                    ...
                    .collect::<Vec<_>>();
                let n_rep = new_orderings.len();
                let orderings_len = orderings.len();
                for relevant_idx in &relevant_indices {
                    for _rep_idx in 0..n_rep {
                        orderings.push(orderings[*relevant_idx].clone());
                    }
                }

                let new_indices = (orderings_len
                    ..orderings_len + (n_rep) * relevant_indices.len())
                    .collect::<Vec<_>>();
        ...
        if PRINT_ON {
            ...
            for ordering in &orderings {
                println!("updated ordering3: {ordering:?}");
            }
        }
    ...
    pub fn find_longest_permutation(
        ...
        let projected = self
            .clone()
            .project(&projection_mapping, projection_mapping.output_schema());
    ...
    fn is_expr_constant(&self, expr: &Arc<dyn PhysicalExpr>) -> bool {
        ...
        physical_exprs_contains(&normalized_constants, &normalized_expr)
    }
...
fn expr_consists_of_ignored_exprs(
    ignored_exprs: &[Arc<dyn PhysicalExpr>],
    expr: &Arc<dyn PhysicalExpr>,
) -> bool {
    if physical_exprs_contains(ignored_exprs, expr) {
        return true;
    }
    let children = expr.children();
    if children.is_empty() {
        ...
        children
            .iter()
            .all(|child| expr_consists_of_ignored_exprs(ignored_exprs, child))
    }
}

fn ordering_consists_of_ignored_exprs(
    ignored_exprs: &[Arc<dyn PhysicalExpr>],
...
mod tests {
    ...
    fn test_ordering_satisfy_with_equivalence_complex_random() -> Result<()> {
        ...
        for seed in 0..N_RANDOM_SCHEMA {
            ...
            let table_data_with_properties =
                generate_table_for_eq_properties(&eq_properties, N_ELEMENTS, N_DISTINCT)?;
            // print_batches(&[table_data_with_properties.clone()])?;
            ...
            for n_req in 0..=exprs.len() {
                for exprs in exprs.iter().combinations(n_req) {
                    ...
                    // assert!(
                    //     !(eq_properties.ordering_satisfy(&requirement) == true
                    //         && expected == false),
                    //     "{}",
                    //     err_msg
                    // );
    ...
    fn test_ordering_satisfy_err_cases() -> Result<()> {
        ...
        for (orderings, eq_group, constants, reqs, expected) in test_cases {
            ...
            let eq_group = eq_group
                .into_iter()
                .map(|eq_class| eq_class.into_iter().cloned().collect::<Vec<_>>())
                .collect::<Vec<_>>();
    ...
    fn is_table_same_after_sort(
        ...
        let sort_columns: Vec<_> = required_ordering
            .iter()
            .map(|order_expr| {
                let expr_result = order_expr.expr.evaluate(&new_batch).unwrap();
    ...
    #[test]
    fn test_expr_consists_of_ignored_exprs() -> Result<()> {
        let schema = Arc::new(Schema::new(vec![
            Field::new("a", DataType::Int32, true),
            Field::new("b", DataType::Int32, true),
            Field::new("c", DataType::Int32, true),
        ...
        let ignored_exprs = vec![col_a.clone(), col_b.clone()];
        let expr = b_plus_d.clone();
        assert!(!expr_consists_of_ignored_exprs(&ignored_exprs, &expr));

        let ignored_exprs = vec![col_a.clone(), col_b.clone(), col_d.clone()];
        let expr = b_plus_d.clone();
        assert!(expr_consists_of_ignored_exprs(&ignored_exprs, &expr));
        Ok(())
    }
