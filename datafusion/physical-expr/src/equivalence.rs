...
impl ProjectionMapping {
    ...
    /// Return target expressions for each key.
    pub fn target_exprs(
        &self,
    ...
    /// Return target expression for given expr
    /// None means given key is not found.
    pub fn target_expr(
        &self,
...
impl EquivalenceProperties {
    ...
    pub fn ordering_satisfy_requirement(&self, reqs: LexRequirementRef) -> bool {
        ...
        let exprs_used = reqs
            ...
            .collect::<Vec<_>>();
        let complex_proj_exprs = exprs_used
            .iter()
            .flat_map(|expr| {
        ...
        if let Some(target_exprs) = projection_mapping.target_exprs(&exprs_used) {
            let projected_reqs = izip!(target_exprs.into_iter(), reqs.iter())
                .map(|(target_expr, sort_req)| PhysicalSortRequirement {
                    expr: target_expr,
...
mod tests {
    ...
    fn project_empty_output_ordering() -> Result<()> {
        ...
        eq_properties.add_new_orderings([ordering]);
        // b as b_new, a as a_new
        let proj_exprs = vec![
            (col("b", &schema)?, "b_new".to_string()),
            (col("a", &schema)?, "a_new".to_string()),
        ];
        ...
        // c as c_new, b as b_new
        let proj_exprs = vec![
            (col("c", &schema)?, "c_new".to_string()),
            (col("b", &schema)?, "b_new".to_string()),
        ];
        let projection_mapping = ProjectionMapping::try_new(&proj_exprs, &schema)?;
        let projected = eq_properties
            .project(&projection_mapping, projection_mapping.output_schema());
