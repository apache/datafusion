...
impl SchemaProperties {
    ...
    /// Checks whether any permutation of `exprs` shares a prefix with an
    /// existing ordering. If so, returns the indices describing the permutation.
    ...
    /// ordering. If so, returns the indices describing the permutation.
    /// Otherwise, returns `None`.
    pub fn set_exactly_satisfy(
        &self,
        exprs: &[Arc<dyn PhysicalExpr>],
    ) -> Option<Vec<usize>> {
        self.set_satisfy(exprs)
            .filter(|indices| indices.len() == exprs.len())
    }
    ...
    /// within. If the premise is false, returns `None`.
