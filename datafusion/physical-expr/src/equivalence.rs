...
/// `OrderingEquivalenceProperties` keeps track of columns that can describe global ordering of the schema,
/// these columns are not necessarily same. Consider table,
/// | a | b |
/// |---|---|
/// | 1 | 9 |
/// | 2 | 8 |
/// | 3 | 7 |
/// | 5 | 5 |
/// both `a ASC` and `b DESC` can describe ordering of the current table.
/// with OrderingEquivalenceProperties we can kee[ track of these equalities
/// and treat `a ASC`, and `b DESC` as same requirement.
...
/// to have the same value in all tuples in a relation. `EquivalentClass<Column>` is generated
/// by equality predicates, typically equijoin conditions and equality conditions
/// in filters. `EquivalentClass<OrderedColumn>` is generated by `ROW_NUMBER` window aggregate.
#[derive(Debug, Clone)]
...
impl OrderingEquivalentClass {
    ...
    fn get_matching_column(&self, column: &Column) -> Option<OrderedColumn> {
        ...
        if self.head.col.eq(column) {
            matching_entry = Some(self.head.clone());
        }
