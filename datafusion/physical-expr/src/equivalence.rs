...
pub enum SetOrderMode<T> {
    /// None of the columns among the partition columns is ordered.
    None,
    /// Some columns of the partition columns are ordered but not all
    PartiallySorted(Vec<T>),
    /// All Partition columns are ordered (Also empty case)
    FullySorted(Vec<T>),
}

impl<T> SetOrderMode<T> {
    // Return a reference to the ordered entries inside modes.
    pub fn entries_ref(&self) -> &[T] {
        match self {
            SetOrderMode::FullySorted(entries)
            | SetOrderMode::PartiallySorted(entries) => entries,
            SetOrderMode::None => &[],
        }
    }

    // Return the ordered entries, inside modes.
    pub fn entries(self) -> Vec<T> {
        match self {
            SetOrderMode::FullySorted(entries)
            | SetOrderMode::PartiallySorted(entries) => entries,
            SetOrderMode::None => vec![],
        }
    }
}
...
impl SchemaProperties {
    ...
    /// Returns the indices that describe an ordered section inside `exprs`.
    /// It can return [`SetOrderMode::FullySorted`], [`SetOrderMode::PartiallySorted`], or [`SetOrderMode::None`] results.
    /// See [`SetOrderMode`] for explanation of different modes.
    pub fn set_ordered_indices(
        &self,
        exprs: &[Arc<dyn PhysicalExpr>],
    ) -> SetOrderMode<usize> {
        let (_ordering, order_indices) = self.set_ordered_helper(exprs);

        if order_indices.len() == exprs.len() {
            // All of the expressions are ordered
            SetOrderMode::FullySorted(order_indices)
        } else if !order_indices.is_empty() {
            // A prefix defines ordering
            SetOrderMode::PartiallySorted(order_indices)
        } else {
            // None of the expressions is among ordered section.
            SetOrderMode::None
        }
    }

    /// Returns the `PhysicalSortExpr`s that describe an ordered section (each expression is contained inside `exprs`).
    /// It can return [`SetOrderMode::FullySorted`], [`SetOrderMode::PartiallySorted`], or [`SetOrderMode::None`] results.
    /// See [`SetOrderMode`] for explanation of different modes.
    pub fn set_ordered_section(
        &self,
        exprs: &[Arc<dyn PhysicalExpr>],
    ) -> SetOrderMode<PhysicalSortExpr> {
        let (ordered_section, _ordering_indices) = self.set_ordered_helper(exprs);
        if ordered_section.len() == exprs.len() {
            // All of the expressions are ordered
            SetOrderMode::FullySorted(ordered_section)
        } else if !ordered_section.is_empty() {
            // A prefix defines ordering
            SetOrderMode::PartiallySorted(ordered_section)
        } else {
            // None of the expressions is among ordered section.
            SetOrderMode::None
        }
    ...
    /// Returns the longest ordered section (and its expression indices) that can be built out of given `exprs`
    /// that is satisfied by existing orderings.
    /// As an example if existing orderings are [a ASC, b ASC], [c DESC]
    /// and `exprs` are \[c,b,a,d\]. This function returns ([a ASC, b ASC, c DESC], \[2,1,0\]). This means that
    /// [a ASC, b ASC, c DESC] is satisfied by exiting ordering, and \[a,b,c\] resides at indices: 2,1,0 respectively
    /// inside the argument `exprs`.
    fn set_ordered_helper(
        &self,
        exprs: &[Arc<dyn PhysicalExpr>],
...
mod tests {
    ...
    fn test_set_ordered_helper() -> Result<()> {
        ...
        for (exprs, expected) in test_cases {
            ...
            let (actual, _indices) = eq_properties.set_ordered_helper(&exprs);
