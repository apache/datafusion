...
//! Backend-independent file paths. Adapted from Apache Arrow ObjectStore.
...
/// [`LocalFileSystem`]: https://docs.rs/object_store/latest/object_store/local/struct.LocalFileSystem.html
...
impl Path {
    ...
    pub fn join<'a>(self, child: impl Into<PathPart<'a>>) -> Self {
        ...
        let raw = if self.raw.is_empty() {
            ...
            write!(raw, "{DELIMITER}{child_cow_str}")
