...
// New: import exclusions submodule (sibling file)
...
impl<S: ContextProvider> SqlToRel<'_, S> {
    ...
    pub(crate) fn plan_match_recognize(
        ...
        let (defines, compile_symbols, classifier_bitsets, exclusion_predicate, ctx_opt) = {
            ...
            if let Some(analysis) = &analysis_opt {
                ...
                if let Some(ctx) = &ctx_opt {
                    let new_defines: Result<Vec<(Expr, String)>> = defines
                        .into_iter()
                        .map(|(expr, name)| {
                            let rewritten = ctx.rewrite_expr(expr)?;
                            Ok((rewritten, name))
                        })
                        .collect();
                    defines = new_defines?;
        ...
        let measures_exprs = if let Some(ctx) = &ctx_opt {
            let rewritten: Result<Vec<(Expr, String)>> = measures_exprs_raw
                .into_iter()
                .map(|(expr, name)| Ok((ctx.rewrite_expr(expr)?, name)))
                .collect();
            rewritten?
        } else {
            measures_exprs_raw
