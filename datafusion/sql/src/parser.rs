...
impl<'a> DFParser<'a> {
    ...
    fn expected<T>(
        ...
        sql_err!(parser_err!(format!("Expected: {expected}, found: {found}{}",found.span.start))?).map_err(
            |e| {
                let span = Span::try_from_sqlparser_span(sql_parser_span);
                let mut diagnostic = Diagnostic::new_error(e.to_string(), span);
                diagnostic.add_note(format!("Expected: {expected}, found: {found}{}", found.span.start), span);
