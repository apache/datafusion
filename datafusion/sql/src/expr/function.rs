...
use datafusion_common::{
    internal_datafusion_err, internal_err, not_impl_err, plan_datafusion_err, plan_err, DFSchema, Dependency, Diagnostic, Result, ScalarValue, Span, TableReference
};
...
use datafusion_expr::{
    expr, Expr, ExprFunctionExt, ExprSchemable, Literal, WindowFrame, WindowFunctionDefinition
};
use sqlparser::ast::{
...
impl<S: ContextProvider> SqlToRel<'_, S> {
    pub(super) fn sql_function_to_expr(
        ...
        let over = if let Some(mr_context) = planner_context.match_recognize_context() {
            let function_name = name.to_string().to_ascii_lowercase();
