    ...
    pub(super) fn try_process_unnest(
        ...
        for i in 0.. {
            ...
            if unnest_columns.is_empty() {
                ...
                // The default SQL `UNNEST` matches DuckDB/PostgreSQL: drop
                // both NULL and empty input lists. Spark `explode_outer`
                // (modelled as `Unnest { outer: true }`) overrides that
                // and selects `NullHandling::PreserveAndExpandEmpty`. Mixing
                // the two in a single SELECT is a planning error because
                // `UnnestOptions` is per-`UnnestExec`, not per-column.
                let null_handling =
                    collect_unnest_null_handling(&intermediate_select_exprs)?;
...
///   per output column.
fn collect_unnest_null_handling(select_exprs: &[Expr]) -> Result<NullHandling> {
