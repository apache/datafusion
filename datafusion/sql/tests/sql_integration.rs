...
fn select_simple_aggregate_with_groupby_can_use_positions() {
    quick_test(
        ...
        "Projection: person.state, person.age AS b, count_star()\
             \n  Aggregate: groupBy=[[person.state, person.age]], aggr=[[count_star()]]\
             \n    TableScan: person",
    );
    quick_test(
        "SELECT state, age AS b, count(1) FROM person GROUP BY 2, 1",
        "Projection: person.state, person.age AS b, count_star()\
             \n  Aggregate: groupBy=[[person.age, person.state]], aggr=[[count_star()]]\
             \n    TableScan: person",
    );
}
...
fn select_count_one() {
    ...
    let expected = "Projection: count_star()\
