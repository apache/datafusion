...
fn create_match_recognize_test_state() -> MockSessionState {
    ...
    for f in
        datafusion_functions_match_recognize::all_default_match_recognize_scalar_functions(
        )
    {
        state = state.with_scalar_function(f);
    }
    ...
    for f in datafusion_functions_match_recognize::all_default_match_recognize_aggregate_functions() {
        state = state.with_aggregate_function(f);
    }
    ...
    for f in
        datafusion_functions_match_recognize::all_default_match_recognize_window_functions(
        )
    {
        state = state.with_window_function(f);
    }
...
fn roundtrip_match_recognize_basic() -> Result<()> {
    ...
    insta::assert_snapshot!(actual_sql, @"SELECT j1.j1_string, last_a FROM (SELECT j1.j1_string, last_value(j1.j1_id ORDER BY __mr_match_sequence_number ASC NULLS LAST) FILTER (WHERE __mr_classifier_a) AS last_a FROM j1 MATCH_RECOGNIZE(PARTITION BY j1.j1_string ORDER BY j1.j1_id ASC NULLS LAST MEASURES classifier() AS __mr_classifier, match_number() AS __mr_match_number, match_sequence_number() AS __mr_match_sequence_number, is_excluded_row() AS __mr_is_excluded_row, classifier() = 'A' AS __mr_classifier_a ALL ROWS PER MATCH AFTER MATCH SKIP PAST LAST ROW PATTERN (A+) DEFINE A AS (j1_id > 1)) GROUP BY j1.j1_string, __mr_match_number)");
...
fn roundtrip_match_recognize_rows_per_match_modes() -> Result<()> {
    ...
    let roundtrip_statement = plan_to_sql(&plan)?;
    let actual_sql = roundtrip_statement.to_string();
    insta::assert_snapshot!(actual_sql, @"SELECT j1.j1_id, j1.j1_string FROM (SELECT j1.j1_id, j1.j1_string FROM j1 MATCH_RECOGNIZE(PARTITION BY j1.j1_string ORDER BY j1.j1_id ASC NULLS LAST MEASURES classifier() AS __mr_classifier, match_number() AS __mr_match_number, match_sequence_number() AS __mr_match_sequence_number, is_excluded_row() AS __mr_is_excluded_row, classifier() = 'A' AS __mr_classifier_a ALL ROWS PER MATCH WITH UNMATCHED ROWS AFTER MATCH SKIP PAST LAST ROW PATTERN (A?) DEFINE A AS (j1_id > 1)))");
...
fn roundtrip_match_recognize_after_match_skip() -> Result<()> {
    ...
    let roundtrip_statement = plan_to_sql(&plan)?;
    let actual_sql = roundtrip_statement.to_string();
    insta::assert_snapshot!(actual_sql, @"SELECT j1.j1_id, j1.j1_string FROM (SELECT j1.j1_id, j1.j1_string FROM j1 MATCH_RECOGNIZE(PARTITION BY j1.j1_string ORDER BY j1.j1_id ASC NULLS LAST MEASURES classifier() AS __mr_classifier, match_number() AS __mr_match_number, match_sequence_number() AS __mr_match_sequence_number, is_excluded_row() AS __mr_is_excluded_row, classifier() = 'A' AS __mr_classifier_a, classifier() = 'B' AS __mr_classifier_b ALL ROWS PER MATCH AFTER MATCH SKIP TO FIRST A PATTERN (A+ B*) DEFINE A AS (j1_id > 1), B AS (j1_id < 5)))");
