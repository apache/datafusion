...
mod tests {
    ...
    fn batch(sort_keys: Vec<i32>, group_keys: Vec<i32>) -> Result<RecordBatch> {
        let values = vec![1; sort_keys.len()];
        Ok(RecordBatch::try_new(
            schema(),
            vec![
                Arc::new(Int32Array::from(sort_keys)),
                Arc::new(Int32Array::from(group_keys)),
                Arc::new(Int64Array::from(values)),
            ],
        )?)
    }

    fn aggregate(batches: Vec<RecordBatch>, fully_sorted: bool) -> Result<AggregateExec> {
        let schema = schema();
        let mut sort_exprs =
            vec![PhysicalSortExpr::new_default(col("sort_key", &schema)?)];
        if fully_sorted {
            sort_exprs.push(PhysicalSortExpr::new_default(col("group_key", &schema)?));
        }
        ...
        AggregateExec::try_new(
            ...
            PhysicalGroupBy::new_single(vec![
                (col("sort_key", &schema)?, "sort_key".to_string()),
    ...
    #[tokio::test]
    async fn materializes_completed_prefix_and_eof_once() -> Result<()> {
        // The first input completes eight groups and leaves eight active groups.
        // The second input updates only those active groups before EOF.
        let aggregate = aggregate(
            vec![
                batch(
                    [vec![1; 8], vec![2; 8]].concat(),
                    (0..8).cycle().take(16).collect(),
                )?,
                batch(vec![2; 8], (0..8).collect())?,
            ],
            false,
        )?;
        let context = Arc::new(
        ...
        let partial = OrderedPartialAggregateStream::new(&aggregate, &context, 0)?;
        let reduction = partial.context.reduction_factor.clone();
        let mut stream = partial.into_stream();

        for range in 1..=2 {
            let mut first_buffer = None;
            let mut first_reservation = 0;
            for slice in 0..4 {
                ...
                // Every slice of this completed range must share the same
                // allocation, rather than re-materializing the hash table.
                let buffer = output
                    .column(1)
                    .as_primitive::<Int32Type>()
                    .values()
                    .inner()
                    .data_ptr();
                assert_eq!(*first_buffer.get_or_insert(buffer), buffer);
                let reserved = context.memory_pool().reserved();
                let held = output.get_array_memory_size();
                if slice == 0 {
                    first_reservation = reserved;
                }
                if slice < 3 {
                    assert_eq!(reserved, first_reservation);
                    assert!(reserved >= held);
                } else {
                    assert_eq!(reserved, first_reservation - held);
                    if range == 2 {
                        assert_eq!(
                            reserved, 0,
                            "release all memory before the last EOF slice"
                        );
                    }
                }
            }
        ...
        assert_eq!(reduction.part(), 16);
        assert_eq!(reduction.total(), 24);
        Ok(())
    }

    #[tokio::test]
    async fn memory_pressure_slices_or_hands_off_whole_batch() -> Result<()> {
        let num_groups = 4096;
        let input = batch(vec![1; num_groups], (0..num_groups as i32).collect())?;
        let aggregate = aggregate(vec![input.clone(), input], false)?;

        for (memory_limit, output_size) in [(128 * 1024, 1024), (32 * 1024, num_groups)] {
            ...
            let context = Arc::new(
                TaskContext::default()
                    ...
                    .with_session_config(SessionConfig::new().with_batch_size(1024)),
            );
            let mut stream = OrderedPartialAggregateStream::new(&aggregate, &context, 0)?
                .into_stream();
            let mut rows = 0;
            while let Some(output) = stream.next().await.transpose()? {
                assert_eq!(output.num_rows(), output_size);
                // Each input must emit before the next input is aggregated,
                // otherwise the counts would be 2 rather than 1.
                assert!(
                    output
                        .column(2)
                        .as_primitive::<Int64Type>()
                ...
                if output_size == num_groups {
                    assert!(
                        context.memory_pool().reserved() < output.get_array_memory_size()
                    );
                }
                rows += output.num_rows();
            }
            assert_eq!(rows, 2 * num_groups);
    ...
    #[tokio::test]
    async fn fully_ordered_memory_pressure_still_errors() -> Result<()> {
        let aggregate = aggregate(vec![batch(vec![1; 8], (0..8).collect())?], true)?;
        let runtime = RuntimeEnvBuilder::default()
            .with_memory_limit(1, 1.0)
            .build_arc()?;
        let context = Arc::new(TaskContext::default().with_runtime(runtime));
        let mut stream =
            OrderedPartialAggregateStream::new(&aggregate, &context, 0)?.into_stream();
        assert!(matches!(
