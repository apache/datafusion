...
/// Streams used by [`BlockedAggregateExec`].
///
...
#[derive(Debug, Clone)]
pub struct BlockedAggregateExec {
...
impl BlockedAggregateExec {
    ...
    /// with this [`BlockedAggregateExec`].
    ///
    /// Only used to restore the filter when decoding a serialized plan: every
    /// other code path creates the filter in [`BlockedAggregateExec::try_new`].
...
impl DisplayAs for BlockedAggregateExec {
    fn fmt_as(
        &self,
        t: DisplayFormatType,
...
impl ExecutionPlan for BlockedAggregateExec {
    ...
    /// The output ordering of [`BlockedAggregateExec`] is determined by its `group_by`
...
#[cfg(feature = "proto")]
impl BlockedAggregateExec {
    /// Reconstruct an [`BlockedAggregateExec`] from its protobuf representation.
    ///
...
/// Creates the output schema for an [`BlockedAggregateExec`] containing the group by columns followed
...
mod tests {
    ...
    fn assert_accumulator_phase_times(aggregate: &BlockedAggregateExec, phases: &[&str]) {
    ...
    async fn check_grouping_sets(
        ...
        let merged_aggregate = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Final,
            final_grouping_set,
            aggregates,
            vec![None],
    ...
    async fn partial_grouped_aggregate_uses_raw_partial_stream() -> Result<()> {
        ...
        let merge = Arc::new(CoalescePartitionsExec::new(partial_aggregate));
        let final_aggregate = BlockedAggregateExec::try_new(
            AggregateMode::Final,
            group_by.as_final(),
            aggregates,
            vec![None],
    ...
    async fn partial_grouped_aggregate_materializes_before_slicing() -> Result<()> {
        ...
        let aggregate = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Partial,
            group_by,
            aggregates,
            vec![None],
...
");
    ...
    fn single_test_aggregate() -> Result<BlockedAggregateExec> {
        let schema = Arc::new(Schema::new(vec![
            Field::new("a", DataType::UInt32, false),
            Field::new("b", DataType::Float64, false),
        ]));
...
");
    ...
    async fn ordered_single_aggregate_planning() -> Result<()> {
        ...
        let aggregate = BlockedAggregateExec::try_new(
            AggregateMode::Single,
...
");
    ...
    fn partial_reduce_test_aggregate() -> Result<BlockedAggregateExec> {
        let schema = Arc::new(Schema::new(vec![
            Field::new("a", DataType::UInt32, false),
            Field::new("b", DataType::Float64, false),
        ]));
        ...
        let partial = BlockedAggregateExec::try_new(
            AggregateMode::Partial,
            group_by.clone(),
            aggregates.clone(),
            vec![None],
...
");
    ...
    async fn ordered_final_aggregate_planning() -> Result<()> {
        ...
        let empty_input =
            TestMemoryExec::try_new_exec(&[vec![]], Arc::clone(&schema), None)?;
        let partial_aggregate = BlockedAggregateExec::try_new(
            AggregateMode::Partial,
            group_by.clone(),
            aggr_expr.clone(),
            vec![None],
...
");
    ...
    async fn first_last_multi_partitions(
        ...
        let aggregate_final = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Final,
            groups,
            aggregates.clone(),
            vec![None],
    ...
    async fn run_test_with_spill_pool_if_necessary(
        ...
        let single_aggregate = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Single,
            grouping_set,
            aggregates,
            vec![None, None],
    ...
    fn assert_spill_count_metric(
        expect_spill: bool,
        single_aggregate: Arc<BlockedAggregateExec>,
    ) {
    ...
    async fn test_aggregate_statistics_edge_cases() -> Result<()> {
        ...
        let single_agg_zero = BlockedAggregateExec::try_new(
            AggregateMode::Single,
            PhysicalGroupBy::default(),
    ...
    async fn test_aggregate_statistics_empty_input_partial_mode_scaling() -> Result<()> {
        ...
        let agg = BlockedAggregateExec::try_new(
            AggregateMode::Partial,
            grouping_sets_with_empty(&schema, 1)?,
            vec![count_a_aggregate(&schema)?],
            vec![None],
    ...
    fn build_test_aggregate_with_mode(
        ...
        let mut agg = BlockedAggregateExec::try_new(
            mode,
            group_by,
            vec![count_a_aggregate(schema)?],
            vec![None],
    ...
    async fn test_aggregate_stats_duplicate_empty_grouping_sets() -> Result<()> {
        ...
        let single_input =
            Arc::new(EmptyExec::new(Arc::clone(&schema))) as Arc<dyn ExecutionPlan>;
        let single_agg = BlockedAggregateExec::try_new(
            AggregateMode::Single,
        ...
        let partial_input =
            Arc::new(EmptyExec::new(Arc::clone(&schema)).with_partitions(2))
                as Arc<dyn ExecutionPlan>;
        let partial_agg = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Partial,
    ...
    async fn test_order_is_retained_when_spilling() -> Result<()> {
        ...
        let aggr = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Single,
            PhysicalGroupBy::new(
                vec![
    ...
    async fn evaluate_partial_reduce(
        ...
        let partial1 = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Partial,
            groups.clone(),
            aggregates.clone(),
        ...
        let partial2 = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Partial,
            groups.clone(),
            aggregates.clone(),
            vec![None; aggregates.len()],
        ...
        let partial_reduce = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::PartialReduce,
            groups.clone(),
            aggregates.clone(),
            vec![None; aggregates.len()],
        ...
        let final_agg = Arc::new(BlockedAggregateExec::try_new(
            AggregateMode::Final,
            groups.clone(),
            aggregates.clone(),
            vec![None; aggregates.len()],
    ...
    /// Test that [`BlockedAggregateExec::set_dynamic_filter`] overrides the existing dynamic filter
    #[test]
    ...
    /// Test that [`BlockedAggregateExec::set_dynamic_filter`] errors when the aggregate does not support dynamic filtering
    #[test]
    ...
    /// Test that [`BlockedAggregateExec::set_dynamic_filter`] errors when the column is not in the schema
    #[test]
