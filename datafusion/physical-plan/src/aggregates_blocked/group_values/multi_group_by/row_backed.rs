...
//! boolean, ...), [`BlockedRowsGroupColumn`] works for *any* data type that arrow's
...
//! see the float `-0.0` / `NaN` note on [`BlockedRowsGroupColumn`].
//!
...
pub(crate) use crate::aggregates_blocked::group_values::row::encode_array_if_necessary;
...
/// [`GroupValuesRows`]: crate::aggregates::group_values::GroupValuesRows
pub struct BlockedRowsGroupColumn<const IS_FIXED_BLOCK_SIZE: bool> {
    /// Accumulated group values in row format; `group_values.row(i)` is the
    /// group value for group index `i`.
    group_values: BlockedRowsBuilder<IS_FIXED_BLOCK_SIZE>,
...
impl<const IS_FIXED_BLOCK_SIZE: bool> BlockedRowsGroupColumn<IS_FIXED_BLOCK_SIZE> {
    ...
    /// Create an empty [`BlockedRowsGroupColumn`] for `data_type`.
...
impl<const IS_FIXED_BLOCK_SIZE: bool> BlockedGroupColumn<IS_FIXED_BLOCK_SIZE>
    for BlockedRowsGroupColumn<IS_FIXED_BLOCK_SIZE>
{
    ...
    fn take_all(mut self: Box<Self>) -> Vec<ArrayRef> {
        let blocks = self.group_values.take_all();

        blocks
            .into_iter()
            .map(|b| {
                let result = self.group_values.row_converter().convert_rows(&b)?;
                let array = result.into_iter().next().unwrap();
                Ok(encode_array_if_necessary(&array, &self.output_type)
                    .expect("dictionary re-encode during emit"))
            })
            .collect::<Result<Vec<_>>>()
            .expect("row conversion during take_all")
    }

    fn take_next_block(&mut self) -> Option<ArrayRef> {
        let b = self.group_values.take_block()?;

        let result = self.group_values.row_converter().convert_rows(&b).unwrap();
        let array = result.into_iter().next().unwrap();
        Some(
            encode_array_if_necessary(&array, &self.output_type)
              .expect("dictionary re-encode during emit"),
    ...
    fn take_n(&mut self, n: usize, adjusted_block_size: Option<&[usize]>) -> ArrayRef {
        ...
        let b = self.group_values.take_n(n, adjusted_block_size.map(|s| s.iter().copied()));
