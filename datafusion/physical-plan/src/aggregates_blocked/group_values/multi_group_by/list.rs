...
use super::{BlockedGroupColumn, nulls_equal_to};
use super::super::null_builder::NullBufferBuilderExt;
...
use std::sync::Arc;
use datafusion_expr_common::blocked_helpers::{BlockedNullsBuilder, BlockedOffsetBufferBuilder};
...
pub struct ListGroupValueBuilder<const IS_FIXED_BLOCK_SIZE: bool, O: OffsetSizeTrait> {
    field: FieldRef,
    offsets: BlockedOffsetBufferBuilder<IS_FIXED_BLOCK_SIZE, O>,
    child: Box<dyn BlockedGroupColumn<false>>,
    outer_nulls: BlockedNullsBuilder<IS_FIXED_BLOCK_SIZE>,
}

impl<const IS_FIXED_BLOCK_SIZE: bool, O: OffsetSizeTrait> ListGroupValueBuilder<IS_FIXED_BLOCK_SIZE, O> {
    pub fn new(field: FieldRef, child: Box<dyn BlockedGroupColumn<false>>, block_size: usize) -> Self {
        Self {
            field,
            offsets: BlockedOffsetBufferBuilder::new(block_size),
            child,
            outer_nulls: BlockedNullsBuilder::new(block_size),
        }
    }

    fn push_offset(&mut self, additional: usize) -> Result<bool> {
        let additional = O::from_usize(additional).ok_or_else(|| {
            internal_datafusion_err!(
                "List sublist length {additional} overflows the offset type"
            )
        })?;
        let next = self.offsets.last_offset().checked_add(&additional).ok_or_else(|| {
            internal_datafusion_err!(
                "List offset overflow: current={} additional={}",
                self.offsets.last_offset().as_usize(),
                additional.as_usize()
            )
        })?;
        Ok(self.offsets.push_next_offset_in_block(next))
    }
    ...
    fn equal_to_typed(
        ...
        let (lhs_start, lhs_end) = self.offsets.value_offsets(lhs_row);
        let lhs_start = lhs_start.as_usize();
        let lhs_end = lhs_end.as_usize();
        ...
        for j in 0..lhs_len {
            if !self
                .child
                .equal_to(BlocksIndex::new(block_index, lhs_start + j), values, rhs_range.start + j)
            {
                return false;
            }
        }
...
impl<const IS_FIXED_BLOCK_SIZE: bool, O: OffsetSizeTrait> BlockedGroupColumn<IS_FIXED_BLOCK_SIZE> for ListGroupValueBuilder<IS_FIXED_BLOCK_SIZE, O> {
    fn batch_size(&self) -> usize {
        self.outer_nulls.block_size()
    }
    ...
    fn values_preserving(&self, selection: BlockedGroupSelection<'_>) -> Result<ArrayRef> {
        selection.validate_num_groups(self.outer_nulls.len())?;
        ...
        for row in selection.iter() {
            ...
            let row_end = row_end.as_usize();
            child_indices.extend((start..row_end).map(|in_block| BlocksIndex::new(row_block_index, in_block)));
        ...
        let child_selection =
            BlockedGroupSelection::try_from_indices(&child_indices, self.child.len(), self.batch_size())?;
    ...
    fn take_all(mut self: Box<Self>) -> Vec<ArrayRef> {
        let offsets = self.offsets.take_all();
        let values = self.child.take_all();
        let nulls = self.outer_nulls.take_all();

        assert_eq!(offsets.len(), values.len());
        assert_eq!(offsets.len(), nulls.len());

        offsets.into_iter().zip(values).zip(nulls)
          .map(|((offsets, values), nulls)| {
    ...
    fn take_next_block(&mut self) -> Option<ArrayRef> {
        let offsets = self.offsets.take_block_finished();
        let values = self.child.take_next_block();
        let nulls = self.outer_nulls.take_block();

        let (offsets, values, nulls) = match (offsets, values, nulls) {
            (Some(offsets), Some(values), Some(nulls)) => (offsets, values, nulls),
            (None, None, None) => return None,
            _ => panic!("either all must be Some or all must be None"),
        };

        Some(Arc::new(GenericListArray::<O>::new(
            Arc::clone(&self.field),
            offsets,
            values,
            nulls,
    ...
    fn take_n(&mut self, n: usize, adjusted_block_size: Option<&[usize]>) -> ArrayRef {
        assert_eq!(adjusted_block_size.is_none(), IS_FIXED_BLOCK_SIZE);

        let offsets = self.offsets.take_n(n, adjusted_block_size.map(|s| s.iter().copied()));
        let nulls = self.outer_nulls.take_n(n, adjusted_block_size.map(|s| s.iter().copied()));

        // The offsets are already re-laid out, the last offset of each block is its byte count
        let items_per_block: Vec<usize> = self
          .offsets
          .blocks_iter()
          .map(|block| block[block.len() - 1].as_usize())
          .collect();
        let values = self.child.take_n(n, Some(items_per_block.as_slice()));
...
// mod tests {
//     use super::*;
//     use crate::aggregates_blocked::group_values::multi_group_by::primitive::PrimitiveGroupValueBuilder;
//     use arrow::array::{
...
//     }
//
//     fn make_child() -> Box<dyn BlockedGroupColumn<false>> {
//         Box::new(PrimitiveGroupValueBuilder::<false, Int32Type, true>::new(
//             DataType::Int32,
//         ))
//     }
//
...
//             Some(vec![]),
//         ]);
//         let mut builder = ListGroupValueBuilder::<true, i32>::new(child_field(), make_child());
