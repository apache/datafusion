...
/// Verifies that a probe-driven single join (`RightSingle`) matched each probe
/// row at most once.
///
/// `probe_indices` holds the matched probe-side indices of one chunk of a probe
/// batch, in ascending order, so a second match for a probe row is either
/// adjacent to the first or -- when a probe row's matches straddle a chunk
/// boundary -- equal to `last_joined_probe_idx`, the last probe index joined by
/// the preceding chunk of the same batch.
