...
pub(crate) trait JoinAcceleratorProber: Debug + Send {
    ...
    ///
    /// Efficiency guideline (violations won't cause an error):
    ///
    ...
    ///   input batch size. Larger batches can be slower to process or use more
    ///   memory.
    ///
...
pub(crate) enum JoinProbeCandidates {
    ...
    // Requires variants for future indexed probes or other representations that
...
impl JoinAcceleratorProber for FallbackNestedLoopJoinProber {
    /// Each call returns candidates like:
    ///
    /// ```text
