...
async fn collect_left_partition_statistics() -> Result<()> {
    ...
    for partition in 0..n {
        // Must not assert/error: the single-partition left is queried with `None`,
        // and right partition `partition` is valid.
        join.statistics_with_args(
            &StatisticsArgs::new().with_partition(Some(partition)),
        )?;
    }
    join.statistics_with_args(&StatisticsArgs::new())?;
    Ok(())
}
