...
impl ExecutionPlan for NestedLoopJoinExec {
    ...
    fn execute(
        ...
        // 3. FULL join with multiple right partitions is not yet supported
...
impl JoinLeftData {
    ...
    pub(crate) fn batch(&self) -> &RecordBatch {
        self.accelerator.build_batch()
    }

    pub(crate) fn accelerator(&self) -> &dyn JoinAccelerator {
...
pub(crate) struct NestedLoopJoinStream {
    ...
    ///
    /// # Boundaries between `JoinAccelerator` and the surrounding join logic
    /// - The accelerator enumerates candidate pairs, represented as indices
    ///   into the current left and right batches, using only accelerated predicates.
    /// - The surrounding join logic handles remaining join filters, bitmap tracking
    ///   for outer/semi/anti joins, output buffering, and memory-limited fallback.
    ///
    /// See the documentation for [`JoinAccelerator`] for details.
    accelerator: JoinAcceleratorRef,
...
impl NestedLoopJoinStream {
    ...
    fn handle_buffering_left_memory_limited(
        ...
        if let Err(e) = accelerator.finish() {
            return ControlFlow::Break(Poll::Ready(Some(Err(e))));
        }
    ...
    fn process_right_unmatched(&mut self) -> Result<Option<RecordBatch>> {
        ...
        let right_batch_bitmap = std::mem::take(&mut self.current_right_batch_matched)
            .ok_or_else(|| {
    ...
    fn update_matched_bitmap(
        ...
        if self.should_track_unmatched_right {
            ...
            // Put the right bitmap back after the bitwise OR.
