...
impl NestedLoopJoinStream {
    ...
    fn process_left_range_join(
        ...
        // matched bitmaps double as the duplicate detectors: a bit that is
        // already set means an earlier pair matched the same row, whether in
        // this left range, an earlier one, or -- for the left bitmap, which is
        // shared -- another partition.
        ...
        if self.should_track_unmatched_right {
            ...
            let current_right_bitmap = local_right_bitmap
                ...
                .finish();
            // A right row already matched by an earlier left range must not
            // match again in this one.
