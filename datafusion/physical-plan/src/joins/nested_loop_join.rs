...
impl RightExistenceProbe {
    ...
    fn probe(
        ...
        let matches = match &self.filter {
            Some(filter) if count == 1 => {
                apply_filter_to_row_join_batch(left, start, &self.candidates, filter)?
            }
            Some(filter) => {
                let left_indices = UInt32Array::from_iter_values(
                    (start..start + count)
                        .flat_map(|i| std::iter::repeat_n(i as u32, candidate_rows)),
                );
...
fn apply_filter_to_indices(
    ...
    let columns = filter
        ...
        .iter()
        .map(|column| {
            let (batch, indices) = if column.side == JoinSide::Left {
                (left, left_indices)
            } else {
                (right, right_indices)
            };
            take(batch.column(column.index).as_ref(), indices, None)
        })
