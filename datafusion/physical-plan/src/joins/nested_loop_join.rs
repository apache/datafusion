...
impl ExecutionPlan for NestedLoopJoinExec {
    ...
    fn with_new_children_and_same_properties(
        ...
        let right_partition_count = right.output_partitioning().partition_count().max(1);
        let with_visited_bitmap = need_produce_result_in_final(self.join_type);
...
/// Helper to await `OnceFut::get_shared` outside of `poll_next` context.
async fn left_spill_fut_get_shared(
    mut fut: OnceFut<LeftSpillData>,
) -> Result<Arc<LeftSpillData>> {
    futures::future::poll_fn(move |cx| fut.get_shared(cx)).await
}
...
pub(crate) struct SpillStateActive {
    ...
    /// Cached left schema, set after the first chunk is fetched.
    /// Used by `EmitGlobalRightUnmatched` to build NULL-padded left
    /// columns for unmatched right rows.
    left_schema: Option<SchemaRef>,
}
...
impl NestedLoopJoinStream {
    ...
    fn initiate_fallback(&mut self) -> Result<()> {
        ...
        let (left_plan, context, left_spill_data, fallback_coordinator) =
