...
mod tests {
    ...
    async fn test_try_finish_marks_finished_with_prefix() -> Result<()> {
        ...
        let array_b1: ArrayRef = Arc::new(Float64Array::from(vec![10.0, 20.0, 30.0]));
        let batch1 = RecordBatch::try_new(Arc::clone(&schema), vec![array_a1, array_b1])?;

        // Insert the first batch.
        // At this point the heap is not yet “finished” because the prefix (last row value in column "a" is 3)
        ...
        let array_b2: ArrayRef = Arc::new(Float64Array::from(vec![30.0, 10.0]));
        let batch2 = RecordBatch::try_new(Arc::clone(&schema), vec![array_a2, array_b2])?;

        // Insert the second batch.
        // The last row in this batch has a prefix value of 3 in column "a",
        ...
        // (the value 30.0 for b is from the second batch).
