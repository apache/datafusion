...
pub use datafusion_expr::{Accumulator, ColumnarValue, MetricsDocumentation};
...
pub trait ExecutionPlan: Any + Debug + DisplayAs + Send + Sync {
    ...
    /// Returns documentation for the metrics this `ExecutionPlan` emits.
    ///
    /// The default returns `None`. Operators that emit operator-specific
    /// metrics should override this to return a `&'static MetricsDocumentation`
    /// (typically backed by a `LazyLock`). The DataFusion metrics user-guide
    /// page is generated by iterating the registry of these documents.
    fn metrics_documentation(&self) -> Option<&'static MetricsDocumentation> {
        None
    }
