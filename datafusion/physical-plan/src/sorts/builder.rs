...
impl BatchBuilder {
    ...
    /// Append the next `n` row from `stream_idx`
    ...
    pub fn build_record_batch(&mut self, n: usize) -> Result<Option<RecordBatch>> {
        ...
        let initial_rows_to_emit = n.min(self.indices.len());

        let (rows_to_emit, columns) =
            retry_interleave(initial_rows_to_emit, initial_rows_to_emit, |rows_to_emit| {
