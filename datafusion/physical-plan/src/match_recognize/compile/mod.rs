...
impl CompiledPatternBuilder {
    ...
    /// Specify the `AFTER MATCH SKIP` strategy (optional).
    pub fn after_match_skip(mut self, skip: Option<AfterMatchSkip>) -> Self {
        self.after_match_skip = skip;
        self
    }

    /// Specify the `ROWS PER MATCH` clause (optional).
    pub fn rows_per_match(mut self, rows: Option<RowsPerMatch>) -> Self {
