...
use crate::match_recognize::pattern_matcher::{pattern_schema, PatternMatcher};
...
impl MatchRecognizePatternExec {
    ...
    pub fn try_new(
        ...
        // Create ordering requirement from order_by
        let _required_input_ordering = if let Some(ref ordering) = order_by {
            ordering.iter().map(|expr| {
                datafusion_physical_expr_common::sort_expr::PhysicalSortRequirement::new(
                    expr.expr.clone(),
                    Some(expr.options)
                )
            }).collect()
...
impl DisplayAs for MatchRecognizePatternExec {
    fn fmt_as(
        ...
        write!(f, "MatchRecognizePatternExec: pattern={:?}", self.pattern)?;
...
impl MatchRecognizePatternStream {
    pub fn try_new(
        ...
        let pattern_matcher = PatternMatcher::new(pattern, symbols, after_match_skip, rows_per_match)?;
