...
use datafusion_expr::match_recognize::{
    AfterMatchSkip, MatchRecognizeOutputSpec, OutputSpecWithArrowSchema, Pattern,
    RowsPerMatch,
};
...
impl MatchRecognizePatternExec {
    ...
    fn compute_properties(
        ...
        // 1) Seed equivalence properties with input properties and current schema
        //    (preserves input equivalences and allows us to advertise additional orderings)
        let mut eq_properties = EquivalenceProperties::new(Arc::clone(schema))
            .extend(input.equivalence_properties().clone())?;

        // 2) Resolve partition-by sort keys that actually preserve the input ordering
        //    These keys always form the prefix of any advertised ordering
        let ordered_partition_by_indices =
            get_ordered_partition_by_indices(partition_by, input)?;
        let prefix_sort_keys = get_partition_by_sort_exprs(
            input,
            partition_by,
            &ordered_partition_by_indices,
        )?;

        // 3) Determine which suffix components are admissible
        //    - order-by keys are admissible only with skip strategies that do not backtrack
        //    - match_number is admissible only when projected and unmatched rows are not emitted
        //    - match_sequence_number is admissible only when projected and unmatched rows are off
        let is_skip_safe = matches!(after_match_skip, AfterMatchSkip::PastLastRow);
        let is_order_by_admissible =
            is_skip_safe && order_by.as_ref().map(|v| !v.is_empty()).unwrap_or(false);
        let order_by_sort_keys: Vec<PhysicalSortExpr> = if is_order_by_admissible {
            match order_by.clone() {
                Some(lex) => Vec::<PhysicalSortExpr>::from(lex),
                None => Vec::new(),
            }
        } else {
            Vec::new()
        ...
        let is_match_number_admissible =
            is_match_number_projected && !compiled_pattern.with_unmatched_rows;

        let is_match_sequence_projected =
            metadata_columns.contains(&MrMetadataColumn::MatchSequenceNumber);
        let is_match_sequence_admissible =
            is_match_sequence_projected && !compiled_pattern.with_unmatched_rows;

        let match_number_sort = if is_match_number_admissible {
            Some(PhysicalSortExpr::new_default(col(
        ...
        // 4) Build at most two admissible suffix orderings (to be prefixed with partition keys):
        //    - [ORDER BY]
        //    - [MATCH_NUMBER, MATCH_SEQUENCE_NUMBER]
        let mut suffix_orderings: Vec<Vec<PhysicalSortExpr>> = Vec::new();

        // [ORDER BY] when admissible
        if is_order_by_admissible {
            suffix_orderings.push(order_by_sort_keys.clone());
        }

        // [MATCH_NUMBER (+ MATCH_SEQUENCE_NUMBER)] when admissible
        ...
        // 5) If nothing else to advertise and we have a prefix, advertise [prefix] only
        if suffix_orderings.is_empty() && !prefix_sort_keys.is_empty() {
            suffix_orderings.push(Vec::new());
        }

        // 6) Prefix all suffix orderings with partition keys and add to equivalence properties
        for tail in suffix_orderings {
            let mut full: Vec<PhysicalSortExpr> =
                Vec::with_capacity(prefix_sort_keys.len() + tail.len());
            full.extend(prefix_sort_keys.iter().cloned());
    ...
    fn fmt_as(
        ...
        if !self.output_spec.is_empty() {
            write!(
                ...
                OutputSpecWithArrowSchema::new(&self.output_spec, &self.schema,),
            )?;
        } else {
            write!(
                f,
...
pub(crate) struct MatchAccumulator {
    ...
    included_builder: Option<BooleanBuilder>,
...
impl MatchAccumulator {
    ...
    pub fn try_new(
        ...
        let has_included_row = output_spec
            .metadata_columns
            .contains(&MrMetadataColumn::IsExcludedRow);
        ...
        Ok(Self {
            ...
            included_builder: if has_included_row {
    ...
    pub fn record_match(
        ...
        for step in path.iter() {
            ...
            if let Some(builder) = &mut self.seq_builder {
                // Advance and append for every matched row (excluded or not)
                seq_counter += 1;
                builder.append_value(seq_counter);
            }

            // __mr_is_excluded_row flag
            if let Some(builder) = &mut self.included_builder {
                builder.append_value(is_excluded);
            }
        }
        Ok(())
    }
    ...
    pub fn flush(&mut self, input_batch: &RecordBatch) -> Result<Option<RecordBatch>> {
        ...
        for col in &self.output_spec.metadata_columns {
            match col {
                ...
                MrMetadataColumn::IsExcludedRow => {
                    if let Some(builder) = &mut self.included_builder {
                        final_columns.push(Arc::new(builder.finish()));
                    }
                }
