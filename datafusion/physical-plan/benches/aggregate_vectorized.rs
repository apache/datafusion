...
use arrow::util::bench_util::{create_primitive_array, create_string_view_array_with_len, create_string_view_array_with_max_len};
...
fn bench_vectorized_append(c: &mut Criterion) {
    ...
    for &size in &SIZES {
        ...
        for &null_density in &NULL_DENSITIES {
            if null_density == 0.0 {
                bench_single::<false>(&mut group, size, &rows, null_density)
            }
            bench_single::<true>(&mut group, size, &rows, null_density);
        }
    }
...
fn bench_single<const NULLABLE: bool>(group: &mut BenchmarkGroup<WallTime>, size: usize, rows: &Vec<usize>, null_density: f32) {
