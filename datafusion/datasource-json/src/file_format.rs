...
impl DataSink for JsonSink {
    ...
    /// Emit a `JsonSink` node wrapping the shared sink config. Kept
    /// byte-identical to the former `try_from_data_sink_exec` JsonSink branch in
    /// `datafusion-proto`.
    #[cfg(feature = "proto")]
    fn try_to_proto(
        &self,
...
#[cfg(feature = "proto")]
impl JsonSink {
    /// Reconstruct a `DataSinkExec` (wrapping a `JsonSink`) from a `JsonSink`
    /// [`PhysicalPlanNode`].
    ///
    /// The inverse of [`DataSink::try_to_proto`] on `JsonSink`; kept
    /// byte-compatible with the former `try_into_json_sink_physical_plan` in
    /// `datafusion-proto`.
    ///
