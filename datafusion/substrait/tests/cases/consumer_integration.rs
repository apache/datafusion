...
mod tests {
    ...
    async fn tpch_test_04() -> Result<()> {
        ...
        assert_eq!(
            plan_str,
            "Projection: ORDERS.O_ORDERPRIORITY, count_star() AS ORDER_COUNT\
            \n  Sort: ORDERS.O_ORDERPRIORITY ASC NULLS LAST\
            \n    Aggregate: groupBy=[[ORDERS.O_ORDERPRIORITY]], aggr=[[count_star()]]\
    ...
    async fn tpch_test_13() -> Result<()> {
        ...
        assert_eq!(
            plan_str,
            "Projection: count(ORDERS.O_ORDERKEY) AS C_COUNT, count_star() AS CUSTDIST\
            \n  Sort: count_star() DESC NULLS FIRST, count(ORDERS.O_ORDERKEY) DESC NULLS FIRST\
            \n    Projection: count(ORDERS.O_ORDERKEY), count_star()\
            \n      Aggregate: groupBy=[[count(ORDERS.O_ORDERKEY)]], aggr=[[count_star()]]\
    ...
    async fn tpch_test_21() -> Result<()> {
        ...
        assert_eq!(
            plan_str,
            "Projection: SUPPLIER.S_NAME, count_star() AS NUMWAIT\
            \n  Limit: skip=0, fetch=100\
            \n    Sort: count_star() DESC NULLS FIRST, SUPPLIER.S_NAME ASC NULLS LAST\
            \n      Aggregate: groupBy=[[SUPPLIER.S_NAME]], aggr=[[count_star()]]\
    ...
    async fn tpch_test_22() -> Result<()> {
        ...
        assert_eq!(
            plan_str,
            "Projection: substr(CUSTOMER.C_PHONE,Int32(1),Int32(2)) AS CNTRYCODE, count_star() AS NUMCUST, sum(CUSTOMER.C_ACCTBAL) AS TOTACCTBAL\
            \n  Sort: substr(CUSTOMER.C_PHONE,Int32(1),Int32(2)) ASC NULLS LAST\
            \n    Aggregate: groupBy=[[substr(CUSTOMER.C_PHONE,Int32(1),Int32(2))]], aggr=[[count_star(), sum(CUSTOMER.C_ACCTBAL)]]\
