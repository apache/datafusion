...
use crate::variation_const::{
    DATE_32_TYPE_VAR_REF, DATE_64_TYPE_VAR_REF, DECIMAL_128_TYPE_VAR_REF,
    DECIMAL_256_TYPE_VAR_REF, DEFAULT_CONTAINER_TYPE_VAR_REF, DEFAULT_TYPE_VAR_REF,
    INTERVAL_DAY_TIME_TYPE_REF, INTERVAL_MONTH_DAY_NANO_TYPE_REF,
    INTERVAL_YEAR_MONTH_TYPE_REF, LARGE_CONTAINER_TYPE_VAR_REF,
    TIMESTAMP_MICRO_TYPE_VAR_REF, TIMESTAMP_MILLI_TYPE_VAR_REF,
    TIMESTAMP_NANO_TYPE_VAR_REF, TIMESTAMP_SECOND_TYPE_VAR_REF,
    UNSIGNED_INTEGER_TYPE_VAR_REF,
};
    ...
    match &dt.kind {
        Some(s_kind) => match s_kind {
            ...
            r#type::Kind::I8(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(DataType::Int8),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(DataType::UInt8),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            r#type::Kind::I16(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(DataType::Int16),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(DataType::UInt16),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            r#type::Kind::I32(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(DataType::Int32),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(DataType::UInt32),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            r#type::Kind::I64(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(DataType::Int64),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(DataType::UInt64),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            ...
            r#type::Kind::Timestamp(ts) => match ts.type_variation_reference {
                TIMESTAMP_SECOND_TYPE_VAR_REF => {
                    Ok(DataType::Timestamp(TimeUnit::Second, None))
                }
                TIMESTAMP_MILLI_TYPE_VAR_REF => {
                    Ok(DataType::Timestamp(TimeUnit::Millisecond, None))
                }
                TIMESTAMP_MICRO_TYPE_VAR_REF => {
                    Ok(DataType::Timestamp(TimeUnit::Microsecond, None))
                }
                TIMESTAMP_NANO_TYPE_VAR_REF => {
            ...
            r#type::Kind::Date(date) => match date.type_variation_reference {
                DATE_32_TYPE_VAR_REF => Ok(DataType::Date32),
                DATE_64_TYPE_VAR_REF => Ok(DataType::Date64),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            r#type::Kind::Binary(binary) => match binary.type_variation_reference {
                DEFAULT_CONTAINER_TYPE_VAR_REF => Ok(DataType::Binary),
                LARGE_CONTAINER_TYPE_VAR_REF => Ok(DataType::LargeBinary),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            ...
            r#type::Kind::String(string) => match string.type_variation_reference {
                DEFAULT_CONTAINER_TYPE_VAR_REF => Ok(DataType::Utf8),
                LARGE_CONTAINER_TYPE_VAR_REF => Ok(DataType::LargeUtf8),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {s_kind:?}"
                ),
            },
            r#type::Kind::List(list) => {
                ...
                match list.type_variation_reference {
                    DEFAULT_CONTAINER_TYPE_VAR_REF => Ok(DataType::List(field)),
                    LARGE_CONTAINER_TYPE_VAR_REF => Ok(DataType::LargeList(field)),
                    v => not_impl_err!(
                        "Unsupported Substrait type variation {v} of type {s_kind:?}"
                    )?,
                }
...
fn from_substrait_literal(
    ...
    let scalar_value = match &lit.literal_type {
        ...
        Some(LiteralType::I8(n)) => match lit.type_variation_reference {
            DEFAULT_TYPE_VAR_REF => ScalarValue::Int8(Some(*n as i8)),
            UNSIGNED_INTEGER_TYPE_VAR_REF => ScalarValue::UInt8(Some(*n as u8)),
            others => {
                return substrait_err!("Unknown type variation reference {others}");
            }
        },
        Some(LiteralType::I16(n)) => match lit.type_variation_reference {
            DEFAULT_TYPE_VAR_REF => ScalarValue::Int16(Some(*n as i16)),
            UNSIGNED_INTEGER_TYPE_VAR_REF => ScalarValue::UInt16(Some(*n as u16)),
            others => {
                return substrait_err!("Unknown type variation reference {others}");
            }
        },
        Some(LiteralType::I32(n)) => match lit.type_variation_reference {
            DEFAULT_TYPE_VAR_REF => ScalarValue::Int32(Some(*n)),
            UNSIGNED_INTEGER_TYPE_VAR_REF => ScalarValue::UInt32(Some(*n as u32)),
            others => {
                return substrait_err!("Unknown type variation reference {others}");
            }
        },
        Some(LiteralType::I64(n)) => match lit.type_variation_reference {
            DEFAULT_TYPE_VAR_REF => ScalarValue::Int64(Some(*n)),
            UNSIGNED_INTEGER_TYPE_VAR_REF => ScalarValue::UInt64(Some(*n as u64)),
            others => {
                return substrait_err!("Unknown type variation reference {others}");
            }
        },
        ...
        Some(LiteralType::Timestamp(t)) => match lit.type_variation_reference {
            TIMESTAMP_SECOND_TYPE_VAR_REF => ScalarValue::TimestampSecond(Some(*t), None),
            TIMESTAMP_MILLI_TYPE_VAR_REF => {
        ...
        Some(LiteralType::String(s)) => match lit.type_variation_reference {
            DEFAULT_CONTAINER_TYPE_VAR_REF => ScalarValue::Utf8(Some(s.clone())),
            LARGE_CONTAINER_TYPE_VAR_REF => ScalarValue::LargeUtf8(Some(s.clone())),
            others => {
                return substrait_err!("Unknown type variation reference {others}");
            }
        },
        Some(LiteralType::Binary(b)) => match lit.type_variation_reference {
            DEFAULT_CONTAINER_TYPE_VAR_REF => ScalarValue::Binary(Some(b.clone())),
            LARGE_CONTAINER_TYPE_VAR_REF => ScalarValue::LargeBinary(Some(b.clone())),
            others => {
                return substrait_err!("Unknown type variation reference {others}");
            }
        },
        ...
        Some(LiteralType::List(l)) => {
            ...
            match lit.type_variation_reference {
                DEFAULT_CONTAINER_TYPE_VAR_REF => ScalarValue::List(
                    ScalarValue::new_list(elements.as_slice(), &element_type),
                ),
                LARGE_CONTAINER_TYPE_VAR_REF => ScalarValue::LargeList(
        ...
        Some(LiteralType::EmptyList(l)) => {
            ...
            match lit.type_variation_reference {
                DEFAULT_CONTAINER_TYPE_VAR_REF => {
...
fn from_substrait_null(
    ...
    if let Some(kind) = &null_type.kind {
        match kind {
            ...
            r#type::Kind::I8(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(ScalarValue::Int8(None)),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(ScalarValue::UInt8(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            r#type::Kind::I16(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(ScalarValue::Int16(None)),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(ScalarValue::UInt16(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            r#type::Kind::I32(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(ScalarValue::Int32(None)),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(ScalarValue::UInt32(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            r#type::Kind::I64(integer) => match integer.type_variation_reference {
                DEFAULT_TYPE_VAR_REF => Ok(ScalarValue::Int64(None)),
                UNSIGNED_INTEGER_TYPE_VAR_REF => Ok(ScalarValue::UInt64(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            ...
            r#type::Kind::Date(date) => match date.type_variation_reference {
                DATE_32_TYPE_VAR_REF => Ok(ScalarValue::Date32(None)),
                DATE_64_TYPE_VAR_REF => Ok(ScalarValue::Date64(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            r#type::Kind::Binary(binary) => match binary.type_variation_reference {
                DEFAULT_CONTAINER_TYPE_VAR_REF => Ok(ScalarValue::Binary(None)),
                LARGE_CONTAINER_TYPE_VAR_REF => Ok(ScalarValue::LargeBinary(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            ...
            r#type::Kind::String(string) => match string.type_variation_reference {
                DEFAULT_CONTAINER_TYPE_VAR_REF => Ok(ScalarValue::Utf8(None)),
                LARGE_CONTAINER_TYPE_VAR_REF => Ok(ScalarValue::LargeUtf8(None)),
                v => not_impl_err!(
                    "Unsupported Substrait type variation {v} of type {kind:?}"
                ),
            },
            ...
            r#type::Kind::List(l) => {
                ...
                match l.type_variation_reference {
                    DEFAULT_CONTAINER_TYPE_VAR_REF => Ok(ScalarValue::List(Arc::new(
                        GenericListArray::new_null(field.into(), 1),
                    ))),
                    LARGE_CONTAINER_TYPE_VAR_REF => Ok(ScalarValue::LargeList(Arc::new(
                        GenericListArray::new_null(field.into(), 1),
                    ))),
                    v => not_impl_err!(
                        "Unsupported Substrait type variation {v} of type {kind:?}"
