...
mod tests {
    ...
    fn ordering(schema: &Schema) -> LexOrdering {
        let expr = datafusion_physical_expr::expressions::col(
            schema.fields().iter().next().unwrap().name(),
            schema,
        )
        .unwrap();
    ...
    fn test_ordering_cache() {
        ...
        if cached.is_valid_for(&meta) && cached.ordering.is_none() {
            cached.ordering = Some(ordering(&schema));
        }
