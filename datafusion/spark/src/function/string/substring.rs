...
mod tests {
    ...
    use crate::function::string::substring::SparkSubstring;
    use crate::function::utils::test::test_scalar_function;

    /// Test macro for substring(str, pos) - 2 argument version
    macro_rules! test_substring_2 {
        ($INPUT:expr, $POS:expr, $EXPECTED:expr) => {
            test_scalar_function!(
                SparkSubstring::new(),
                vec![
                    ColumnarValue::Scalar(ScalarValue::Utf8($INPUT)),
                    ColumnarValue::Scalar(ScalarValue::Int64($POS))
                ],
                $EXPECTED,
                &str,
                DataType::Utf8,
    ...
    /// Test macro for substring(str, pos, len) - 3 argument version
    macro_rules! test_substring_3 {
        ($INPUT:expr, $POS:expr, $LEN:expr, $EXPECTED:expr) => {
            test_scalar_function!(
                SparkSubstring::new(),
                vec![
                    ColumnarValue::Scalar(ScalarValue::Utf8($INPUT)),
                    ColumnarValue::Scalar(ScalarValue::Int64($POS)),
                    ColumnarValue::Scalar(ScalarValue::Int64($LEN))
                ],
                $EXPECTED,
                &str,
                DataType::Utf8,
    ...
    #[test]
    fn test_length_is_respected() -> Result<()> {
        // substring("hello, world!", 2, 7) -> "ello, w"
        test_substring_3!(
        ...
        // substring("short", 2, 7) -> "hort" (length exceeds remaining)
        test_substring_3!(
    ...
    #[test]
    fn test_negative_start_with_length() -> Result<()> {
        // substring("hello, world!", -5, 3) -> "orl"
        // -5 + 13 = 8, so start at index 8 (0-indexed), take 3 chars
        test_substring_3!(
            Some(String::from("hello, world!")),
            Some(-5),
            Some(3),
        ...
        // substring("short", -5, 3) -> "sho"
        // -5 + 5 = 0, so start at index 0, take 3 chars
        test_substring_3!(
            Some(String::from("short")),
            Some(-5),
            Some(3),
    ...
    #[test]
    fn test_negative_start_without_length() -> Result<()> {
        // substring("hello, world!", -2) -> "d!"
        // -2 + 13 = 11, so start at index 11 (0-indexed)
        test_substring_2!(
            Some(String::from("hello, world!")),
            Some(-2),
            Ok(Some("d!"))
        );
        // substring("short", -2) -> "rt"
        // -2 + 5 = 3, so start at index 3
        test_substring_2!(Some(String::from("short")), Some(-2), Ok(Some("rt")));
        Ok(())
    }

    #[test]
    fn test_null_start_returns_null() -> Result<()> {
        test_substring_2!(Some(String::from("hello, world!")), None, Ok(None::<&str>));
        Ok(())
    }

    #[test]
    fn test_null_length_returns_null() -> Result<()> {
        test_substring_3!(
            Some(String::from("hello, world!")),
            Some(1),
            None,
    ...
    #[test]
    fn test_null_expression_returns_null() -> Result<()> {
        test_substring_3!(None::<String>, Some(2), Some(7), Ok(None::<&str>));
        Ok(())
    }

    #[test]
    fn test_negative_length_returns_empty_string() -> Result<()> {
        // Spark returns empty string for negative length
        test_substring_3!(
            Some(String::from("hello, world!")),
            Some(2),
            Some(-2),
    ...
    #[test]
    fn test_positive_positions() -> Result<()> {
        // 1-indexed positions
        test_substring_2!(
            Some(String::from("alphabet")),
            Some(1),
            Ok(Some("alphabet"))
        );
        test_substring_2!(Some(String::from("alphabet")), Some(2), Ok(Some("lphabet")));
        test_substring_2!(Some(String::from("alphabet")), Some(3), Ok(Some("phabet")));
        test_substring_3!(
            Some(String::from("alphabet")),
            Some(3),
            Some(2),
    ...
    #[test]
    fn test_position_zero() -> Result<()> {
        // Position 0 is treated as position 1 in Spark
        test_substring_2!(
            Some(String::from("alphabet")),
            Some(0),
            Ok(Some("alphabet"))
    ...
    #[test]
    fn test_position_beyond_length() -> Result<()> {
        test_substring_2!(Some(String::from("alphabet")), Some(30), Ok(Some("")));
        Ok(())
    }

    #[test]
    fn test_unicode_strings() -> Result<()> {
        test_substring_2!(Some(String::from("joséésoj")), Some(5), Ok(Some("ésoj")));
        test_substring_3!(
            Some(String::from("joséésoj")),
            Some(5),
            Some(2),
            Ok(Some("és"))
        );
