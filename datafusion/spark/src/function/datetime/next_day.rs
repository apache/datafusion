...
///
/// Returns `None` when `days` is not a representable date, and also when the
/// result would fall past `chrono::NaiveDate::MAX`. Spark computes the answer
/// with plain `Int` arithmetic and has no calendar range limit, so it keeps
/// returning values in that far-future range where this returns NULL.
...
mod tests {
    ...
    #[test]
    fn next_day_returns_null_past_the_last_representable_date() {
        // `chrono::NaiveDate::MAX` is +262142-12-31, which is a Monday.
        let max = 95026236;
        assert_eq!(next_date_for_day_of_week(max, Weekday::Mon), None);
        ...
        // so it overflows from six days earlier.
        assert_eq!(next_date_for_day_of_week(max - 6, Weekday::Tue), None);
        assert_eq!(next_date_for_day_of_week(max - 6, Weekday::Mon), Some(max));
        // Values that are not representable dates at all still return NULL.
