...
use datafusion_expr::{binary::comparison_coercion_numeric, ColumnarValue, ScalarFunctionArgs, ScalarUDFImpl, Signature, Volatility};
...
impl ScalarUDFImpl for SparkIf {
    ...
    fn coerce_types(&self, arg_types: &[DataType]) -> Result<Vec<DataType>> {
        ...
        let Some(target_type) = comparison_coercion_numeric(&arg_types[1], &arg_types[2]) else {
            return plan_err!("For function 'if' {} and {} is not comparable", arg_types[1], arg_types[2]);
        };
        // Convert null to String type.
        if target_type.is_null() {
            Ok(vec![DataType::Boolean, DataType::Utf8View, DataType::Utf8View])
        } else {
...
mod tests {
    ...
    fn test_if_basic() {
        ...
        let result = if_udf.invoke_with_args(ScalarFunctionArgs {
            args,
            arg_fields,
            number_rows: 4,
            return_field: arrow::datatypes::Field::new("result", DataType::Int32, true).into(),
        }).unwrap();
    ...
    fn test_if_with_nulls() {
        ...
        let condition = BooleanArray::from(vec![Some(true), None, Some(false), Some(true)]);
        let true_value = Int32Array::from(vec![10, 20, 30, 40]);
        let false_value = Int32Array::from(vec![100, 200, 300, 400]);
        ...
        assert_eq!(result_int32.value(0), 10);   // true -> 10
        assert!(result_int32.is_null(1));        // NULL -> NULL
    ...
    fn test_if_string_types() {
        ...
        let true_value = StringArray::from(vec!["yes", "maybe", "yes"]);
        ...
        let result = if_udf.invoke_with_args(ScalarFunctionArgs {
            ...
            arg_fields,
            number_rows: 3,
            return_field: arrow::datatypes::Field::new("result", DataType::Utf8, true).into(),
        }).unwrap();
    ...
    fn test_if_float_types() {
        ...
        let result = if_udf.invoke_with_args(ScalarFunctionArgs {
            ...
            arg_fields,
            number_rows: 4,
            return_field: arrow::datatypes::Field::new("result", DataType::Float64, true).into(),
        }).unwrap();
        
        let result_array = result.into_array(4).unwrap();
        let result_float = result_array.as_any().downcast_ref::<Float64Array>().unwrap();
    ...
    fn test_if_type_coercion() {
        ...
        // Test type coercion between Int32 and Int64
        let condition = BooleanArray::from(vec![true, false]);
        let true_value = Int32Array::from(vec![10, 20]);
        let false_value = Int64Array::from(vec![100, 200]);
        ...
        let result = if_udf.invoke_with_args(ScalarFunctionArgs {
            ...
            arg_fields,
            number_rows: 2,
            return_field: arrow::datatypes::Field::new("result", DataType::Int64, true).into(),
        }).unwrap();
        
        let result_array = result.into_array(2).unwrap();
        let result_int64 = result_array.as_any().downcast_ref::<Int64Array>().unwrap();
        
        assert_eq!(result_int64.value(0), 10);   // true -> 10 (coerced to Int64)
        assert_eq!(result_int64.value(1), 200);  // false -> 200
    }
    ...
    fn test_if_all_true() {
        ...
        let condition = BooleanArray::from(vec![true, true, true]);
        let true_value = Int32Array::from(vec![1, 2, 3]);
        let false_value = Int32Array::from(vec![100, 200, 300]);
        ...
        assert_eq!(result_int32.value(1), 2);
        assert_eq!(result_int32.value(2), 3);
    }

    #[test]
    fn test_if_all_false() {
        let if_udf = SparkIf::default();
        
        // Test when all conditions are false
        let condition = BooleanArray::from(vec![false, false, false]);
        let true_value = Int32Array::from(vec![1, 2, 3]);
        let false_value = Int32Array::from(vec![100, 200, 300]);
        ...
        assert_eq!(result_int32.value(0), 100);
        assert_eq!(result_int32.value(1), 200);
        assert_eq!(result_int32.value(2), 300);
    }

    #[test]
    fn test_if_empty_arrays() {
        let if_udf = SparkIf::default();
        
        // Test with empty arrays
        let condition = BooleanArray::from(Vec::<bool>::new());
        let true_value = Int32Array::from(Vec::<i32>::new());
        let false_value = Int32Array::from(Vec::<i32>::new());
        ...
        let result_array = result.into_array(0).unwrap();
        let result_int32 = result_array.as_any().downcast_ref::<Int32Array>().unwrap();
        
        assert_eq!(result_int32.len(), 0);
    }
    ...
    fn test_if_wrong_argument_count() {
        ...
        let arg_types = vec![DataType::Boolean, DataType::Int32];
        let result = if_udf.coerce_types(&arg_types);
        
        assert!(result.is_err());
        assert!(result.unwrap_err().message().contains("expects 3 arguments"));
    }

    #[test]
    fn test_if_incompatible_types() {
        let if_udf = SparkIf::default();
        
        // Test with incompatible types (Boolean and String)
        let arg_types = vec![DataType::Boolean, DataType::Boolean, DataType::Utf8];
        let result = if_udf.coerce_types(&arg_types);
        
        // This should work as Boolean and String can be coerced
        assert!(result.is_ok());
    }
}
