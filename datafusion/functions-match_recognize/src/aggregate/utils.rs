...
pub fn bits_col_from_expr(expr: &Expr) -> Option<Expr> {
    use datafusion_expr::utils::find_symbol_predicates;
    let symbols = find_symbol_predicates(expr);
    symbols.first().map(|s| col(classifier_bits_col_name(s)))
}
