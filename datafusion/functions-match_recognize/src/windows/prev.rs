...
impl Default for MatchRecognizePrev {
    fn default() -> Self {
        Self::new()
    }
}
...
static MR_PREV_DOCUMENTATION: LazyLock<Documentation> = LazyLock::new(|| {
    ...
    .with_argument("offset", "Non-negative integer. Rows back to look. Defaults to 1.")
...
impl WindowUDFImpl for MatchRecognizePrev {
    ...
    fn partition_evaluator(
        ...
        // New calling convention: prev(value, [offset, [default, [classifier_mask]]])
        ...
        if shift_offset < 0 {
            return datafusion_common::exec_err!("Offset for PREV must be non-negative");
        }
...
pub fn mr_prev(
    ...
    Ok(match bits_col_expr_from_expr(&arg)? {
        Some(mask) => mr_prev_udwf().call(vec![arg, offset_lit, default_lit, mask]),
        None => mr_prev_udwf().call(vec![arg, offset_lit, default_lit]),
    })
}
