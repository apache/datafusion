...
use crate::binary_map::{INITIAL_BUFFER_CAPACITY, INITIAL_MAP_CAPACITY, SHORT_VALUE_LEN};
...
where
    O: OffsetSizeTrait,
{
    pub fn new(_array: &ArrayRef, output_type: OutputType) -> Self {
        Self {
            output_type,
        ...
        B: ByteArrayType,
    {
        ...
        //     let existing_elem = unsafe { self.buffer.as_slice().get_unchecked(l..r) };
        //     let existing_inline = existing_elem.iter().fold(0usize, |acc, &x| acc << 8 | x as usize);
