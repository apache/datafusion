...
async fn parquet_distinct_partition_col() -> Result<()> {
    ...
    max_limit += 1;
    let last_batch = results.last().expect("There shouled be at least one record batch returned");
    let last_row_idx = last_batch.num_rows() - 1;
    let mut min_limit = match ScalarValue::try_from_array(last_batch.column(0), last_row_idx)?{
        ScalarValue::UInt64(Some(count)) => count,
        s => panic!("Expected count as Int64 found {}", s),
    };
    ...
    let sql_on_partition_boundary = format!("SELECT month from t where month = '{}' LIMIT {}", month, max_limit - 1);
    let resulting_limit: u64 = ctx.sql(sql_on_partition_boundary.as_str())
        .await?
        .collect()
        .await?
        .into_iter().map(|r| r.num_rows() as u64)
        .sum();
