...
impl RowGroupAccessPlanFilter {
    ...
    fn identify_fully_matched_row_groups(
        ...
        // TODO(QPIERRE): inject from upstream
        let Ok(inverted_predicate) =
            PruningPredicate::try_new(inverted_expr, Arc::clone(predicate.schema()), &PruningPredicateConfig::default())
        else {
            return;
        };
...
mod tests {
    ...
    fn row_group_pruning_predicate_simple_expr() {
        ...
        let pruning_predicate = PruningPredicate::try_new(expr, schema.clone(), &PruningPredicateConfig::default()).unwrap();
    ...
    fn row_group_pruning_predicate_file_schema() {
        ...
        let pruning_predicate =
            PruningPredicate::try_new(expr, table_schema.clone(), &PruningPredicateConfig::default()).unwrap();
    ...
    async fn test_row_group_bloom_filter_pruning_predicate_sql_in() {
        ...
        let expr = logical2physical(&expr, &schema);
        let pruning_predicate =
            PruningPredicate::try_new(expr, Arc::new(schema), &PruningPredicateConfig::default()).unwrap();
