...
use datafusion_datasource::file_format::{FileFormat, FileFormatFactory, FileMeta};
...
impl FileFormat for ParquetFormat {
    ...
    async fn infer_ordering(
        ...
        let metadata = DFParquetMetadata::new(store, object)
            ...
            .fetch_metadata()
            .await?;
        DFParquetMetadata::ordering_from_parquet_metadata(&metadata, &table_schema)
    }
    ...
    async fn infer_stats_and_ordering(
        ...
        let file_metadata_cache =
            state.runtime_env().cache_manager.get_file_metadata_cache();
        let (statistics, ordering) = DFParquetMetadata::new(store, object)
            .with_metadata_size_hint(self.metadata_size_hint())
            .with_decryption_properties(file_decryption_properties)
            .with_file_metadata_cache(Some(file_metadata_cache))
            .fetch_statistics_and_ordering(&table_schema)
            .await?;
