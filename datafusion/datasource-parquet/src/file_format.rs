...
/// - Min/max values are properly extracted and represented as Precision::Exact or Precision::Inexact depending on the `is_max_value_exact` and `is_min_value_exact` flags.
    ...
    (0..schema.fields().len())
        .map(|i| {
            let max_value = match (
                ...
                (Some(max_value), Some(true)) | (Some(max_value), None) => {
                    max_value.evaluate().ok().map(Precision::Exact)
                }
                (Some(max_value), Some(false)) => {
            ...
            let min_value = match (
                ...
                (Some(min_value), Some(true)) | (Some(min_value), None) => {
                    min_value.evaluate().ok().map(Precision::Exact)
                }
                (Some(min_value), Some(false)) => {
...
fn summarize_min_max_null_counts(
    ...
    if let Some(max_acc) = &mut max_accs[arrow_schema_index] {
        ...
        is_max_value_exact[arrow_schema_index] = get_exactness(
            // Safety: `MaxAccumulator::evaluate()` always returns an Ok.
    ...
    if let Some(min_acc) = &mut min_accs[arrow_schema_index] {
        ...
        is_min_value_exact[arrow_schema_index] = get_exactness(
            // Safety: `MinAccumulator::evaluate()` always returns an Ok.
...
fn get_exactness(
    ...
    value.to_scalar().ok().and_then(|value| {
        eq(&value, &values)
            .ok()
            .and_then(|mask| and(&mask, &exactness).ok())
            .map(|result| result.iter().filter(|&x| x == Some(true)).count() > 0)
    })
}
