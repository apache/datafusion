...
impl RowFilterContext {
    ...
    /// Returns the union of the [`ProjectionMask`]s of all prebuilt filter
    /// candidates: the parquet leaf columns read to evaluate the row filter.
    /// Used by the opener to compute one-shot I/O ranges when
    /// [`progressive_io`](datafusion_common::config::ParquetOptions::progressive_io)
    /// is disabled. `None` when the candidate list is empty.
    pub(crate) fn filter_mask(&self) -> Option<ProjectionMask> {
        crate::row_filter::prebuilt_filter_mask(self.prebuilt.as_slice())
    }
}
