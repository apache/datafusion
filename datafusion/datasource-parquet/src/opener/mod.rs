...
use crate::push_decoder::{
    ...
    RowGroupPruner, one_shot_row_group_ranges,
};
...
impl RowGroupsPrunedParquetOpen {
    ...
    fn build_stream(self) -> Result<BoxStream<'static, Result<RecordBatch>>> {
        ...
        let (
            ...
            //
            // The filter mask covers every prebuilt filter candidate even
            // when the first row group is fully matched (empty filter
            // installed): later row groups may install the real filter, and
            // over-covering ranges are harmless.
            ...
            let filter_mask = row_filter_context
                .as_ref()
                .and_then(|ctx| ctx.filter_mask());
