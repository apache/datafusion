...
mod test {
    ...
    mod filter_placement {
        ...
        /// pass most rows. With this prior, the conjunct stays in the
        /// post-scan filter. Without statistics pruning there is no prior:
        /// the conjunct removes a long run of rows in the second row group
        /// and moves to the row filter.
        #[tokio::test]
        async fn statistics_prior_keeps_clustered_filter_post_scan() {
            let store = Arc::new(InMemory::new()) as Arc<dyn ObjectStore>;
            let (schema, file) = write_file(&store, |i| (i / 1000) as i32).await;
