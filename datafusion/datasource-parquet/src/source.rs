...
impl ParquetSource {
    ...
    /// Set the legacy `reorder_filters` config flag.
    ///
    /// The adaptive selectivity tracker subsumes static filter reordering
    /// (filters are now ranked online by measured bytes-saved/sec), so the
    /// flag is preserved for backwards compatibility but no longer has any
    /// effect on filter placement.
    #[deprecated(
        since = "53.2.0",
        note = "the adaptive selectivity tracker reorders filters by measured \
                effectiveness; this flag is now a no-op"
    )]
    ...
    /// Return the value of the legacy `reorder_filters` config flag. Now a
    /// no-op — see [`Self::with_reorder_filters`].
    #[deprecated(
        since = "53.2.0",
        note = "the adaptive selectivity tracker reorders filters by measured \
                effectiveness; this flag is now a no-op"
