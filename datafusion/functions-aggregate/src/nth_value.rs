...
impl AggregateUDFImpl for NthValueAgg {
    ...
    fn state_fields(
        ...
        let mut fields = vec![Field::new_list(
            format_state_name(args.name, "nth_value"),
            Field::new("item", args.input_type.clone(), true),
            true,
        )];
        if !args.ordering_fields.is_empty() {
            fields.push(Field::new_list(
                format_state_name(args.name, "nth_value_orderings"),
                Field::new(
                    "item",
    ...
    fn accumulator(&self, acc_args: AccumulatorArgs) -> Result<Box<dyn Accumulator>> {
        let n = match acc_args.input_exprs[1] {
            ...
            _ => return exec_err!("Second argument of NTH_VALUE needs to be a literal"),
        };
