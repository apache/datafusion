...
use datafusion_expr::blocked_helpers::{BlockedVecBuilder, CopyItemBlockedVecBuilder};
...
impl MinMaxBytesBlockedAccumulator {
...
impl BlockedGroupsAccumulator for MinMaxBytesBlockedAccumulator {
...
impl BlockedMinMaxBytesState {
    ...
    fn new(data_type: DataType, block_size: usize) -> Self {
        Self {
            min_max: BlockedVecBuilder::new(block_size),
            data_type,
            total_data_bytes: 0,
        }
    }
    ...
    fn emit(&mut self, emit_to: BlockedEmitTo) -> Vec<(usize, Vec<Option<Vec<u8>>>)> {
        match emit_to {
            BlockedEmitTo::All => {
                let blocks = self
                  .min_max
            ...
            BlockedEmitTo::NextBlock => {
                ...
                let first_data_capacity: usize = next_block
                  ...
                  .sum();
                self.total_data_bytes -= first_data_capacity;
                vec![(first_data_capacity, next_block)]
            }
            BlockedEmitTo::First(n) => {
                let next_block = self
                  .min_max
                  .take_n(n, None::<std::iter::Empty<_>>);
    ...
    fn size(&self) -> usize {
        self.total_data_bytes + self.min_max.allocated_size()
    }
}
