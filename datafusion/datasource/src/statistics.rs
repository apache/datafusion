...
use datafusion_common::stats::{add_row_stats, set_max_if_greater, set_min_if_lesser, Precision};
...
pub fn compute_file_group_statistics(
    ...
    let statistics_iter = file_group
        .iter()
        .filter_map(|file| file.statistics.as_ref().map(|stats| stats.as_ref()));
    let statistics = Statistics::merge_iter(statistics_iter, &file_schema);
...
pub fn compute_all_files_statistics(
    ...
    let statistics_iter = file_groups_with_stats
        .iter()
        .filter_map(|file_group| file_group.statistics());

    let mut statistics = Statistics::merge_iter(statistics_iter, &file_schema);
