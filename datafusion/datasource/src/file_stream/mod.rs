...
/// Build a default shared state object for one file scan.
pub(crate) fn new_shared_file_stream_state(
    num_partitions: usize,
    preserve_order: bool,
) -> SharedFileStreamState {
    SharedFileStreamState::new(
        target_datasource_outstanding_ios(num_partitions),
        if preserve_order {
...
pub(crate) fn shared_file_stream_state_for(
    ...
    new_shared_file_stream_state(config.file_groups.len(), config.preserve_order)
}
...
pub struct FileStream {
    ...
    ///
    /// If false, ready work may migrate to a sibling `FileStream`.
    ///
    /// If true, ready work must remain in the same output partition that
    /// discovered it, though it may still be reordered within that partition.
    ...
    /// stream has transitioned to the done-producing state.
    done_producing_marked: bool,
        ...
        loop {
            ...
            while self.queues.morsel_len() < max_buffered_morsels() {
                ...
                let Some(io_permit) = self
                    .shared_file_stream_state
                    .try_acquire_io_permit(self.stream_id)
                else {
                    self.shared_file_stream_state
