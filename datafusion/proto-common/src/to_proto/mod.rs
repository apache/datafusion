...
impl TryFrom<&ParquetOptions> for protobuf::ParquetOptions {
    ...
    fn try_from(value: &ParquetOptions) -> datafusion_common::Result<Self, Self::Error> {
        Ok(protobuf::ParquetOptions {
            ...
            content_defined_chunking: value.enable_content_defined_chunking,
            cdc_min_chunk_size_opt: value.enable_content_defined_chunking.then(||
                protobuf::parquet_options::CdcMinChunkSizeOpt::CdcMinChunkSize(value.cdc_min_chunk_size as u64)
            ),
            cdc_max_chunk_size_opt: value.enable_content_defined_chunking.then(||
                protobuf::parquet_options::CdcMaxChunkSizeOpt::CdcMaxChunkSize(value.cdc_max_chunk_size as u64)
            ),
            cdc_norm_level_opt: value.enable_content_defined_chunking.then(||
                protobuf::parquet_options::CdcNormLevelOpt::CdcNormLevel(value.cdc_norm_level as i32)
            ),
        })
    }
}
