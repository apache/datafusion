...
unsafe extern "C" fn fmt_sql_fn_wrapper(expr: &FFI_PhysicalExpr) -> FFIResult<SString> {
    ...
    FfiResult::Ok(SString::from(result.as_str()))
}
...
unsafe extern "C" fn display_fn_wrapper(expr: &FFI_PhysicalExpr) -> SString {
    let expr = expr.inner();
    SString::from(format!("{expr}").as_str())
}
...
impl PhysicalExpr for ForeignPhysicalExpr {
    ...
    fn with_new_children(
        ...
        unsafe {
            ...
            df_result!(
                (self.expr.new_with_children)(&self.expr, &children)
                    .into_result()
                    .map(|expr| <Arc<dyn PhysicalExpr>>::from(&expr))
            )
        }
    ...
    fn propagate_constraints(
        ...
        unsafe {
            ...
            let result: Option<SVec<FFI_Interval>> = result.into();
            let result = result.map(|intervals| {
                intervals
                    .into_iter()
                    .map(Interval::try_from)
                    .collect::<Result<Vec<_>>>()
    ...
    fn propagate_statistics(
        ...
        unsafe {
            ...
            let result: Option<SVec<FFI_Distribution>> = result.into();
            let result: Option<Result<Vec<Distribution>>> = result.map(|dists| {
                dists
                    .into_iter()
                    .map(Distribution::try_from)
                    .collect::<Result<Vec<_>>>()
    ...
    fn fmt_sql(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        unsafe {
            match Into::<std::result::Result<_, _>>::into((self.expr.fmt_sql)(&self.expr))
            {
                Ok(sql) => write!(f, "{sql}"),
    ...
    fn snapshot(&self) -> Result<Option<Arc<dyn PhysicalExpr>>> {
        unsafe {
            ...
            let result: Option<FFI_PhysicalExpr> = result.into();
            Ok(result.map(|expr| <Arc<dyn PhysicalExpr>>::from(&expr)))
        }
    }
