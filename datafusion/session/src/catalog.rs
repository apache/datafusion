...
pub struct EmptyCatalogProviderList;
...
#[cfg(test)]
mod tests {
    use super::{CatalogProviderList, EmptyCatalogProviderList};

    #[test]
    fn empty_catalog_provider_list_has_no_catalogs() {
        let catalogs = EmptyCatalogProviderList;
        assert!(catalogs.catalog_names().is_empty());
        assert!(catalogs.catalog("missing").is_none());
    }
}
