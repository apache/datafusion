...
/// Create an tmplementation of `now()` that always returns the
/// specified timestamp.
///
...
pub fn make_now(now_ts: Option<DateTime<Utc>>) -> impl Fn( &[ColumnarValue]) -> Result<ColumnarValue>
{
    let now_ts = now_ts.map(|t| t.timestamp_nanos());
    move |_arg| {
        Ok(ColumnarValue::Scalar(ScalarValue::TimestampNanosecond(now_ts)))
    }
}
