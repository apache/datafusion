...
mod tests {
    ...
    async fn partitioned_join_collect(
        ...
        for i in 0..partition_count {
            let task_ctx = Arc::new(TaskContext::from(session_ctx));
            let stream = join.execute(i, task_ctx).await?;
    ...
    async fn join_inner_one_no_shared_column_names() -> Result<()> {
        ...
        let (columns, batches) =
            join_collect(left, right, on, &JoinType::Inner, false, &session_ctx).await?;
    ...
    async fn join_inner_one_two_parts_right() -> Result<()> {
        ...
        // second part
        let task_ctx = Arc::new(TaskContext::from(&session_ctx));
        let stream = join.execute(1, task_ctx).await?;
        let batches = common::collect(stream).await?;
        assert_eq!(batches.len(), 1);
        let expected = vec![
            "+----+----+----+----+----+----+",
    ...
    async fn join_right_one() -> Result<()> {
        ...
        let (columns, batches) =
            join_collect(left, right, on, &JoinType::Right, false, &session_ctx).await?;
