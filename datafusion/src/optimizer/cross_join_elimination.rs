...
use crate::logical_plan::plan::{
    Aggregate, Analyze, Filter, Join, Projection, Sort, Window,
};
use crate::logical_plan::{
    Column, CrossJoin, DFSchema, Expr, JoinConstraint, JoinType, Limit, LogicalPlan,
    Repartition, Union,
};
        ...
        let p = match plan {
            LogicalPlan::Filter(Filter { predicate, input }) => {
                let new_plan = match input.as_ref() {
                    crate::logical_plan::LogicalPlan::CrossJoin(CrossJoin {
                        left,
                        right,
                        schema,
                    }) => {
                        let all_schemas = input.all_schemas();
                        find_join_keys(left, right, predicate, all_schemas).map(
                            |(new_filter_predicate, join_keys)| {
                                let new_input = LogicalPlan::Join(Join {
                                    left: left.clone(),
                                    right: right.clone(),
                                ...
                                LogicalPlan::Filter(Filter {
                                    predicate: new_filter_predicate,
            ...
            | LogicalPlan::Projection(Projection { input, .. }) => utils::from_plan(
                plan,
                &plan.expressions(),
                &[self.optimize(input, execution_props)?],
            )?,
...
fn find_join_and_replace_join_keys_rec(
    predicate: &mut Expr,
    output: &mut HashSet<(Column, Column)>,
    left_schema: &DFSchema,
    right_schema: &DFSchema,
) {
    match predicate {
        Expr::Alias(c, _) => {
            find_join_and_replace_join_keys_rec(c, output, left_schema, right_schema)
        }
        Expr::BinaryExpr { left, op, right } => match op {
            crate::logical_plan::Operator::Eq => match (left.as_ref(), right.as_ref()) {
                (Expr::Column(l), Expr::Column(r)) => {
                    let mut remove_eq = false;
                    if let (Ok(_), Ok(_)) = (
                        left_schema.index_of_column(l),
                        right_schema.index_of_column(r),
                    ) {
                        output.insert((l.clone(), r.clone()));
                        remove_eq = true;
                    } else if let (Ok(_), Ok(_)) = (
                        left_schema.index_of_column(r),
                        right_schema.index_of_column(l),
                    ) {
