...
fn try_create_group_join(
    ...
    // Group-by expressions must all resolve to left-side join columns
    for (expr, _alias) in group_by.expr() {
        if !expr_columns_in_side(expr.as_ref(), &*resolve_index, 0, left_field_count) {
            return Ok(None);
        }
    }
    ...
    // Remap group-by expressions to reference the left input schema directly
    let remapped_group_by: Vec<_> = group_by
        .expr()
        .iter()
        .map(|(expr, alias)| {
            let remapped = remap_columns(expr, &*resolve_index, 0);
