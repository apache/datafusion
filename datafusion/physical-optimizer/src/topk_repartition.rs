...
mod tests {
    ...
    fn scan_partitioned(partitions: usize) -> Arc<dyn ExecutionPlan> {
        let s = schema();
        Arc::new(PlaceholderRowExec::new(s).with_partitions(partitions))
    }
    ...
    fn topk_pushed_below_hash_repartition() {
        ...
        assert_eq!(
            display.trim(),
            r#"
SortExec: TopK(fetch=3), expr=[a@0 ASC, b@1 ASC], preserve_partitioning=[true], sort_prefix=[a@0 ASC, b@1 ASC]
