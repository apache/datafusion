...
fn replace_with_partial_sort(
    ...
    let Some(sort_plan) = plan.as_ref().downcast_ref::<SortExec>() else {
        return Ok(plan);
    };
...
fn analyze_immediate_sort_removal(
    ...
    let Some(sort_exec) = node.plan.as_ref().downcast_ref::<SortExec>() else {
...
fn adjust_window_sort_removal(
    ...
    let (window_expr, new_window) =
        if let Some(exec) = window_tree.plan.as_ref().downcast_ref::<WindowAggExec>() {
...
fn remove_bottleneck_in_subplan(
    ...
    if let Some(repartition) = new_reqs.plan.as_ref().downcast_ref::<RepartitionExec>() {
...
fn remove_corresponding_sort_from_sub_plan(
    ...
    if let Some(sort_exec) = node.plan.as_ref().downcast_ref::<SortExec>() {
        ...
        if is_sort_preserving_merge(&node.plan) {
            ...
            node.plan.as_ref().downcast_ref::<RepartitionExec>()
        {
...
fn get_sort_exprs(
    ...
    if let Some(sort_exec) = sort_any.as_ref().downcast_ref::<SortExec>() {
        Ok((sort_exec.expr(), sort_exec.fetch()))
    } else if let Some(spm) = sort_any.as_ref().downcast_ref::<SortPreservingMergeExec>()
    {
