                ...
                for (i, (left, right)) in join.on.iter().enumerate() {
                    ...
                    if let (Expr::Column(l), Expr::Column(_)) = (left, right) {
                        join_filter_pushdown_info.push_filter(
                            DynamicJoinFilterPushdownColumn::new(
                                i,
                                Arc::new(Expr::Column(l.clone())),
                            ),
                        );
                    }
                }
                    ...
                    match probe {
                        ...
                        LogicalPlan::Projection(project) => {
                            for filter in join_filter_pushdown_info.filters.iter() {
                                if let Expr::Column(column) = filter.column.as_ref() {
                                    if !project.schema.has_column(column) {
                                        return Ok(Transformed::no(LogicalPlan::Join(
                                            join,
                                        )));
                                    }
                                } else {
                                    // If the expression is not a column, we can't push down the filter
                                    return Ok(Transformed::no(LogicalPlan::Join(join)));
                                }
                            }
                            probe = &probe.inputs()[0];
