...
use datafusion_common::tree_node::{Transformed, TreeNode, TreeNodeMutator};
use log::{debug, warn};
...
/// Returns an error if the plan has a different schema than `prev_schema`
///
/// It ignores metadata and nullability.
pub(crate) fn assert_only_schema_is_the_same(
    rule_name: &str,
...
mod tests {
    ...
    fn generate_different_schema() {
        ...
        let err = opt.optimize(plan, &config, &observe).unwrap_err();
        assert_eq!("Optimizer rule 'get table_scan rule' failed\n\
            caused by\nget table_scan rule\ncaused by\n\
            Internal error: Failed due to a difference in schemas, \
            original schema: DFSchema { inner: Schema { \
            fields: [], \
            metadata: {} }, field_qualifiers: [], functional_dependencies: FunctionalDependencies { deps: [] } }, \
            new schema: \
            DFSchema { inner: Schema { \
            fields: [\
