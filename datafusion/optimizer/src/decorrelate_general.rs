...
#[derive(Debug)]
pub struct GeneralPullUpCorrelatedExpr {
...
impl Default for GeneralPullUpCorrelatedExpr {
    fn default() -> Self {
        Self::new()
    }
}

impl GeneralPullUpCorrelatedExpr {
    pub fn new() -> Self {
        Self {
            join_filters: vec![],
...
impl TreeNodeRewriter for GeneralPullUpCorrelatedExpr {
    ...
    fn f_down(&mut self, plan: LogicalPlan) -> Result<Transformed<LogicalPlan>> {
        match plan {
            ...
            LogicalPlan::Union(_) | LogicalPlan::Sort(_) | LogicalPlan::Extension(_) => {
                ...
                println!("plan hold outer and contains union");
                if plan_hold_outer {
            ...
            LogicalPlan::Limit(_) => {
                ...
                match (self.exists_sub_query, plan_hold_outer) {
                    (false, true) => {
                        // the unsupported case
                        println!("plan has limit and no subquery found and plan hold outer ref");
                        self.can_pull_up = false;
                        Ok(Transformed::new(plan, false, TreeNodeRecursion::Jump))
                    }
            ...
            _ if plan.contains_outer_reference() => {
                println!("plan contains outer reference, cannot pull up");
    ...
    fn f_up(&mut self, plan: LogicalPlan) -> Result<Transformed<LogicalPlan>> {
        ...
        println!("XXXXXXXXXXXXX Plan type {}", plan.display());
        match &plan {
            // TODO: what if this happen recursively?
            // select * from exams e1 where grade > (select avg(grade) from exams as e2 where e1.sid = e2.sid
            ...
            LogicalPlan::Filter(plan_filter) => {
                ...
                let or_filters = split_disjunction(&plan_filter.predicate);
                or_filters.iter().for_each(|f| {
                    println!("or filter {}", f);
                });
                self.can_pull_over_aggregation = self.can_pull_over_aggregation
                    && subquery_filter_exprs
                        ...
                        .all(|&e| {
                            let ret = can_pullup_over_aggregation(e);
                            if !ret {
                                println!("can NOT pull up over aggregation {:?}", e);
                            }
                            ret
                        });
                ...
                println!("JOIN FILTERS");
                for expr in join_filters.iter() {
                    println!("{}", expr);
                }

                // TODO: these cols only include the inner's table columns which is not sufficient
                // in the case of complex unnest
                //
                // We need to collect all the columns in the outer table to construct the domain
                // and join the domain with the inner table to prepare for the aggregation
                let correlated_subquery_cols =
                    collect_subquery_cols(&join_filters, subquery_schema)?;
                println!("CORRELATED COLUMS");
                for col in correlated_subquery_cols.iter() {
                    println!("{}", col);
                }
                // TODO: these join filters may need to be transformed, because now the join
                // happen between the outer table columns and the newly built relation
                ...
                if self.in_predicate_opt.is_some() || !self.join_filters.is_empty() =>
            {
                ...
                if !is_distinct {
                    println!(
                        "can pull up {:?} and can pull over aggregation {:?}",
                        self.can_pull_up, self.can_pull_over_aggregation
                    );
...
/// for now only simple exprs can be pulled up over aggregation
/// such as binaryExpr between a outer column ref vs non column expr
/// In the general unnesting framework, the complex expr is pulled up, but being decomposed in some way
/// for example:
...
/// In this case, the complex expr to be pulled up is
/// ```
/// e1.student_id=e1.student_id or (e2.year > e1.year and e2.subject=e1.subject)
/// ```
/// The complex expr is decomposed during the pull up over aggregation avg(score)
/// into a new relation
