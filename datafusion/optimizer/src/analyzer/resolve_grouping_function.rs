...
fn grouping_function_on_id(
    ...
    // always 0
    if !is_grouping_set {
        return Ok(Expr::from(ScalarValue::from(0i32)));
    }

    let group_by_expr_count = group_by_expr.len();
    let literal = |value: usize| {
        if group_by_expr_count < 8 {
            Expr::from(ScalarValue::from(value as u8))
        } else if group_by_expr_count < 16 {
            Expr::from(ScalarValue::from(value as u16))
        } else if group_by_expr_count < 32 {
            Expr::from(ScalarValue::from(value as u32))
        } else {
            Expr::from(ScalarValue::from(value as u64))
        }
    };
