...
fn fold_case_branch(
    ...
    match const_evaluator.evaluate_to_scalar(call) {
        ConstSimplifyResult::Simplified(s, m)
        | ConstSimplifyResult::NotSimplified(s, m) => Ok(Some(Expr::Literal(s, m))),
        ...
        Ok(match expr {
            ...
            Expr::ScalarFunction(ScalarFunction { func, args })
                if func.signature().volatility == Volatility::Immutable
                    && case_pushdown_target(&args).is_some() =>
            {
                // The function's own simplify() gets the first shot (arrow_cast
                // must rewrite itself into a Cast); push down only if it declines.
                ...
                let Some(case_position) = case_pushdown_target(&args) else {
                    return internal_err!(
                        "case pushdown guard guaranteed a CASE argument"
                    );
                };
                let Expr::Case(case) = &args[case_position] else {
                    return internal_err!(
                        "case pushdown guard guaranteed a CASE argument"
                    );
                };
