...
mod tests {
    ...
    fn expr_is_restrict_null_predicate() -> Result<()> {
        let test_cases = vec![
            ...
            // a = NULL
            (
                binary_expr(col("a"), Operator::Eq, Expr::from(ScalarValue::Null)),
                true,
            ),
            // a > 8
            ...
            // a IN (NULL)
            (
                in_list(col("a"), vec![Expr::from(ScalarValue::Null)], false),
                true,
            ),
            // a NOT IN (NULL)
            (
                in_list(col("a"), vec![Expr::from(ScalarValue::Null)], true),
                true,
            ),
        ];
