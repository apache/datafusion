...
impl OptimizerRule for EliminateLimit {
    fn optimize(
        ...
        match plan {
            LogicalPlan::Limit(limit) => match limit.fetch {
                Some(fetch) => {
                    if fetch == 0 {
...
mod tests {
    ...
    fn assert_optimized_plan_eq(plan: &LogicalPlan, expected: &str) -> Result<()> {
        let _optimized_plan = EliminateLimit::new()
            .optimize(plan, &mut OptimizerConfig::new())
            .expect("failed to optimize plan");
        let formatted_plan = format!("{:?}", _optimized_plan);
        assert_eq!(formatted_plan, expected);
        assert_eq!(plan.schema(), _optimized_plan.schema());
        Ok(())
    }
    ...
    fn assert_optimized_plan_eq_with_pushdown(
        ...
        let mut _optimized_plan = EliminateLimit::new()
            .optimize(plan, &mut OptimizerConfig::new())
            .expect("failed to optimize plan");
        _optimized_plan = LimitPushDown::new()
            .optimize(plan, &mut OptimizerConfig::new())
            .expect("failed to optimize plan");
        let formatted_plan = format!("{:?}", _optimized_plan);
        assert_eq!(formatted_plan, expected);
