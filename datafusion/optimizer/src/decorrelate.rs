...
fn agg_exprs_evaluation_result_on_empty_batch(
    ...
    for e in agg_expr.iter() {
            ...
            .transform_up(|expr| {
                let new_expr = match expr {
                    Expr::AggregateFunction(expr::AggregateFunction { func, .. }) => {
                        if func.name() == "count" {
                            Transformed::yes(Expr::Literal(LogicalScalar::Int64(Some(0))))
                        } else {
                            Transformed::yes(Expr::Literal(LogicalScalar::Null))
                        }
                    }
        ...
        if matches!(result_expr, Expr::Literal(LogicalScalar::Int64(_))) {
...
fn filter_exprs_evaluation_result_on_empty_batch(
    ...
    let pull_up_expr = if result_expr.ne(filter_expr) {
        ...
        match &result_expr {
            ...
            | Expr::Literal(LogicalScalar::Boolean(Some(false))) => None,
            // evaluate to true on empty batch, need to pull up the expr
            Expr::Literal(LogicalScalar::Boolean(Some(true))) => {
            ...
            _ => {
                for input_expr in input_expr_result_map_for_count_bug.values() {
                    let new_expr = Expr::Case(expr::Case {
                        ...
                        else_expr: Some(Box::new(Expr::Literal(LogicalScalar::Null))),
                    });
