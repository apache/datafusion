    ...
    fn f_down(&mut self, node: LogicalPlan) -> Result<Transformed<LogicalPlan>> {
        ...
        match &node {
            LogicalPlan::Filter(f) => {
                ...
                // If there exists any predicates without subquery, we create a new filter children node including
                ...
                if is_dependent_join_node && !predicates_without_subquery.is_empty() {
                    ...
                    return Ok(Transformed::yes(new_plan_to_traverse));
                }
...
// recursively traverse the given expression and append the subquery types to the given queue
...
mod tests {
    ...
    #[test]
    fn correlated_any_all_1() -> Result<()> {
    ...
    #[test]
    fn simple_correlated_agg_subquery() -> Result<()> {
    ...
    #[test]
    fn simple_subquery_in_agg() -> Result<()> {
        // CREATE TABLE t(a INT, b INT);
        // SELECT a,
        //     SUM(
    ...
    #[test]
    fn two_exists_subqueries_with_or_filter() -> Result<()> {
    ...
    #[test]
    fn any_subquery_with_derived_join() -> Result<()> {
        // SQL equivalent:
