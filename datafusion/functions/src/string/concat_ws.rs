...
impl ScalarUDFImpl for ConcatWsFunc {
    ...
    fn invoke_batch(
        ...
        if array_len.is_none() {
            ...
            let sep = match scalar.value().try_as_str() {
                Some(Some(s)) => s,
                Some(None) => {
                    // null literal string
                ...
                match delimiter {
                    ...
                    Some(delimiter) => {
                        ...
                        for arg in args {
                            ...
                            s => return internal_err!("The scalar {s} should be casted to string type during the type coercion."),
                                }
                    ...
                    None => Ok(ExprSimplifyResult::Simplified(Expr::from(
                        ScalarValue::Utf8(None),
                    ))),
                }
            }
...
fn is_null(expr: &Expr) -> bool {
    match expr {
        Expr::Literal(v) => v.value().is_null(),
        _ => false,
    }
}
