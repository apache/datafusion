    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        let result: ArrayRef = match value.data_type() {
            ...
            DataType::Decimal128(_, scale) => {
                apply_decimal_op(value, *scale, floor_decimal_value)?
            }
            other => {
                return exec_err!(
                    "Unsupported data type {other:?} for function {}",
...
fn decimal_scale_factor(scale: i8) -> Result<i128> {
    ...
    if let Some(value) = 10_i128.checked_pow(exponent) {
        ...
        exec_err!("Decimal scale {scale} is too large for floor")
    }
}

fn floor_decimal_value(
    value: i128,
    factor: i128,
    ...
    if value >= 0 {
        value.checked_sub(remainder).ok_or_else(|| {
            ArrowError::ComputeError("Decimal128 overflow while applying floor".into())
        })
    } else {
        let adjustment = factor + remainder;
        value.checked_sub(adjustment).ok_or_else(|| {
            ArrowError::ComputeError("Decimal128 overflow while applying floor".into())
        })
    }
}
...
mod tests {
    ...
    fn test_decimal128_floor() {
        ...
        let result = FloorFunc::new()
            .invoke_with_args(args)
            .expect("floor evaluation succeeded");
        ...
        assert_eq!(values.value(0), 1200);
        assert_eq!(values.value(1), -1300);
    ...
    #[test]
    fn test_decimal128_floor_zero_scale() {
