...
mod tests {
    ...
    fn test_case_sensitive_regexp_count_scalar() {
        ...
        values.iter().enumerate().for_each(|(pos, &v)| {
            ...
            let expected = ScalarValue::Int64(expected.get(pos).cloned());
            #[allow(deprecated)] // TODO: migrate to invoke_with_args
            let re = RegexpCountFunc::new().invoke_batch(
                &[ColumnarValue::from(v_sv), ColumnarValue::from(regex_sv)],
                1,
            );
            match re {
                Ok(ColumnarValue::Scalar(scalar)) => {
                    assert_eq!(
                        scalar.value(),
                        &expected,
                        "regexp_count scalar test failed"
                    );
                }
    ...
    fn test_case_sensitive_regexp_count_scalar_start() {
        ...
        values.iter().enumerate().for_each(|(pos, &v)| {
            ...
            let re = RegexpCountFunc::new().invoke_batch(
                &[
                    ColumnarValue::from(v_sv),
                    ColumnarValue::from(regex_sv),
                    ColumnarValue::from(start_sv.clone()),
                ],
                1,
            );
            match re {
        ...
        values.iter().enumerate().for_each(|(pos, &v)| {
            ...
            let re = RegexpCountFunc::new().invoke_batch(
                &[
                    ...
                    ColumnarValue::from(regex_sv),
                    ColumnarValue::from(start_sv.clone()),
                    ColumnarValue::from(flags_sv.clone()),
                ],
                1,
            );
            match re {
