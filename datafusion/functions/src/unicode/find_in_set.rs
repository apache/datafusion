...
impl ScalarUDFImpl for FindInSetFunc {
    ...
    fn invoke_with_args(&self, args: ScalarFunctionArgs) -> Result<ColumnarValue> {
        ...
        match (string, str_list) {
            ...
            (ColumnarValue::Scalar(string), ColumnarValue::Scalar(str_list)) => {
                invoke_with_args_scalar_scalar(string, str_list)
            }

            // `string` is an array, `str_list` is scalar
            (ColumnarValue::Array(str_array), ColumnarValue::Scalar(str_list)) => {
                invoke_with_args_array_scalar(str_array, str_list)
            }

            // `string` is scalar, `str_list` is an array
            (ColumnarValue::Scalar(scalar), ColumnarValue::Array(str_list_array)) => {
                invoke_with_args_scalar_array(scalar, str_list_array)
            }
...
/// Handles the `find_in_set` case for two scalar arguments.
fn invoke_with_args_scalar_scalar(
    string: Scalar,
    str_list: Scalar,
) -> Result<ColumnarValue> {
    let string = match string.into_value() {
        ScalarValue::Utf8View(string)
        | ScalarValue::Utf8(string)
        | ScalarValue::LargeUtf8(string) => string,
        _ => {
            return internal_err!(
                "Invalid argument type for 'str' in `find_in_set` function"
            )
        }
    };

    let str_list = match str_list.into_value() {
        ScalarValue::Utf8View(str_list)
        | ScalarValue::Utf8(str_list)
        | ScalarValue::LargeUtf8(str_list) => str_list,
        _ => {
            return internal_err!(
                "Invalid argument type for 'str_list' in `find_in_set` function"
            )
        }
    };
...
/// Handles the `find_in_set` case for an array and a scalar argument.
fn invoke_with_args_array_scalar(
    str_array: ArrayRef,
    str_list: Scalar,
) -> Result<ColumnarValue> {
    let str_list = match str_list.into_value() {
    ...
    let result_array = match str_list {
        ...
        Some(str_list_literal) => {
            ...
            let result = match str_array.data_type() {
                DataType::Utf8 => {
                    ...
                    find_in_set_right_literal::<Int32Type, _>(string_array, str_list)
                }
                DataType::LargeUtf8 => {
                    let string_array = str_array.as_string::<i64>();
                    find_in_set_right_literal::<Int64Type, _>(string_array, str_list)
                }
                DataType::Utf8View => {
...
/// Handles the `find_in_set` case for a scalar and an array argument.
fn invoke_with_args_scalar_array(
    string_literal: Scalar,
    str_list_array: ArrayRef,
) -> Result<ColumnarValue> {
    let string_literal = match string_literal.into_value() {
        ScalarValue::Utf8View(string_literal)
        | ScalarValue::Utf8(string_literal)
        | ScalarValue::LargeUtf8(string_literal) => string_literal,
        _ => {
            return internal_err!(
                "Invalid argument type for 'str' in `find_in_set` function"
            )
    ...
    let res = match string_literal {
        ...
        None => new_null_array(str_list_array.data_type(), str_list_array.len()),
        Some(string) => {
...
mod tests {
    ...
    fn test_functions() -> Result<()> {
        test_function!(
            FindInSetFunc::new(),
            vec![
                ColumnarValue::from(ScalarValue::Utf8(Some(String::from("a")))),
                ColumnarValue::from(ScalarValue::Utf8(Some(String::from("a,b,c")))),
            ],
            Ok(Some(1)),
            i32,
            Int32,
        ...
        test_function!(
            ...
            vec![
                ...
                ColumnarValue::from(ScalarValue::Utf8View(Some(String::from("a,Д,🔥")))),
            ],
            Ok(Some(3)),
            i32,
            Int32,
        ...
        test_function!(
            FindInSetFunc::new(),
            vec![
                ColumnarValue::from(ScalarValue::Utf8View(Some(String::from("d")))),
                ColumnarValue::from(ScalarValue::Utf8View(Some(String::from("a,b,c")))),
            ],
            Ok(Some(0)),
            i32,
            Int32,
        ...
        test_function!(
            FindInSetFunc::new(),
            vec![
                ColumnarValue::from(ScalarValue::Utf8View(Some(String::from("a")))),
    ...
    test_find_in_set!(
        ...
        vec![
            ...
            ColumnarValue::from(ScalarValue::Utf8(Some("b,c,d".to_string()))),
        ],
    ...
    test_find_in_set!(
        ...
        vec![
            ...
            ColumnarValue::from(ScalarValue::Utf8View(Some("a,b,c".to_string()))),
        ],
    ...
    test_find_in_set!(
        ...
        vec![
            ColumnarValue::from(ScalarValue::Utf8View(Some("a".to_string()))),
            ColumnarValue::Array(Arc::new(StringArray::from(vec![None::<&str>; 3]))),
        ],
        Int32Array::from(vec![None::<i32>; 3])
    );
