...
impl SessionState {
    ...
    pub fn with_config_rt(config: SessionConfig, runtime: Arc<RuntimeEnv>) -> Self {
        ...
        let physical_optimizers: Vec<Arc<dyn PhysicalOptimizerRule + Sync + Send>> = vec![
            ...
            // EnforceDistribution is for adding essential repartition
            // to satisfy the required distribution.
            // Please make sure that the whole plan tree is determined.
            Arc::new(EnforceDistribution::new()),
            // `EnforceSorting` is for adding essential local sorting
            // to satisfy the required ordering.
            // Please make sure that the whole plan tree is determined. Since `RepartitionExec`s
            // added by `EnforceDistribution` may invalidate ordering requirement. `EnforceSorting` should run
