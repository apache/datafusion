...
use datafusion_expr::{interval_arithmetic::Interval, statistics::ColumnStatisticsNew};
use datafusion_expr::statistics::{new_generic_from_binary_op, Distribution, StatisticsNew};
    ...
    if let Some(first_file) = all_files.next().await {
        ...
        if conservative_num_rows <= limit {
            while let Some(current) = all_files.next().await {
                ...
                num_rows = add_row_stats(file_stats.num_rows.clone(), num_rows.clone());

                total_byte_size = add_row_stats(file_stats.total_byte_size.clone(), total_byte_size.clone());
