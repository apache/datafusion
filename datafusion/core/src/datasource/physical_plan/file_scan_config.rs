...
impl PartitionColumnProjector {
    ...
    pub fn project(
        ...
        for &(pidx, sidx) in &self.projected_partition_indexes {
            ...
            let partition_value = Cow::Borrowed(p_value);
...
where
    T: ArrowNativeType,
{
    let dict_vals = val.to_array_of_type(inner_type)?;

    let sliced_key_buffer = buffer_gen.get_buffer(len);

    // assemble pieces together
    let mut builder = ArrayData::builder(dict_type.clone())
        .len(len)
...
fn create_output_array(
    ...
    if let DataType::Dictionary(key_type, inner_type) = data_type {
    ...
    val.to_array_of_size_and_type(len, data_type)
}
