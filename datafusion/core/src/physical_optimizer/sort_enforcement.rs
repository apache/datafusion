...
mod tests {
    ...
    async fn test_do_not_remove_sort_with_limit() -> Result<()> {
        ...
        let expected_input = vec![
            ...
            "    UnionExec",
            "      ParquetExec: file_groups={1 group: [[x]]}, projection=[nullable_col, non_nullable_col], limit=None, output_ordering=[nullable_col@0 ASC]",
            "      GlobalLimitExec: skip=0, fetch=100",
            "        LocalLimitExec: fetch=100",
            "          SortExec: expr=[nullable_col@0 ASC,non_nullable_col@1 ASC]",
            "            ParquetExec: file_groups={1 group: [[x]]}, projection=[nullable_col, non_nullable_col], limit=None, output_ordering=[]",
        ];
    ...
    async fn test_union_inputs_different_sorted() -> Result<()> {
        ...
        let expected_input = vec![
            "SortPreservingMergeExec: [nullable_col@0 ASC]",
            "  UnionExec",
            "    ParquetExec: file_groups={1 group: [[x]]}, projection=[nullable_col, non_nullable_col], limit=None, output_ordering=[nullable_col@0 ASC, non_nullable_col@1 ASC]",
            "    SortExec: expr=[nullable_col@0 ASC]",
            "      ParquetExec: file_groups={1 group: [[x]]}, projection=[nullable_col, non_nullable_col], limit=None, output_ordering=[]",
        ];
        // should not add a sort at the output of the union, input plan should not be changed
    ...
    async fn test_sort_merge_join_order_by_left() -> Result<()> {
        ...
        for join_type in join_types {
            ...
            let expected_input = vec![
                ...
                join_plan2.as_str(),
                "    ParquetExec: file_groups={1 group: [[x]]}, projection=[nullable_col, non_nullable_col], limit=None, output_ordering=[]",
                "    ParquetExec: file_groups={1 group: [[x]]}, projection=[col_a, col_b], limit=None, output_ordering=[]",
            ];
