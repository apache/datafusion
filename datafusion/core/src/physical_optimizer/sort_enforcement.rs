...
fn parallelize_sorts(
        ...
        && plan.output_partitioning().partition_count() <= 1
    {
        ...
        println!("prev_layer.ordering_equivalence_properties(): {:?}", prev_layer.ordering_equivalence_properties());
        println!("prev_layer.equivalence_properties(): {:?}", prev_layer.equivalence_properties());
        println!("prev_layer.output_ordering(): {:?}", prev_layer.output_ordering());
...
fn analyze_immediate_sort_removal(
    ...
    if let Some(sort_exec) = plan.as_any().downcast_ref::<SortExec>() {
        ...
        println!("sort_input.ordering_equivalence_properties().classes(): {:?}", sort_input.ordering_equivalence_properties().classes());
...
mod tmp_tests {
    ...
    fn partition_record_batch<const STREAM: bool>(
        mut remainder: RecordBatch,
        batch_size: usize,
    ) -> Vec<RecordBatch> {
        let mut batches = vec![];
        if STREAM {
    ...
    fn get_test_record_batch(schema: &SchemaRef, n_elem: usize) -> RecordBatch {
        let entries = (0..n_elem as i32).collect::<Vec<i32>>();
        let record_batch = RecordBatch::try_new(
            schema.clone(),
            vec![
                Arc::new(Int32Array::from(entries.clone())),
                Arc::new(Int32Array::from(entries.clone())),
                Arc::new(Int32Array::from(entries.clone())),
                Arc::new(Int32Array::from(entries.clone())),
            ],
    ...
    fn memory_exec_sorted<const STREAM: bool>(
        schema: SchemaRef,
        sort_exprs: Vec<PhysicalSortExpr>,
        n_elem: usize,
        batch_size: usize,
    ) -> Arc<dyn ExecutionPlan> {
        let record_batch = get_test_record_batch(&schema, n_elem);
        let batches = partition_record_batch::<STREAM>(record_batch, batch_size);
        let partitions = vec![batches];
        Arc::new(
            MemoryExec::try_new(&partitions, schema, None)
                .unwrap()
                .with_sort_information(sort_exprs),
        )
    }
...
LOCATION 'tests/data/window_2.csv'",
        ...
        // let source = memory_exec_sorted::<true>(schema.clone(), sort_exprs.clone(), n_elem, batch_size);
        ...
        ctx.register_table("stream_table", Arc::new(mem_table))?;
        ...
        // let sql = "SELECT RANK() OVER(ORDER BY a, b) FROM stream_table";

        //       let sql = "SELECT *
        // FROM (SELECT *, ROW_NUMBER() OVER() as rn1
        //      FROM annotated_data_finite2 ) as l_table
        // JOIN annotated_data_finite2 as r_table
        ...
        //       let sql = "SELECT *
        // FROM (SELECT *, ROW_NUMBER() OVER() as rn1
        //      FROM annotated_data_infinite2 ) as l_table
        // JOIN annotated_data_infinite2 as r_table
        // ON l_table.a = r_table.a
        // ORDER BY l_table.rn1";
        ...
        //      FROM annotated_data_infinite2 ) as r_table
        // ON l_table.a = r_table.a
        // ORDER BY r_table.a0";

       //  let sql = "SELECT *, CAST(ROW_NUMBER() OVER() AS BIGINT) as rn1
  ...
  JOIN (SELECT *, ROW_NUMBER() OVER() as rn1
       FROM annotated_data_finite2 ) as r_table
  ON l_table.a = r_table.a
  ORDER BY l_table.a ASC NULLS FIRST, l_table.b, l_table.c, r_table.rn1
  LIMIT 5";

        // let sql = "SELECT *
        // FROM stream_table as l_table
        // JOIN stream_table as r_table
        // ON l_table.a = r_table.a
        // ";
    ...
    #[tokio::test]
    #[ignore]
    async fn test_sort_merge_join() -> Result<()> {
        let session_config = SessionConfig::new().with_batch_size(1);
        ...
        let schema = create_test_schema()?;
        let sort_exprs = vec![sort_expr("a", &schema)];
        let source = memory_exec_sorted::<true>(
            schema.clone(),
            sort_exprs.clone(),
            n_elem,
            batch_size,
        ...
        let sort_merge_join = Arc::new(SortMergeJoinExec::try_new(
            source.clone(),
            source.clone(),
            on_columns,
            JoinType::Inner,
        ...
        let sort_merge_join = collect(sort_merge_join, task_ctx.clone()).await.unwrap();
        for chunk in sort_merge_join {
            print_batches(&[chunk])?;
        }
        assert_eq!(0, 1);
        Ok(())
