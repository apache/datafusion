...
mod tmp_tests {
    ...
    async fn test_trivial() -> Result<()> {
        ...
        let _dataframe = ctx
            .sql("create table foo (ts1 timestamp, ts2 timestamp) as values ('2000-01-01T00:00:00'::timestamp, '2000-01-01T00:00:00'::timestamp),
             ('2000-01-01T00:00:00'::timestamp, '2000-02-01T00:00:00'::timestamp), 
             ('2000-01-01T00:00:00'::timestamp, '2000-03-01T00:00:00'::timestamp);")
            .await?;
        ...
        let dataframe = ctx
            .sql(
                ...
                foo a1
                JOIN
                foo a2 ON a1.ts1 = a2.ts2
                AND a1.ts1 - a2.ts1 < INTERVAL '15 minute'
                AND a1.ts2 - a2.ts2 >= INTERVAL '15 minute';",
            )
            .await?;
