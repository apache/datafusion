...
mod schema_rewriter_tests {
    ...
    /// Test basic functionality of PhysicalExprSchemaRewriter
    #[test]
    fn test_schema_rewriter_basic() {
        ...
        let logical_schema = Schema::new(vec![
            Field::new("a", DataType::Int64, false), // Type mismatch
            Field::new("b", DataType::Utf8, true),
            Field::new("c", DataType::Float64, true), // Missing in physical
        ]);

        let rewriter = PhysicalExprSchemaRewriter::new(&physical_schema, &logical_schema);

        // Test column with type cast
        let column_a = col("a", &physical_schema).unwrap();
        let result = rewriter.rewrite(column_a).unwrap();
        assert!(result.to_string().contains("CAST"));

        // Test column with no changes needed
        let column_b = col("b", &physical_schema).unwrap();
        let result = rewriter.rewrite(column_b).unwrap();
        assert_eq!(result.to_string(), "b@1");

        // Test missing column (should be replaced with null)
        let column_c =
            Arc::new(datafusion::physical_expr::expressions::Column::new("c", 2));
        let result = rewriter.rewrite(column_c).unwrap();
        assert!(result.to_string().contains("NULL"));
    }

    /// Test edge case: non-nullable missing column should error
    #[test]
    fn test_schema_rewriter_non_nullable_missing_error() {
        let physical_schema = Schema::new(vec![Field::new("a", DataType::Int32, false)]);

        let logical_schema = Schema::new(vec![
            Field::new("a", DataType::Int32, false),
            Field::new("b", DataType::Utf8, false), // Non-nullable missing
        ]);

        let rewriter = PhysicalExprSchemaRewriter::new(&physical_schema, &logical_schema);
        let column_b =
            Arc::new(datafusion::physical_expr::expressions::Column::new("b", 1));
        let result = rewriter.rewrite(column_b);
    ...
    /// Test partition columns functionality
    #[test]
    fn test_schema_rewriter_partition_columns() {
        let physical_schema = Schema::new(vec![Field::new("a", DataType::Int32, false)]);
        ...
        let partition_column = Arc::new(
            datafusion::physical_expr::expressions::Column::new("partition_col", 0),
        );
        let result = rewriter.rewrite(partition_column).unwrap();
        assert!(result.to_string().contains("test_value"));
    }

    /// Test complex expressions with multiple columns
    #[test]
    fn test_schema_rewriter_complex_expressions() {
        let physical_schema = Schema::new(vec![
            Field::new("a", DataType::Int32, false),
            Field::new("b", DataType::Utf8, true),
        ]);
        ...
        let logical_schema = Schema::new(vec![
            Field::new("a", DataType::Int64, false), // Type mismatch
            Field::new("b", DataType::Utf8, true),
            Field::new("c", DataType::Float64, true), // Missing
        ]);

        let rewriter = PhysicalExprSchemaRewriter::new(&physical_schema, &logical_schema);

        // Create a complex expression: a + 10 > 5
        let column_a = col("a", &physical_schema).unwrap();
        let literal_10 = lit(ScalarValue::Int64(Some(10)));
        let literal_5 = lit(ScalarValue::Int64(Some(5)));

        let add_expr = Arc::new(datafusion::physical_expr::expressions::BinaryExpr::new(
            column_a,
            datafusion_expr::Operator::Plus,
            literal_10,
        ));

        let gt_expr = Arc::new(datafusion::physical_expr::expressions::BinaryExpr::new(
            add_expr,
            datafusion_expr::Operator::Gt,
            literal_5,
        ));

        let result = rewriter.rewrite(gt_expr).unwrap();
        let result_str = result.to_string();
        // Should contain cast for column 'a'
        assert!(result_str.contains("CAST"));
        // Should still have the same logical structure
        assert!(result_str.contains("+ 10"));
        assert!(result_str.contains("> 5"));
    }

    /// Test that invalid casts are properly handled
    #[test]
    fn test_schema_rewriter_invalid_cast_error() {
        let physical_schema = Schema::new(vec![Field::new("a", DataType::Binary, false)]);

        // Try to cast binary to struct (should fail)
        let logical_schema = Schema::new(vec![Field::new(
            "a",
            DataType::Struct(vec![Field::new("field", DataType::Int32, true)].into()),
            false,
        )]);

        let rewriter = PhysicalExprSchemaRewriter::new(&physical_schema, &logical_schema);
        let column_a = col("a", &physical_schema).unwrap();
        let result = rewriter.rewrite(column_a);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Cannot cast"));
    }

    /// Test that column indexes are properly handled
    #[test]
    fn test_schema_rewriter_column_indexes() {
        let physical_schema = Schema::new(vec![
        ...
        let logical_schema = Schema::new(vec![
            Field::new("a", DataType::Int64, false), // Different index and type
            Field::new("b", DataType::Utf8, true),
        ]);

        let rewriter = PhysicalExprSchemaRewriter::new(&physical_schema, &logical_schema);

        // Create column with wrong index (should be corrected)
        let column_a_wrong_idx =
            Arc::new(datafusion::physical_expr::expressions::Column::new("a", 0));
        let result = rewriter.rewrite(column_a_wrong_idx).unwrap();
        let result_str = result.to_string();
        // Should contain cast and correct index
        assert!(result_str.contains("CAST"));
        assert!(result_str.contains("a@1")); // Should be index 1 in physical schema
    }

    /// Mock rewrite hook for testing custom rewriter integration
    #[derive(Debug)]
    struct TestRewriteHook;

    impl PhysicalSchemaExprRewriter for TestRewriteHook {
        fn rewrite(
            &self,
            expr: Arc<dyn PhysicalExpr>,
            _physical_file_schema: &Schema,
        ) -> datafusion::common::Result<Arc<dyn PhysicalExpr>> {
            // Simple hook that adds 1 to any integer literal
            if let Some(literal) =
                expr.as_any()
                    .downcast_ref::<datafusion::physical_expr::expressions::Literal>()
            {
                if let ScalarValue::Int32(Some(val)) = literal.value() {
                    return Ok(lit(ScalarValue::Int32(Some(val + 1))));
                }
            }
            Ok(expr)
        }
    ...
    fn test_schema_rewriter_physical_only_columns() {
        let physical_schema = Schema::new(vec![
            Field::new("a", DataType::Int32, false),
            Field::new("_computed_col", DataType::Float64, false), // Only in physical
        ]);
        ...
        // Reference a column that only exists in physical schema
        let computed_col = Arc::new(datafusion::physical_expr::expressions::Column::new(
            "_computed_col",
            1,
        ));
        let result = rewriter.rewrite(computed_col);

        // Should succeed and use the physical column
        assert!(result.is_ok());
        assert_eq!(result.unwrap().to_string(), "_computed_col@1");
    }

    /// Test that rewriter handles completely unknown columns properly
    #[test]
    fn test_schema_rewriter_unknown_column_error() {
        let physical_schema = Schema::new(vec![Field::new("a", DataType::Int32, false)]);
        let logical_schema = Schema::new(vec![Field::new("a", DataType::Int32, false)]);

        let rewriter = PhysicalExprSchemaRewriter::new(&physical_schema, &logical_schema);

        // Reference a column that doesn't exist in either schema
        let unknown_col = Arc::new(datafusion::physical_expr::expressions::Column::new(
            "unknown", 99,
        ));
        let result = rewriter.rewrite(unknown_col);
