...
/// Tests for array function predicate pushdown to Parquet files.
/// These tests verify that array_has, array_has_all, and array_has_any predicates
/// are correctly pushed down to the DataSourceExec node in the physical plan.
mod array_function_pushdown_tests {
    use super::*;
    ...
    /// Create a temporary Parquet file with array column data for testing
    fn create_test_parquet_file() -> Result<NamedTempFile> {
        let item_field = Arc::new(Field::new("item", DataType::Utf8, true));
        let schema = Arc::new(Schema::new(vec![
            Field::new("id", DataType::Int32, false),
            Field::new("tags", DataType::List(item_field), true),
        ]));
        ...
        let temp_file = tempfile::Builder::new().suffix(".parquet").tempfile()?;
    ...
    /// Helper to verify that a predicate appears in the DataSourceExec node,
    /// indicating successful pushdown to the Parquet decoder.
    fn assert_predicate_pushed_down(
        physical_plan_display: &str,
        expected_predicate: &str,
    ) {
        // Check for DataSourceExec with the predicate
        assert!(
            physical_plan_display.contains("DataSourceExec"),
            "Physical plan should contain DataSourceExec:\n{physical_plan_display}"
        );
        assert!(
            physical_plan_display.contains(&format!("predicate={expected_predicate}")),
            "DataSourceExec should contain predicate '{expected_predicate}':\n{physical_plan_display}"
        );
    }

    #[tokio::test]
    async fn test_array_has_pushdown_in_physical_plan() {
        let ctx = SessionContext::new();
        let parquet_file = create_test_parquet_file().unwrap();
        ...
        let df = ctx
            .sql("SELECT * FROM test_table WHERE array_has(tags, 'rust')")
            .await
            .unwrap();
        ...
        // Verify pushdown appears in physical plan
        assert_predicate_pushed_down(&plan_display, "array_has(tags@1, rust)");
    }

    #[tokio::test]
    async fn test_array_has_all_pushdown_in_physical_plan() {
        let ctx = SessionContext::new();
        let parquet_file = create_test_parquet_file().unwrap();
        ...
        let df = ctx
            .sql(
                "SELECT * FROM test_table WHERE array_has_all(tags, ['rust', 'performance'])",
            )
            .await
            .unwrap();
        ...
        // Verify pushdown appears in physical plan
        assert_predicate_pushed_down(
            &plan_display,
            "array_has_all(tags@1, [rust, performance])",
        );
    }

    #[tokio::test]
    async fn test_array_has_any_pushdown_in_physical_plan() {
        let ctx = SessionContext::new();
        let parquet_file = create_test_parquet_file().unwrap();
        ...
        let df = ctx
            .sql("SELECT * FROM test_table WHERE array_has_any(tags, ['python', 'go'])")
            .await
            .unwrap();
        ...
        assert_predicate_pushed_down(
            &plan_display,
            "array_has_any(tags@1, [python, go])",
        );
    }

    #[tokio::test]
    async fn test_complex_predicate_pushdown_in_physical_plan() {
        let ctx = SessionContext::new();
        let parquet_file = create_test_parquet_file().unwrap();
        ...
        let df = ctx
            .sql("SELECT * FROM test_table WHERE array_has_all(tags, ['rust']) OR array_has_any(tags, ['python', 'go'])")
            .await
            .unwrap();
        ...
        // Verify the complex predicate is pushed down
        assert_predicate_pushed_down(
            &plan_display,
            "array_has_all(tags@1, [rust]) OR array_has_any(tags@1, [python, go])",
        );
    }

    #[tokio::test]
    async fn test_array_function_with_other_predicates_pushdown() {
        let ctx = SessionContext::new();
        let parquet_file = create_test_parquet_file().unwrap();
        ...
        let df = ctx
            .sql("SELECT * FROM test_table WHERE id > 1 AND array_has(tags, 'rust')")
            .await
            .unwrap();
        ...
        // Verify both predicates are pushed down
        assert_predicate_pushed_down(
            &plan_display,
            "id@0 > 1 AND array_has(tags@1, rust)",
        );
    }
}
