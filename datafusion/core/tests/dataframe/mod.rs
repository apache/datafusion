...
async fn test_count_wildcard_on_sort() -> Result<()> {
    ...
    |               |       AggregateExec: mode=FinalPartitioned, gby=[b@0 as b], aggr=[count(Int64(1))], stream=PartialFinalHashAggregateStream |
    |               |         RepartitionExec: partitioning=Hash([b@0], 4), input_partitions=1                                                   |
    |               |           AggregateExec: mode=Partial, gby=[b@0 as b], aggr=[count(Int64(1))], stream=RawPartialHashAggregateStream        |
    |               |             DataSourceExec: partitions=1, partition_sizes=[1]                                                              |
    |               |                                                                                                                            |
    +---------------+----------------------------------------------------------------------------------------------------------------------------+
    "
    ...
    | physical_plan | SortPreservingMergeExec: [count(*)@1 ASC NULLS LAST]                                                                          |
    |               |   SortExec: expr=[count(*)@1 ASC NULLS LAST], preserve_partitioning=[true]                                                    |
    |               |     AggregateExec: mode=FinalPartitioned, gby=[b@0 as b], aggr=[count(1) as count(*)], stream=PartialFinalHashAggregateStream |
    |               |       RepartitionExec: partitioning=Hash([b@0], 4), input_partitions=1                                                        |
    |               |         AggregateExec: mode=Partial, gby=[b@0 as b], aggr=[count(1) as count(*)], stream=RawPartialHashAggregateStream        |
    |               |           DataSourceExec: partitions=1, partition_sizes=[1]                                                                   |
    |               |                                                                                                                               |
    +---------------+-------------------------------------------------------------------------------------------------------------------------------+
    "
...
async fn test_count_wildcard_on_where_scalar_subquery() -> Result<()> {
    ...
    |               |           AggregateExec: mode=FinalPartitioned, gby=[a@0 as a], aggr=[count(Int64(1))], stream=PartialFinalHashAggregateStream |
    |               |             RepartitionExec: partitioning=Hash([a@0], 4), input_partitions=1                                                   |
    |               |               AggregateExec: mode=Partial, gby=[a@0 as a], aggr=[count(Int64(1))], stream=RawPartialHashAggregateStream        |
    |               |                 DataSourceExec: partitions=1, partition_sizes=[1]                                                              |
    |               |       DataSourceExec: partitions=1, partition_sizes=[1]                                                                        |
    |               |                                                                                                                                |
    +---------------+--------------------------------------------------------------------------------------------------------------------------------+
    ...
    |               |           AggregateExec: mode=FinalPartitioned, gby=[a@0 as a], aggr=[count(1) as count(*)], stream=PartialFinalHashAggregateStream |
    |               |             RepartitionExec: partitioning=Hash([a@0], 4), input_partitions=1                                                        |
    |               |               AggregateExec: mode=Partial, gby=[a@0 as a], aggr=[count(1) as count(*)], stream=RawPartialHashAggregateStream        |
    |               |                 DataSourceExec: partitions=1, partition_sizes=[1]                                                                   |
    |               |       DataSourceExec: partitions=1, partition_sizes=[1]                                                                             |
    |               |                                                                                                                                     |
    +---------------+-------------------------------------------------------------------------------------------------------------------------------------+
