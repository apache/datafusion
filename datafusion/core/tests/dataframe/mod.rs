...
async fn unnest_with_redundant_columns() -> Result<()> {
    ...
    let expected = vec![
        ...
        "    Aggregate: groupBy=[[shapes.shape_id]], aggr=[[array_agg(shapes.shape_id) AS shape_id2]] [shape_id:UInt32, shape_id2:List(Field { name: \"item\", data_type: UInt32, nullable: true, dict_is_ordered: false, metadata: {} });N]",
