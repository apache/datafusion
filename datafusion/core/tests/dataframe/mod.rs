...
async fn test_sql_support_sql_to_view_types() -> Result<()> {
    // Test that the default behaviour is false
    let cfg = SessionConfig::new();
    let ctx = SessionContext::new_with_config(cfg);

    ctx.sql("CREATE TABLE t (a varchar)").await?;

    let df = ctx.sql("describe t").await?.collect().await?;
    let expected = [
        "+-------------+-----------+-------------+",
    ...
    // Test that the behaviour can be changed to true which will return Utf8View
    let mut cfg = SessionConfig::new();
    let opts = cfg.options_mut();
    opts.sql_parser.support_varchar_to_view_types = true;
    let ctx = SessionContext::new_with_config(cfg);
