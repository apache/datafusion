...
use arrow_array::{Array, Float32Array, Float64Array, Int64Array, UnionArray};
...
#[tokio::test]
async fn test_low_cardinality_multi_groupby() -> Result<()> {
    async fn query(ctx: &mut SessionContext, sql: &str) -> Result<Vec<RecordBatch>> {
        // execute the query
...
#[tokio::test]
async fn test_high_cardinality_multi_groupby() -> Result<()> {
    async fn query(ctx: &mut SessionContext, sql: &str) -> Result<Vec<RecordBatch>> {
        // execute the query
        ctx.sql(sql).await?.collect().await
    }
