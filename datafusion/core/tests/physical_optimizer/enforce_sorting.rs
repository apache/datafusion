...
impl EnforceSortingTest {
    ...
    pub(crate) fn run(&self) -> String {
        ...
        let optimized_physical_plan = EnsureRequirements::new()
            .optimize(Arc::clone(&self.plan), &config)
            .expect("ensure_requirements failed");
...
async fn test_commutativity() -> Result<()> {
    ...
    let config = ConfigOptions::new();
    // Idempotency check: under the previous design this verified that
    // `[EnforceDistribution, EnforceSorting]` produced the same plan as
    // `[EnforceSorting, EnforceDistribution, EnforceSorting]`. With the
    // merged `EnsureRequirements` rule the property collapses to
    // "running EnsureRequirements N times is the same as running it once",
    // which is the idempotency guarantee the merged rule provides.
    let rule = EnsureRequirements::new();
    let first_plan = rule.optimize(orig_plan.clone(), &config)?;
...
async fn check_unique_build_join_unbounded_limit(
    ...
    let mut plan: Arc<dyn ExecutionPlan> =
        Arc::new(SortExec::new(required, join).with_fetch(Some(1)));
    ...
    if join_type == JoinType::Left {
        assert!(
            displayable(plan.as_ref())
                .indent(true)
                .to_string()
                .contains("SortMergeJoinExec: join_type=Left")
        );
    }
    ...
    .unwrap_or_else(|_| {
        panic!(
            "LIMIT waited for an unfinished probe-key group:\n{}",
            displayable(plan.as_ref()).indent(true)
        )
    })?;
    // SMJ emits probe columns first; use the same column order for the assertion.
    let batches = if join_type == JoinType::Left {
        batches
            .iter()
            .map(|batch| batch.project(&[1, 2, 0]))
...
/// A filter can make equal probe keys alternate between a build row and NULLs.
#[tokio::test]
async fn test_unique_build_outer_join_filter_requires_sort() -> Result<()> {
    let build = unique_join_build()?;
    ...
    .unwrap();
    let mut plan: Arc<dyn ExecutionPlan> = Arc::new(SortExec::new(required, join));
    let ctx =
