...
async fn test_concat_lex_ordering_with_project(optimize: bool) -> Result<()> {
    ...
    let batch = record_batch!(("a", Utf8, ["a", "a0"]), ("b", Utf8, ["1", "1"]))?;
    let schema = batch.schema();
    ...
    let concat = Arc::new(datafusion_physical_expr::ScalarFunctionExpr::try_new(
...
///     SortExec: expr=[c@2 ASC, a@0 ASC, b@1 ASC], preserve_partitioning=[false]
...
async fn test_concat_lex_ordering_with_filter(optimize: bool) -> Result<()> {
    ...
    let predicate =
        Arc::new(BinaryExpr::new(col("c", &schema)?, Operator::Eq, concat)) as _;
    let filter = filter_exec(predicate, sort_cab);
    let plan = sort_exec(
        LexOrdering::new([sort_expr("a", &schema), sort_expr("b", &schema)]).unwrap(),
        filter,
    );
...
async fn assert_plan_output_eq(
    ...
    let results =
        datafusion_physical_plan::collect(plan, Arc::new(TaskContext::default())).await?;
    let all = arrow::compute::concat_batches(&results[0].schema(), &results)?;
    let expected = create_array!(Utf8, expected_result) as ArrayRef;
    assert_eq!(
        all.column(0),
        &expected,
        "output must satisfy the requested ordering (optimize={optimize}); plan:\n{plan_str}"
    );
    Ok(())
}
