...
mod sp_fuzz_tests {
    ...
    use datafusion::physical_plan::memory::MemoryExec;
    use datafusion::physical_plan::repartition::sort_preserving_repartition::SPRepartitionExec;
    ...
    async fn run_sort_preserving_repartition_test(
        ...
        let hash_exprs = vec![col("d", &schema).unwrap()];
        ...
        print_batches(&vec![concat_res.clone()]).unwrap();
        print_batches(&vec![concat_input_record.clone()]).unwrap();
        assert_eq!(concat_res, concat_input_record);

        assert_eq!(0, 1);
        // println!("-------RESULT-------");
        // print_batches(&vec![concat_res]).unwrap();
        // println!("-------INPUT-------");
        // print_batches(&vec![concat_input_record]).unwrap();
    }
