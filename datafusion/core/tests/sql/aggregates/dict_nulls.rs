...
/// Test MIN timestamp with fuzz table containing dictionary columns with null keys and values (single and multiple partitions) - alternative test
#[tokio::test]
async fn test_min_timestamp_with_fuzz_table_dict_nulls_alternative() -> Result<()> {
    let (ctx_single, ctx_multi) = setup_fuzz_timestamp_test_contexts().await?;

    // Execute the SQL query with MIN timestamp aggregation
    let sql = "SELECT
        utf8_low,
        u8_low,
        dictionary_utf8_low,
        min(timestamp_us) as min_timestamp
    FROM
        fuzz_table
    GROUP BY
        utf8_low,
    ...
    assert_snapshot!(
        ...
        @r###"
    +----------+--------+---------------------+---------------+
    | utf8_low | u8_low | dictionary_utf8_low | min_timestamp |
    +----------+--------+---------------------+---------------+
    | alpha    | 10     | dict_x              | 1970-01-01T00:00:01 |
    | beta     | 20     |                     | 1970-01-01T00:00:02 |
    | delta    | 20     |                     | 1970-01-01T00:00:03.500 |
