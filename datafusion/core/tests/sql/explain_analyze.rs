...
async fn test_physical_plan_display_indent() {
    ...
    let expected = vec![
        ...
        "                      CsvExec: file_groups={1 group: [[ARROW_TEST_DATA/csv/aggregate_test_100.csv]]}, projection=[c1, c12], limit=None, output_ordering=[], has_header=true",
    ];
...
async fn test_physical_plan_display_indent_multi_children() {
    ...
    let expected = vec![
        ...
        "        RepartitionExec: partitioning=Hash([Column { name: \"c1\", index: 0 }], 9000), input_partitions=9000",
        "          RepartitionExec: partitioning=RoundRobinBatch(9000), input_partitions=1",
        "            CsvExec: file_groups={1 group: [[ARROW_TEST_DATA/csv/aggregate_test_100.csv]]}, projection=[c1], limit=None, output_ordering=[], has_header=true",
        "      CoalesceBatchesExec: target_batch_size=4096",
