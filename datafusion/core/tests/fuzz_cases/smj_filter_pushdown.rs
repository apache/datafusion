...
/// Regression test for a non-deterministic correctness bug: the SMJ dynamic
/// filter is a single expression shared across the concurrently-executing hash
/// partitions feeding the join. An earlier design advanced a one-sided bound as
/// partitions made progress (and *raised* it on partition exhaustion), which
/// could prune valid join rows depending on thread scheduling.
///
/// This exercises the failure shape directly — join keys with NULLs and values
/// at the range extremes, multiple partitions, and a tiny batch size — and runs
/// it repeatedly. The match set is keys {1, 3}; the previously-buggy filter
/// would intermittently drop the key=3 (max) row.
#[tokio::test]
async fn test_smj_dynamic_filter_multi_partition_null_keys_correct() {
    async fn build(enable_dynamic_filters: bool) -> SessionContext {
        let cfg = SessionConfig::new()
            .with_target_partitions(2)
            .with_batch_size(2)
            .set_bool("datafusion.optimizer.prefer_hash_join", false)
        ...
        .unwrap();
        write_parquet(&store, "l.parquet", &left).await;
        write_parquet(&store, "r.parquet", &right).await;
        register_listing_table(&ctx, "l", Arc::clone(&schema), "l.parquet");
        register_listing_table(&ctx, "r", schema, "r.parquet");
        ctx
    }

    let query = "SELECT l.id, l.tag, r.tag FROM l INNER JOIN r ON l.id = r.id";

    // Baseline (filter off) is the ground truth: keys {1, 3} match → 2 rows.
    let ctx_off = build(false).await;
    let expected_batches = ctx_off.sql(query).await.unwrap().collect().await.unwrap();
    let expected_rows: usize = expected_batches.iter().map(|b| b.num_rows()).sum();
    assert_eq!(expected_rows, 2, "expected 2 result rows (keys 1 and 3)");
    let expected = pretty_format_batches(&expected_batches)
        .unwrap()
        .to_string();

    // Run many times to surface any scheduling-dependent pruning.
    for i in 0..40 {
        let ctx_on = build(true).await;
        let actual = pretty_format_batches(
            &ctx_on.sql(query).await.unwrap().collect().await.unwrap(),
        )
        .unwrap()
        .to_string();
        assert_eq!(
