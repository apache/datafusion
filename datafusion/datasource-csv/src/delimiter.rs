...
//! Groups CSV input into complete records for schema inference.
...
impl LineDelimiter {
    ...
    fn finish(&mut self) -> Result<bool> {
        if !self.remainder.is_empty() {
            if self.is_quote {
                return Err(exec_datafusion_err!("encountered unterminated string"));
            }
            if self.is_escape {
                return Err(exec_datafusion_err!(
                    "encountered trailing escape character"
                ));
            }
