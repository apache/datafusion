...
//!
//! Partition identity is preserved across the cut: upstream partition
//! `p` feeds downstream partition `p`. Load imbalance at the scan is
//! handled by file-level work-stealing in `FileStream`, so we don't
//! need a shared MPMC queue here.
...
/// Downstream source for a pipeline cut. One [`Inbox`] per output
/// partition; each is taken once by its corresponding `execute(p)`
/// call.
