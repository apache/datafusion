...
use datafusion_common::{cast::as_generic_list_array, utils::array_into_list_array};	
use datafusion_functions_nested::{	
    array_has_dispatch, array_has_dispatch_eq_kernel, general_array_has
};	
use rand::Rng;	
...
fn criterion_benchmark(c: &mut Criterion) {	
    ...
    c.bench_function("array_has eq kernel approach", |b| {	
        b.iter(|| {	
            let is_contained =	
                black_box(array_has_dispatch_eq_kernel::<i32>(&array, &sub_array).unwrap());	
            assert_eq!(&is_contained, &expected);	
        });	
    });	

    c.bench_function("array_has simple iterate approach", |b| {	
        b.iter(|| {	
            let is_contained = black_box(	
                array_has_dispatch::<i32>(	
                    &array,	
                    &sub_array,	
                )	
                .unwrap(),	
