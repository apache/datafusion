...
fn try_array_position_scalar(args: &[ColumnarValue]) -> Result<Option<ColumnarValue>> {
    ...
    let scalar_element = match &args[1] {
...
fn try_array_positions_scalar(args: &[ColumnarValue]) -> Result<Option<ColumnarValue>> {
    ...
    let scalar_element = match needle_arg {
        ColumnarValue::Scalar(s) => s,
        ColumnarValue::Array(_) => return Ok(None),
    };
    ...
    let result = match haystack.data_type() {
        List(_) => {
            let list = as_list_array(&haystack)?;
            array_positions_scalar::<i32>(list, &element_arr)
        }
        LargeList(_) => {
            let list = as_large_list_array(&haystack)?;
            array_positions_scalar::<i64>(list, &element_arr)
        }
...
fn array_positions_inner(args: &[ArrayRef]) -> Result<ArrayRef> {
    ...
    match &array.data_type() {
        List(_) => general_positions::<i32>(as_list_array(&array)?, element),
        LargeList(_) => general_positions::<i64>(as_large_list_array(&array)?, element),
        array_type => {
...
/// Fast path for `array_positions` when the element is a scalar.
///
/// Performs a single bulk `not_distinct` comparison of the scalar element
/// against the entire flattened values buffer, then walks the result bitmap
/// using offsets to collect all per-row match positions.
