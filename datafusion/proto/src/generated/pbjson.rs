...
impl serde::Serialize for ScanOrderingNode {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        use serde::ser::SerializeStruct;
        let mut len = 0;
        if self.preferred_ordering.is_some() {
            len += 1;
        }
        let mut struct_ser = serializer.serialize_struct("datafusion.ScanOrderingNode", len)?;
        if let Some(v) = self.preferred_ordering.as_ref() {
            struct_ser.serialize_field("preferredOrdering", v)?;
        }
        struct_ser.end()
    }
}
impl<'de> serde::Deserialize<'de> for ScanOrderingNode {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::Deserialize<'de> for GeneratedField {
                ...
                D: serde::Deserializer<'de>,
            {
                ...
                impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
                    ...
                    {
                        match value {
                            "preferredOrdering" | "preferred_ordering" => Ok(GeneratedField::PreferredOrdering),
                            _ => Err(serde::de::Error::unknown_field(value, FIELDS)),
                        }
                    }
                }
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = ScanOrderingNode;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct datafusion.ScanOrderingNode")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<ScanOrderingNode, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut preferred_ordering__ = None;
                while let Some(k) = map_.next_key()? {
                    match k {
                        GeneratedField::PreferredOrdering => {
                            if preferred_ordering__.is_some() {
                                return Err(serde::de::Error::duplicate_field("preferredOrdering"));
                            }
                            preferred_ordering__ = map_.next_value()?;
                        }
                    }
                }
                Ok(ScanOrderingNode {
                    preferred_ordering: preferred_ordering__,
                })
            }
        }
        deserializer.deserialize_struct("datafusion.ScanOrderingNode", FIELDS, GeneratedVisitor)
    }
}
