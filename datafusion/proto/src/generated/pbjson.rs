...
impl<'de> serde::Deserialize<'de> for FileFormatKind {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            ...
            {
                match value {
                    "CSV" => Ok(FileFormatKind::Csv),
                    "JSON" => Ok(FileFormatKind::Json),
                    "PARQUET" => Ok(FileFormatKind::Parquet),
                    "ARROW" => Ok(FileFormatKind::Arrow),
                    _ => Err(serde::de::Error::unknown_variant(value, FIELDS)),
                }
            }
        }
...
impl serde::Serialize for FileFormatWrapper {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("datafusion.FileFormatWrapper", len)?;
        if self.kind != 0 {
...
impl<'de> serde::Deserialize<'de> for FileFormatWrapper {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = FileFormatWrapper;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct datafusion.FileFormatWrapper")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<FileFormatWrapper, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut kind__ = None;
                ...
                Ok(FileFormatWrapper {
                    kind: kind__.unwrap_or_default(),
                    blob: blob__.unwrap_or_default(),
                })
            }
        }
        deserializer.deserialize_struct("datafusion.FileFormatWrapper", FIELDS, GeneratedVisitor)
    }
}
