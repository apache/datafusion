        ...
        S: serde::Serializer,
    {
        ...
        if let Some(v) = self.partition_method.as_ref() {
            match v {
                ...
                partitioning::PartitionMethod::HashSelectionBitmap(v) => {
                    struct_ser.serialize_field("hashSelectionBitmap", v)?;
                }
            }
        }
        struct_ser.end()
                ...
                impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
                        ...
                        E: serde::de::Error,
                    {
                        match value {
                            ...
                            "hashSelectionBitmap" | "hash_selection_bitmap" => Ok(GeneratedField::HashSelectionBitmap),
                            _ => Err(serde::de::Error::unknown_field(value, FIELDS)),
                        }
                    }
                }
...
;
                        ...
                        GeneratedField::HashSelectionBitmap => {
                            if partition_method__.is_some() {
                                return Err(serde::de::Error::duplicate_field("hashSelectionBitmap"));
                            }
                            partition_method__ = map_.next_value::<::std::option::Option<_>>()?.map(partitioning::PartitionMethod::HashSelectionBitmap)
;
                        }
                    }
                }
...
impl serde::Serialize for PhysicalHashSelectionBitmapRepartition {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("datafusion.PhysicalHashSelectionBitmapRepartition", len)?;
        if !self.hash_expr.is_empty() {
            struct_ser.serialize_field("hashExpr", &self.hash_expr)?;
        }
        if self.partition_count != 0 {
...
impl<'de> serde::Deserialize<'de> for PhysicalHashSelectionBitmapRepartition {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = PhysicalHashSelectionBitmapRepartition;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct datafusion.PhysicalHashSelectionBitmapRepartition")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<PhysicalHashSelectionBitmapRepartition, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
                let mut hash_expr__ = None;
                ...
                Ok(PhysicalHashSelectionBitmapRepartition {
                    hash_expr: hash_expr__.unwrap_or_default(),
                    partition_count: partition_count__.unwrap_or_default(),
                })
            }
        }
        deserializer.deserialize_struct("datafusion.PhysicalHashSelectionBitmapRepartition", FIELDS, GeneratedVisitor)
    }
}
