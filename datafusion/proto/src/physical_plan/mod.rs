...
use datafusion_physical_plan::proto::{
    ...
    ExecutionPlanEncodeCtx, ExecutionPlanFromProto, ExecutionPlanRegistry,
};
...
fn unregistered_extension_plan_err(
    ...
    config_datafusion_err!(
        "No decoder is registered for the extension ExecutionPlan '{plan_name}', and the \
         PhysicalExtensionCodec failed with [{codec_error}]. Register the plan in the \
         ExecutionPlanRegistry attached to the decoding session's SessionConfig, or \
         supply a PhysicalExtensionCodec that handles it. Registered extension plans: \
         {registered}"
    )
}
