...
impl ExecutionPlanFromProto for ShuffleExec {
...
impl ExecutionPlanFromProto for SamplerExec {
...
fn registry(
    register: impl FnOnce(&mut ExecutionPlanRegistry) -> Result<()>,
) -> Result<Arc<ExecutionPlanRegistry>> {
...
fn extension_plan_round_trips_with_no_codec_at_all() -> Result<()> {
    let config = session_with(&["worker-1", "worker-2"])
        .with_extension(registry(|registry| registry.register::<ShuffleExec>())?);
...
fn extension_plan_decode_reads_session_state() -> Result<()> {
    ...
    let reader = SessionContext::new_with_config(
        session_with(&["reader-a", "reader-b"])
            .with_extension(registry(|registry| registry.register::<ShuffleExec>())?),
    );
...
fn codec_encoded_extension_plans_are_unaffected() -> Result<()> {
    ...
    let ctx = SessionContext::new_with_config(
        SessionConfig::new()
            .with_extension(registry(|registry| registry.register::<SamplerExec>())?),
    );
...
fn a_named_plan_the_session_does_not_know_reports_the_missing_registration() -> Result<()>
{
    ...
    assert!(
        err.contains("ExecutionPlanRegistry"),
