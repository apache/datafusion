...
impl ShuffleExec {
    fn encode_self(&self, ctx: &ExecutionPlanEncodeCtx<'_>) -> Result<PhysicalPlanNode> {
        ...
        ctx.extension_node::<Self>(payload, self.children())
    }
}

impl ExtensionPlanFromProto for ShuffleExec {
    const NAME: &'static str = "datafusion-test.ShuffleExec";
    ...
    fn try_from_proto(
        ...
        let Some(PhysicalPlanType::Extension(extension)) = &node.physical_plan_type
            ...
            return internal_err!("expected an extension node");
        };
        let children = ctx.decode_children(&extension.inputs)?;
        let payload = extension.node.as_slice();
        ...
        Ok(Arc::new(ShuffleExec {
            stage: proto.stage,
            pool,
            child: only_child(Self::NAME, children)?,
        }))
    }
}
...
impl ExtensionPlanFromProto for SamplerExec {
    const NAME: &'static str = "other-crate.SamplerExec";
...
/// name-blind `try_decode` would happily decode another crate's plan wrongly.
#[derive(Debug)]
struct GreedyCodec {}
...
/// A codec that recognizes a payload but fails while decoding it, with an error
/// kind of its own. Stands in for every codec failure whose cause is *not* a
...
impl PhysicalExtensionCodec for ExhaustedCodec {
    ...
    fn try_encode(
        ...
        internal_err!("ExhaustedCodec does not encode")
    }
}
...
fn extension_plan_round_trips_with_no_codec_at_all() -> Result<()> {
    let config =
        session_with(&["worker-1", "worker-2"]).with_extension(registry(|registry| {
            register_execution_plan::<ShuffleExec>(registry)
        })?);
...
fn extension_plan_decode_reads_session_state() -> Result<()> {
    ...
    let writer = SessionContext::new_with_config(
        session_with(&["writer-only"])
            .with_extension(registry(register_execution_plan::<ShuffleExec>)?),
    );
    let node = encode(
        Arc::new(ShuffleExec {
            stage: 1,
    ...
    let reader = SessionContext::new_with_config(
        session_with(&["reader-a", "reader-b"]).with_extension(registry(|registry| {
            register_execution_plan::<ShuffleExec>(registry)
        })?),
    );
...
fn independent_extension_plans_coexist_without_a_composed_codec() -> Result<()> {
    let config = session_with(&["worker-1"]).with_extension(registry(|registry| {
        register_execution_plan::<ShuffleExec>(registry)?;
        register_execution_plan::<SamplerExec>(registry)
    })?);
    let ctx = SessionContext::new_with_config(config);
    let codec = DefaultPhysicalExtensionCodec {};
...
fn codec_encoded_extension_plans_are_unaffected() -> Result<()> {
    ...
    let ctx = SessionContext::new_with_config(
        SessionConfig::new()
            .with_extension(registry(register_execution_plan::<SamplerExec>)?),
    );
...
fn a_named_plan_the_session_does_not_know_reports_the_missing_registration() -> Result<()>
{
    ...
    assert!(err.contains(ShuffleExec::NAME), "unexpected error: {err}");
    assert!(
    ...
    assert!(err.contains(SamplerExec::NAME), "unexpected error: {err}");
    Ok(())
}
...
fn a_registered_decoder_failing_does_not_fall_back_to_the_codec() -> Result<()> {
    ...
    // wrong decode this replaces: `GreedyCodec` would happily turn a corrupt
    ...
    let node = PhysicalPlanNode {
        physical_plan_type: Some(PhysicalPlanType::Extension(PhysicalExtensionNode {
            ...
            plan_name: Some(ShuffleExec::NAME.to_string()),
        })),
    };
...
fn a_named_plan_can_still_be_decoded_by_a_codec() -> Result<()> {
    // A name this session did not register is not fatal on its own, even
    // when the session has a registry with other plans in it: a codec that
    // understands the payload still decodes it, which is what keeps a mixed
    // fleet (some nodes upgraded, some not) working.
    let config = SessionConfig::new().with_extension(registry(|registry| {
        register_execution_plan::<SamplerExec>(registry)
    })?);
    let ctx = SessionContext::new_with_config(config);
    let node = PhysicalPlanNode {
...
fn the_codecs_own_error_survives_the_missing_registration_hint() -> Result<()> {
    // The hint is added to whatever the codec returned, never substituted for
    // it: a codec can fail for a reason that has nothing to do with a missing
    // registration, and a caller matching on the error kind must still see the
    // kind the codec chose.
    let ctx = SessionContext::new_with_config(
        SessionConfig::new()
            .with_extension(registry(register_execution_plan::<SamplerExec>)?),
    );
    let node = PhysicalPlanNode {
        physical_plan_type: Some(PhysicalPlanType::Extension(PhysicalExtensionNode {
            node: b"whatever".to_vec(),
            inputs: vec![encode(child(), &DefaultPhysicalExtensionCodec {})?],
            plan_name: Some("some-crate.UnknownExec".to_string()),
        })),
    };

    let err = decode(&node, &ctx, &ExhaustedCodec {})
    ...
    assert!(
        text.contains("No decoder is registered")
            && text.contains("some-crate.UnknownExec"),
