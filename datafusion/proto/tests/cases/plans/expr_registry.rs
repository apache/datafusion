...
use datafusion::physical_plan::proto::{ExprDecodeSession, PhysicalExprRegistry};
...
impl<K: TagKind> PhysicalExprFromProto for TaggedExpr<K> {
...
fn registry(
    register: impl FnOnce(&mut PhysicalExprRegistry) -> Result<()>,
) -> Result<Arc<PhysicalExprRegistry>> {
...
fn registry_decodes_named_extension_expr() -> Result<()> {
    ...
    let config = SessionConfig::new()
        ...
        .with_extension(registry(|registry| registry.register::<TagExpr>())?);
...
fn an_unregistered_name_that_the_codec_rejects_says_so() -> Result<()> {
    ...
    let config = SessionConfig::new()
        .with_extension(registry(|registry| registry.register::<OtherExpr>())?);
    let session = SessionContext::new_with_config(config);
    ...
    assert!(
        err.contains("PhysicalExprRegistry"),
...
fn registering_the_same_type_twice_is_a_noop() -> Result<()> {
    let mut registry = PhysicalExprRegistry::new();
    registry.register::<TagExpr>()?;
    registry.register::<TagExpr>()?;
    assert_eq!(registry.names().count(), 1);
    assert!(registry.names().any(|name| name == TagExpr::NAME));
    Ok(())
}
...
fn a_name_collision_between_types_is_an_error() -> Result<()> {
    let mut registry = PhysicalExprRegistry::new();
    registry.register::<TagExpr>()?;
    let err = registry.register::<TagExprClone>().unwrap_err().to_string();
    assert!(
...
fn an_empty_name_is_rejected() {
    let mut registry = PhysicalExprRegistry::new();
    let err = registry.register::<UnnamedExpr>().unwrap_err();
    assert!(
...
fn a_failing_registry_decoder_does_not_fall_through_to_the_codec() -> Result<()> {
    ...
    let config = SessionConfig::new()
        .with_extension(registry(|registry| registry.register::<FailingTagExpr>())?);
    let session = SessionContext::new_with_config(config);
