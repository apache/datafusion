...
async fn roundtrip_expr_api() -> Result<()> {
    ...
    let expr_list = vec![
        ...
        approx_median(lit(2)),
        approx_percentile_cont(lit(2), lit(0.5), None, true),
        approx_percentile_cont(lit(2), lit(0.5), Some(lit(50)), true),
        approx_percentile_cont_with_weight(lit(2), lit(1), lit(0.5), None, true),
        approx_percentile_cont_with_weight(lit(2), lit(1), lit(0.5), Some(lit(50)), true),
        grouping(lit(1)),
        bit_and(lit(2)),
        bit_or(lit(2)),
        bit_xor(lit(2)),
