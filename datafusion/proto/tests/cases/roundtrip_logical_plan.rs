...
fn test_default_codec_encode_decode_all_formats() -> Result<()> {
    ...
    for factory in [csv_factory, json_factory, parquet_factory, arrow_factory] {
        ...
        let decoded = codec.try_decode_file_format(&encoded, &ctx)?;
...
fn test_default_codec_legacy_empty_buffer() -> Result<()> {
    ...
    let empty_buf: &[u8] = &[];
    let decoded = codec.try_decode_file_format(empty_buf, &ctx)?;
    ...
    let re_decoded = codec.try_decode_file_format(&re_encoded, &ctx)?;
    assert_eq!(
        re_decoded.get_ext(),
        "arrow",
        "Re-encoded Arrow format should decode back to Arrow"
...
fn test_default_codec_legacy_raw_bytes_roundtrip() -> Result<()> {
    ...
    let decoded_csv = codec.try_decode_file_format(&csv_raw, &ctx)?;
    decoded_csv
        .as_ref()
        .as_any()
        .downcast_ref::<CsvFormatFactory>()
    ...
    let decoded_json = codec.try_decode_file_format(&json_raw, &ctx)?;
    decoded_json
        .as_ref()
        .as_any()
        .downcast_ref::<JsonFormatFactory>()
    ...
    let decoded_parquet = codec.try_decode_file_format(&parquet_raw, &ctx)?;
    decoded_parquet
        .as_ref()
        .as_any()
        .downcast_ref::<ParquetFormatFactory>()
...
fn test_default_codec_error_invalid_bytes() -> Result<()> {
    ...
    let result = codec.try_decode_file_format(invalid_bytes, &ctx);
