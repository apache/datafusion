...
use datafusion_expr::{
    expr::{WindowFunction, WindowFunctionParams}, expr_rewriter::NamePreserver, planner::{ExprPlanner, PlannerResult, RawWindowExpr}, utils::COUNT_STAR_EXPANSION, Case, Expr, ExprFunctionExt
};
    ...
    fn plan_window(
        ...
        if fun.name() == "count" {
            ...
            if let Some(arg_expr) = rewrite_arg {
                let new_expr = Expr::from(WindowFunction::new(fun.clone(), vec![arg_expr]))
            ...
            match &e {
                ...
                Expr::WindowFunction(wf) => {
                        ...
                        &wf.fun
                    {
                        ...
                        #[expect(deprecated)]
                        if udaf.name().eq_ignore_ascii_case("count") {
                            if let Some(Expr::Wildcard { symbol: Some(symbol), .. }) =
                                wf.params.args.get(0)
                            {
                                ...
                                let new_wf = Expr::from(WindowFunction::new(wf.fun.clone(), new_args))
                        ...
                        if let Some(value_expr) = wf.params.args.get(0) {
                            ...
                            let symbols =
                                datafusion_expr::utils::find_symbol_predicates(value_expr);
                            ...
                            return Ok(datafusion_common::tree_node::Transformed::yes(new_wf));
                        }
                    }
