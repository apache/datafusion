...
async fn test_lag_with_partition_by_and_column_default_sql() -> Result<()> {
    let batch = create_batch(vec![
        (
            "department",
            Arc::new(Int32Array::from(vec![1, 1, 1, 2, 2, 2, 3, 3])),
        ),
        (
            "employee_id",
            Arc::new(Int32Array::from(vec![1, 2, 3, 4, 5, 6, 7, 8])),
...
async fn test_lag_with_null_column_default_sql() -> Result<()> {
    ...
    let expected =
        Int32Array::from(vec![None, Some(100), Some(200), Some(300), Some(400)]);
...
async fn test_lead_lag_mixed_with_column_defaults_sql() -> Result<()> {
    ...
    let expected_prev =
        Int32Array::from(vec![Some(1), Some(10), Some(20), Some(30), Some(40)]);

    let expected_next =
        Int32Array::from(vec![Some(20), Some(30), Some(40), Some(50), Some(10)]);
