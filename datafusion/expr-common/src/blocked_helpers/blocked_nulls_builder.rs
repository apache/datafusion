...
impl<const FIXED_BLOCK_SIZING: bool> BlockedNullsBuilder<FIXED_BLOCK_SIZING> {
    ...
    /// Take every block that counts, see [`Self::num_blocks`], `None` entries have no nulls
...
mod tests {
    ...
    fn fixed_of_len(len: usize) -> Fixed {
        let mut builder = Fixed::new(3);
        for i in 0..len {
            if i % 2 == 0 { builder.push_null() } else { builder.push_non_null() };
        }
        builder
    }
    ...
    fn fixed_and_manual_block_ends_match() {
        ...
        for i in 0..9 {
            if i % 2 == 0 { fixed.push_null() } else { fixed.push_non_null() };
            if i % 2 == 0 { manual.push_null() } else { manual.push_non_null() };
            if (i + 1) % 3 == 0 {
