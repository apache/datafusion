...
pub trait ExprSchemable {
    ...
    fn get_type<S: ExprSchema>(&self, schema: &S, param_data_types: &Vec<DataType>) -> Result<DataType>;
    ...
    fn get_type<S: ExprSchema>(&self, schema: &S, param_data_types: &Vec<DataType>) -> Result<DataType> {
        match self {
            Expr::Alias(expr, _) | Expr::Sort { expr, .. } | Expr::Negative(expr) => {
                expr.get_type(schema, param_data_types)
            }
            Expr::Column(c) => Ok(schema.data_type(c)?.clone()),
            Expr::ScalarVariable(ty, _) => Ok(ty.clone()),
            Expr::Literal(l) => Ok(l.get_datatype()),
            Expr::Case(case) => case.when_then_expr[0].1.get_type(schema, param_data_types),
            ...
            Expr::ScalarUDF { fun, args } => {
                let data_types = args
                    .iter()
                    .map(|e| e.get_type(schema, param_data_types))
            ...
            Expr::BinaryExpr(BinaryExpr {
                ...
                &left.get_type(schema, param_data_types)?,
                op,
                &right.get_type(schema, param_data_types)?,
            ),
            ...
            Expr::Placeholder(param) => {
                ...
                let idx = match index {
                    Ok(index) =>  index - 1,
                    Err(_) => return Err(DataFusionError::Internal(format!(
                        "Invalid placeholder: {}",
                        param
                    )))
                };
            ...
            Expr::GetIndexedField(GetIndexedField { key, expr }) => {
                let data_type = expr.get_type(schema, param_data_types)?;
