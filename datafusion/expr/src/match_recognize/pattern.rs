...
pub enum RowsPerMatch {
    ...
    OneRow,
    /// Return all rows per match with optional empty matches mode
...
impl Display for RowsPerMatch {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            RowsPerMatch::OneRow => write!(f, "ONE_ROW"),
            RowsPerMatch::AllRows(None) => write!(f, "ALL_ROWS"),
            RowsPerMatch::AllRows(Some(mode)) => write!(f, "ALL_ROWS {mode}"),
        }
    }
}
...
impl Display for EmptyMatchesMode {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            EmptyMatchesMode::Show => write!(f, "SHOW_EMPTY_MATCHES"),
            EmptyMatchesMode::Omit => write!(f, "OMIT_EMPTY_MATCHES"),
            EmptyMatchesMode::WithUnmatched => write!(f, "WITH_UNMATCHED_ROWS"),
        }
    }
}
...
impl Display for AfterMatchSkip {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            AfterMatchSkip::PastLastRow => write!(f, "PAST_LAST_ROW"),
            AfterMatchSkip::ToNextRow => write!(f, "TO_NEXT_ROW"),
            AfterMatchSkip::ToFirst(symbol) => write!(f, "TO_FIRST({symbol})"),
            AfterMatchSkip::ToLast(symbol) => write!(f, "TO_LAST({symbol})"),
        }
    }
}
