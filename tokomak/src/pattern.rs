...
pub fn transforming_pattern<A, TR>(name: &str, search: &str, applier: &str, transform: TR, bound_vars: &[Var])->Result<Rewrite<TokomakLogicalPlan, A>, DataFusionError>
where 
A:  Analysis<TokomakLogicalPlan> +'static,
TR: Send+Sync+'static + Fn(&mut EGraph<TokomakLogicalPlan, A>, & mut Subst)->Option<()>{
    ...
    let inner_applier: egg::Pattern<TokomakLogicalPlan> = egg::Pattern::from_str(applier).map_err(|e|DataFusionError::Plan(format!("Rule '{}' could not parse the applier pattern : {}",name, e)))?;
    let applier = ModifyingApplier::new(name, inner_applier, transform, bound_vars)?;
    let res = Rewrite::<TokomakLogicalPlan, A>::new(name, searcher, applier).map_err(|e| DataFusionError::Plan(format!("Rule '{}' could not create rewrite: {}", name, e)));
    return res;
}
...
APP: Applier<TokomakLogicalPlan, A>,
TR: Fn(& mut EGraph<TokomakLogicalPlan, A>, & mut Subst)->Option<()>{
    pub fn new(rule_name:&str, inner_applier: APP, transform: TR, bound_vars: &[Var])->Result<Self, DataFusionError>{
        use std::fmt::Write;
        use std::collections::HashMap;
        let mut bound_set = bound_vars.iter().map(|v| (v.clone(), false)).collect::<HashMap<Var, bool, fxhash::FxBuildHasher>>();
    ...
    fn apply_transform(&self, egraph: &mut EGraph<TokomakLogicalPlan, A>, subst: &mut Subst)->Option<()>{
